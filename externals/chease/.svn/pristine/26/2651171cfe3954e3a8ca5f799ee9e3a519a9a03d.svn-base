...
!!$  use euitm_routines
  ...
  IF (NITMOPT.GT.0 .AND. MOD(NITMOPT,10) .GE. 1) THEN
    ...
    if (associated(equil_in_ids%Code_Parameters%Parameters)) then
      write(6,*) 'size(equil_in_ids%Code_Parameters%Parameters)= ',size(equil_in_ids%Code_Parameters%Parameters)
      deallocate(equil_in_ids%Code_Parameters%Parameters)
    end if
    allocate(equil_in_ids%Code_Parameters%Parameters(size(param_input_ids%parameters_value)))
    equil_in_ids%Code_Parameters%Parameters = param_input_ids%parameters_value
    !
    call copy_ids_to_itm_equilibrium(equil_in_ids,equil_in,flag_status)
  ...
  IF (NITMOPT .EQ. 22) THEN
    ...
    !
    CALL PRESET
    call assign_code_parameters(param_code,istatus)
    if (istatus /= 0) then
...
!!$     call euitm_copy(equil_in,eqchease_in) 
  ...
  IF (NITMOPT .GE. 10) THEN
    ...
    do it=1,size(equil_out)
      allocate(extra_data_itm_to_ids(it)%prof1d_list(1))
      ! 1: magnetic_shear
      ilist = 1
    ...
    !
    iextra_option = 1
    call copy_itm_to_ids_equilibrium(equil_out,equil_out_ids,flag_status)
    if (flag_status .ne. 0) then
