...
!!$  use euitm_routines
  ...
  !
  type(type_equilibrium),pointer      :: equil_in(:) => null(), equil_out(:) => null()
  type(type_param) :: param_code
  type(extra_data_gen),pointer :: extra_data_itm_to_ids(:) ! scans through same time index as equil_in_itm
  integer :: istatus, iout_size, i, isize_comment, flag_status, it, ilist, iextra_option
  !
  !**********************************************************************
  !
  !
  ! At this stage run chease only for one time point inside chease, thus consider only output_size=1
  iout_size = 1
  !
  IF (NITMOPT.GT.0 .AND. MOD(NITMOPT,10) .GE. 1) THEN
    ! Do this only if did get an ids, thus either from within kepler (nitmopt=22) or if reading from database (nitmopt=1 or 11)
    ! !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!Part for IDS compatibility !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    !
    if (associated(param_input_ids%parameters_value)) then
      ! Copy ids equilibrium to itm since chease is coded with CPO structure
      ! 1st copy input params to ids structure
      if (associated(equil_in_ids%code%parameters)) then
        write(6,*) 'size(equil_in_ids%code%parameters)= ',size(equil_in_ids%code%parameters)
        deallocate(equil_in_ids%code%parameters)
      end if
      allocate(equil_in_ids%code%parameters(size(param_input_ids%parameters_value)))
      equil_in_ids%code%parameters = param_input_ids%parameters_value
      !
    else
      print *,'param_input_ids%parameters_value not associated, probably read input from chease_namelist'
    end if
    call copy_ids_to_itm_equilibrium(equil_in_ids,equil_in,iout_size,flag_status)
    if (flag_status .ne. 0) then
      write(6,*) 'problem with copy_ids_to_itm_equilibrium, flag_status = ',flag_status
      call flush(6)
      return
    end if
    ! copy ids_param_input to CPO_code_param
    if (associated(param_input_ids%parameters_value)) then
      allocate(param_code%parameters(size(param_input_ids%parameters_value)))
      param_code%parameters = param_input_ids%parameters_value
      allocate(param_code%default_param(size(param_input_ids%parameters_default)))
  ...
  ELSE
    ! equil_in_ids not defined so just allocate equil_in to size(1)
    !allocate(equil_in(1))
  END IF
  !
  ! !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ! From now on, should be same as ITM version until copy back to IDS part near end 
  ! !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !
  if (associated(equil_in)) then
    iout_size = min(iout_size,size(equil_in))
  else
    iout_size = 1
  end if
  if (nverbose .ge. 3) write(*,*) 'iout_size = ',iout_size
  !
...
!!$     call euitm_copy(equil_in,eqchease_in) 
    IF (NVERBOSE .GE. 1) write(*,*) 'eqchease_in(1)%eqgeometry%geom_axis%r = ',eqchease_in(1)%eqgeometry%geom_axis%r
    !
  ...
  END IF
  !
  if (iout_size > 1) write(0,*) 'iout_size = ',iout_size,' > 1: is it correct?'
  !
  ...
  IF (NITMOPT .GE. 10) THEN
    ! Do this only if need an ids, thus either from within kepler (nitmopt=22) or if writing onto database (nitmopt>=10)
    ! !!!!!!!!!!!!!!!!!!!!!!!!!!!!!! sub-Part for IDS compatibility !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ! !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ! From now on, copy ITM-CPO structure back to IDS structure
    ! !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    !
    ! add extra data needed for ids structure, not included in equil_out ITM-CPO
    allocate(extra_data_itm_to_ids(size(equil_out)))
    do it=1,size(equil_out)
      ! print *,'salut1, it= ',it
      allocate(extra_data_itm_to_ids(it)%prof1d_list(2))
      ! 1: magnetic_shear
      ilist = 1
      allocate(extra_data_itm_to_ids(it)%prof1d_list(ilist)%values(NISO1EFF1))
      extra_data_itm_to_ids(it)%prof1d_list(ilist)%values(1:NISO1EFF1) = eqchease_out_add_1d(1:NISO1EFF1,iishear)
      extra_data_itm_to_ids(it)%prof1d_list(ilist)%name = "magnetic_shear"
      ! 2: length poloidal
      ilist = 2
      allocate(extra_data_itm_to_ids(it)%prof1d_list(ilist)%values(NISO1EFF1))
      extra_data_itm_to_ids(it)%prof1d_list(ilist)%values(1:NISO1EFF1) = (/ RC0P, RLENG(1:NISO1EFF)*R0EXP /)
      extra_data_itm_to_ids(it)%prof1d_list(ilist)%name = "length_pol"
    end do
    !
    iextra_option = 1
    ...
    if (flag_status .ne. 0) then
      write(6,*) 'Problem with copy_itm_to_ids_equilibrium, flag_status = ',flag_status
      call flush(6)
      return
    end if
    !
    ! extra because of bug in wrapper:
!!$  IF (NITMOPT .EQ. 22) THEN
!!$    deallocate(equil_out_ids%timebase)
!!$  end IF
    ! !!!!!!!!!!!!!!!!!!!!!!!!!!!!!! end of sub-Part for IDS compatibility !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  end IF
  !
  ! Deallocate arrays
  !
