...
# Write namelist in specified format (defaults to 'f90')
    ...
    } elsif (lc($format) eq 'matlab') {
        my $namenl = $self->name();
	$header = "% from namelist: $namenl \nnamelistname='$namenl';\ni=0;";
	$footer = ";\n% end of namelist\nvarsorted=sort(fields(namelist_out))';\nnamelist_out.varnames=varnames;\nnamelist_out.varsorted=varsorted;\nnamelist_out.namelistname=namelistname;";
        #
        $head_pref = "";
#        $head_suff = " \$$newline";
#        $slot_pref = ", ";
...
# for matlab
#    my $assmnt = "\n$var";
    my $assmnt = "i=i+1;varnames{i}='$var';\nnamelist_out.$var";
    ...
    if (@vals > 1) {
	$assmnt .= add_array_bracket(join(",", @vals), $format,$type);
    } else {
	$assmnt .= $vals[0];
    }
...
# for Fortran and IDL and matlab
    my $val = shift;
