subroutine copy_itm_to_ids_equilibrium(equil_in_itm,equil_out_ids,flag_status,extra_data_itm_to_ids,extra_option)
  ...
  !
  integer :: i, it, iextra_option, nb_times, ndim1, ndim2, nb_2d, itmax_2d, max_nb_points, nb_lines, nb_points, &
  ...
  ! 1.1 IDS_properties
  !
  equil_out_ids%ids_properties%homogeneous_time = 0
  ...
  do it=1,nb_times
    ...
    equil_out_ids%time_slice(it)%boundary%a_minor = equil_in_itm(it)%eqgeometry%a_minor
    ...
    equil_out_ids%time_slice(it)%global_quantities%b_tor_axis = equil_in_itm(it)%global_param%mag_axis%bphi
...
!!$    equil_out_ids%time_slice(it)%global_quantities%q_min%rho_tor_norm = 
    ...
    equil_out_ids%time_slice(it)%profiles_1d%rho_tor_norm(1:npsi_1d) = 1.0_dp
    ...
    if (NPROF2D .gt. 0) then
      ...
      do iprof2d=1,nb_2d
        ...
        !
        read(equil_in_itm(itmax_2d)%profiles_2d(i)%grid_type(1),*) index_1
        read(equil_in_itm(itmax_2d)%profiles_2d(i)%grid_type(3),*) index_2
        ...
        elseif ( index_1 .eq. 2) then
          ...
          index_ids = 0
          do i=1,len(equil_in_itm(itmax_2d)%profiles_2d(i)%grid_type(2))-3
            if ( equil_in_itm(itmax_2d)%profiles_2d(i)%grid_type(2)(i:i+2) .eq. 'psi' ) index_ids = 10
          end do
          do i=1,len(equil_in_itm(itmax_2d)%profiles_2d(i)%grid_type(2))-11
            if ( equil_in_itm(itmax_2d)%profiles_2d(i)%grid_type(2)(i:i+11) .eq. 'rho_pol_norm' ) index_ids = 20
          end do
          do i=1,len(equil_in_itm(itmax_2d)%profiles_2d(i)%grid_type(2))-11
            if ( equil_in_itm(itmax_2d)%profiles_2d(i)%grid_type(2)(i:i+11) .eq. 'rho_tor_norm' ) index_ids = 30
          end do
        ...
        equil_out_ids%time_slice(it)%profiles_2d(i)%grid%dim1(1:ndim1) = equil_in_itm(it)%profiles_2d(i)%grid%dim1(1:ndim1)
        allocate(equil_out_ids%time_slice(it)%profiles_2d(iprof2d)%grid%dim2(ndim2))
        equil_out_ids%time_slice(it)%profiles_2d(i)%grid%dim2(1:ndim2) = equil_in_itm(it)%profiles_2d(i)%grid%dim2(1:ndim2)
        allocate(equil_out_ids%time_slice(it)%profiles_2d(iprof2d)%r(ndim1,ndim2))
        equil_out_ids%time_slice(it)%profiles_2d(i)%r(1:ndim1,1:ndim2) = equil_in_itm(it)%profiles_2d(i)%r(1:ndim1,1:ndim2)
        allocate(equil_out_ids%time_slice(it)%profiles_2d(iprof2d)%z(ndim1,ndim2))
        equil_out_ids%time_slice(it)%profiles_2d(i)%z(1:ndim1,1:ndim2) = equil_in_itm(it)%profiles_2d(i)%z(1:ndim1,1:ndim2)
        allocate(equil_out_ids%time_slice(it)%profiles_2d(iprof2d)%psi(ndim1,ndim2))
        equil_out_ids%time_slice(it)%profiles_2d(i)%psi(1:ndim1,1:ndim2) = equil_in_itm(it)%profiles_2d(i)%psi(1:ndim1,1:ndim2)
        allocate(equil_out_ids%time_slice(it)%profiles_2d(iprof2d)%theta(ndim1,ndim2))
        equil_out_ids%time_slice(it)%profiles_2d(i)%theta(1:ndim1,1:ndim2) = equil_in_itm(it)%profiles_2d(i)%theta(1:ndim1,1:ndim2)
        allocate(equil_out_ids%time_slice(it)%profiles_2d(iprof2d)%phi(ndim1,ndim2))
        equil_out_ids%time_slice(it)%profiles_2d(i)%phi(1:ndim1,1:ndim2) = equil_in_itm(it)%profiles_2d(i)%phi(1:ndim1,1:ndim2)
        allocate(equil_out_ids%time_slice(it)%profiles_2d(iprof2d)%j_tor(ndim1,ndim2))
        equil_out_ids%time_slice(it)%profiles_2d(i)%j_tor(1:ndim1,1:ndim2) = equil_in_itm(it)%profiles_2d(i)%jphi(1:ndim1,1:ndim2)
        allocate(equil_out_ids%time_slice(it)%profiles_2d(iprof2d)%j_parallel(ndim1,ndim2))
        equil_out_ids%time_slice(it)%profiles_2d(i)%j_parallel(1:ndim1,1:ndim2) = equil_in_itm(it)%profiles_2d(i)%jpar(1:ndim1,1:ndim2)
        allocate(equil_out_ids%time_slice(it)%profiles_2d(iprof2d)%b_r(ndim1,ndim2))
        equil_out_ids%time_slice(it)%profiles_2d(i)%b_r(1:ndim1,1:ndim2) = equil_in_itm(it)%profiles_2d(i)%br(1:ndim1,1:ndim2)
        allocate(equil_out_ids%time_slice(it)%profiles_2d(iprof2d)%b_z(ndim1,ndim2))
        equil_out_ids%time_slice(it)%profiles_2d(i)%b_z(1:ndim1,1:ndim2) = equil_in_itm(it)%profiles_2d(i)%bz(1:ndim1,1:ndim2)
        allocate(equil_out_ids%time_slice(it)%profiles_2d(iprof2d)%b_tor(ndim1,ndim2))
        equil_out_ids%time_slice(it)%profiles_2d(i)%b_tor(1:ndim1,1:ndim2) = equil_in_itm(it)%profiles_2d(i)%bphi(1:ndim1,1:ndim2)
      end do
    end if
    !
    ! coordinate_system:
    ...
    allocate(equil_out_ids%time_slice(it)%coordinate_system%g_11(ndim1,ndim2))
    equil_out_ids%time_slice(it)%coordinate_system%g_11(1:ndim1,1:ndim2) = equil_in_itm(it)%coord_sys%g_11(1:ndim1,1:ndim2)
    allocate(equil_out_ids%time_slice(it)%coordinate_system%g_12(ndim1,ndim2))
    equil_out_ids%time_slice(it)%coordinate_system%g_12(1:ndim1,1:ndim2) = equil_in_itm(it)%coord_sys%g_12(1:ndim1,1:ndim2)
    allocate(equil_out_ids%time_slice(it)%coordinate_system%g_13(ndim1,ndim2))
    equil_out_ids%time_slice(it)%coordinate_system%g_13(1:ndim1,1:ndim2) = equil_in_itm(it)%coord_sys%g_13(1:ndim1,1:ndim2)
    allocate(equil_out_ids%time_slice(it)%coordinate_system%g_22(ndim1,ndim2))
    equil_out_ids%time_slice(it)%coordinate_system%g_22(1:ndim1,1:ndim2) = equil_in_itm(it)%coord_sys%g_22(1:ndim1,1:ndim2)
    allocate(equil_out_ids%time_slice(it)%coordinate_system%g_23(ndim1,ndim2))
    equil_out_ids%time_slice(it)%coordinate_system%g_23(1:ndim1,1:ndim2) = equil_in_itm(it)%coord_sys%g_23(1:ndim1,1:ndim2)
    allocate(equil_out_ids%time_slice(it)%coordinate_system%g_33(ndim1,ndim2))
    equil_out_ids%time_slice(it)%coordinate_system%g_33(1:ndim1,1:ndim2) = equil_in_itm(it)%coord_sys%g_33(1:ndim1,1:ndim2)
    !
