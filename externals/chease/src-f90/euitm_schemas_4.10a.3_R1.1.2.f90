...
type type_cplx_type  !    Temporary structure for real and imaginary part of complex numbers (scalar)
  real(EUITM_R8)  :: re=-9.0E40_EUITM_R8       ! /re - Real part
  real(EUITM_R8)  :: im=-9.0E40_EUITM_R8       ! /im - Imaginary part
endtype

  
type type_veccplx_type  !    Temporary structure for real and imaginary part of complex numbers (vector)
  real(EUITM_R8),pointer  :: re(:) => null()     ! /re - Real part
  real(EUITM_R8),pointer  :: im(:) => null()     ! /im - Imaginary part
endtype
...
type type_coredelta_values  !    Description of the delta term for a given origin
  ...
  real(EUITM_R8),pointer  :: delta_tz(:,:,:) => null()     ! /delta_tz - Instant change of the impurity (multiple charge states) temperature [eV]. Time-dependent. Array3d (n
  ...
  real(EUITM_R8),pointer  :: delta_nz(:,:,:) => null()     ! /delta_nz - Instant change of the impurity (multiple charge states) density [m^-3]. Time-dependent. Array3d (nrh
...
type type_coreprofion  !    Structure for core plasma ion profile; Time-dependent
  real(EUITM_R8),pointer  :: value(:,:) => null()     ! /value - Signal value; Time-dependent; Vector (nrho,nion)
  character(len=132), dimension(:), pointer ::source => null()       ! /source - Source of the profile (any comment describing the origin of the profile : code, path to diagnostic s
endtype
...
type type_dist_src_snk_vol  !    
  real(EUITM_R8),pointer  :: particles(:) => null()     ! /particles - Source/sink particles [1/s]; Time-dependedent; Vector (npsi)
  real(EUITM_R8),pointer  :: power(:) => null()     ! /power - Power associated with the source/sink of particles [W]; Time-dependent; Vector (npsi)
  real(EUITM_R8),pointer  :: torque(:) => null()     ! /torque - Torque due to the source/sink of particles [N.m]; Time-dependent; Vector (npsi)
endtype

  
type type_dist_src_snk_surf  !    
  real(EUITM_R8),pointer  :: particlesd(:) => null()     ! /particlesd - Source/sink  particles [s^-1 m^-3]; Time-dependedent; Vector (npsi)
  real(EUITM_R8),pointer  :: powerd(:) => null()     ! /powerd - Power density associated with the source/sink of particles [W.m^-3]; Time-dependent; Vector (npsi)
  real(EUITM_R8),pointer  :: torqued(:) => null()     ! /torqued - Torque density due to the source/sink of particles [N.m^-2]; Time-dependent; Vector (npsi)
endtype

  
type type_dist_src_snk_tot  !    
  real(EUITM_R8)  :: particles=-9.0E40_EUITM_R8       ! /particles - Source/sink  particles [1/s]; Time-dependedent
  real(EUITM_R8)  :: power=-9.0E40_EUITM_R8       ! /power - Power associated with the source/sink of particles [W]; Time-dependent
  real(EUITM_R8)  :: torque=-9.0E40_EUITM_R8       ! /torque - Torque due to the source/sink of particles [N.m]; Time-dependent
endtype
...
type type_dist_wave_src  !    Auxiliary wave absorbed by the distribution species
  ...
  real(EUITM_R8)  :: wave_power=-9.0E40_EUITM_R8       ! /wave_power - Auxiliary wave power absorbed by the distribution species [W]; Time-dependent.
  real(EUITM_R8),pointer  :: wave_powerd(:) => null()     ! /wave_powerd - Auxiliary flux surface averaged wave power density absorbed by the distribution species [W/m^3]; Tim
endtype
...
type type_dist_prof_vol_nucl_reac_sf  !    Nuclear reactions of the calculated species with itself (thermal + non-thermal).
  real(EUITM_R8),pointer  :: rate(:) => null()     ! /rate - Reaction rate [1/s]; Time-dependent; Vector (npsi)
  real(EUITM_R8),pointer  :: power(:) => null()     ! /power - Fusion reaction power[W]; Time-dependent; Vector (npsi)
endtype
...
type type_dist_prof_vol_nucl_reac_th  !    Nuclear reactions between the calculated species and other species assumed to have thermal distributions.
  real(EUITM_R8),pointer  :: rate(:,:) => null()     ! /rate - Reaction rate [1/s]; Time-dependent; Matrix (nreac, npsi)
  real(EUITM_R8),pointer  :: power(:,:) => null()     ! /power - Fusion reaction power[W]; Time-dependent; Matrix (nreac, npsi)
endtype

  
type type_dist_prof_surf_nucl_reac_sf  !    Nuclear reactions of the calculated species with itself (thermal + non-thermal).
  real(EUITM_R8),pointer  :: rate(:) => null()     ! /rate - Reaction rate [s^-1.m^-3]; Time-dependent; Matrix (npsi)
  real(EUITM_R8),pointer  :: power(:) => null()     ! /power - Fusion reaction power [W.m^-3]; Time-dependent; Matrix (npsi)
endtype
...
type type_dist_prof_surf_nucl_reac_th  !    Nuclear reactions between the cacluated species and oher species assumed to have thermal distributions.
  real(EUITM_R8),pointer  :: rated(:,:) => null()     ! /rated - Reaction rate [s^-1.m^-3]; Time-dependent; Matrix (nreac, max_npsi)
  real(EUITM_R8),pointer  :: powerd(:,:) => null()     ! /powerd - Nuclear reaction power density [W.m^-3]; Time-dependent; Matrix (nreac, max_npsi)
endtype

  
type type_dist_nucl_reac_sf  !    Nuclear reactions of the calculated species with itself (thermal + non-thermal).
  real(EUITM_R8)  :: rate=-9.0E40_EUITM_R8       ! /rate - Reaction rate [1/s]; Time-dependent
  real(EUITM_R8)  :: power=-9.0E40_EUITM_R8       ! /power - Fusion reaction power[W]; Time-dependent
endtype
...
type type_dist_nucl_reac_th  !    Nuclear reactions between the calculated species and other species assumed to have thermal distributions.
  real(EUITM_R8),pointer  :: rate(:) => null()     ! /rate - Reaction rate [1/s]; Time-dependent; Vector (nreac)
  real(EUITM_R8),pointer  :: power(:) => null()     ! /power - Fusion reaction power[W]; Time-dependent; Vector (nreac)
endtype
...
type type_dist_glob  !    Global parameters (in most cases volume integrated and surface averaged quanatities).
  real(EUITM_R8)  :: n_particles=-9.0E40_EUITM_R8       ! /n_particles - Number of particles in the distribution (note: this is the number of real particles and not markers)
  real(EUITM_R8)  :: enrg=-9.0E40_EUITM_R8       ! /enrg - Energy content of the distribution [J]; Time-dependent
  real(EUITM_R8)  :: enrg_para=-9.0E40_EUITM_R8       ! /enrg_para - Parallel energy content of the distribution [J]; Time-dependent
  real(EUITM_R8),pointer  :: pow_coll_i(:) => null()     ! /pow_coll_i - Collisional power to ions [W]; Time-dependent; Matrix(nion)
  real(EUITM_R8)  :: pow_coll_e=-9.0E40_EUITM_R8       ! /pow_coll_e - Collisional power to the electrons [W]; Time-dependent
  ...
  real(EUITM_R8)  :: cur_dr_tor=-9.0E40_EUITM_R8       ! /cur_dr_tor - Toroidal current of non-thermal particles (excluding electron back current for fast ions) [A]; Time-
  real(EUITM_R8),pointer  :: trq_i(:) => null()     ! /trq_i - Collisional torque to background ions [N.m]; Time-dependent; Vector (nion)
  real(EUITM_R8)  :: trq_e=-9.0E40_EUITM_R8       ! /trq_e - Collisional torque to electrons [N.m]; Time-dependent
  real(EUITM_R8)  :: trq_j_rxb=-9.0E40_EUITM_R8       ! /trq_j_rxb - Torque due to radial currents of non-thermal particles [N.m]; Time-dependent.
...
type type_dist_profiles  !    Profiles (volume integrated and flux surface averaged)
  ...
  real(EUITM_R8),pointer  :: dens(:) => null()     ! /dens - Flux surface averaged particle density of the distribution [J/m^3]; Time-dependent; Vector (npsi)
  real(EUITM_R8),pointer  :: enrgd_tot(:) => null()     ! /enrgd_tot - Flux surface averaged energy density of the distribution [J/m^3]; Time-dependent; Vector (npsi)
  real(EUITM_R8),pointer  :: enrgd_para(:) => null()     ! /enrgd_para - Flux surface averaged parallel energy density of the distribution [J/m^3] Time-dependent; Vector (np
  real(EUITM_R8),pointer  :: powd_coll_i(:,:) => null()     ! /powd_coll_i - Flux surface averaged collisional power to ions [W.m^-3]; Time-dependent; Matrix (nion, npsi)
  real(EUITM_R8),pointer  :: powd_coll_e(:) => null()     ! /powd_coll_e - Flux surface averaged collisional power to the electrons [W.m^-3]; Time-dependent; Vector(npsi)
  ...
  real(EUITM_R8),pointer  :: curd_fp(:) => null()     ! /curd_fp - Flux surface averaged toroidal current density of non-thermal (fast) particles of the distribution s
  real(EUITM_R8),pointer  :: curd_dr(:) => null()     ! /curd_dr - Total toroidal driven current density (including electron back current in the presence of fast ions)
  real(EUITM_R8),pointer  :: trqd_i(:,:) => null()     ! /trqd_i - Flux surface averaged collisional toroidal torque to background ions [N.m^-2]; Time-dependent; Matri
  real(EUITM_R8),pointer  :: trqd_e(:) => null()     ! /trqd_e - Flux surface averaged collisional toroidal torque density to electrons [N.m^-2]; Time-dependent; Vec
  real(EUITM_R8),pointer  :: trqd_jrxb(:) => null()     ! /trqd_jrxb - Toroidal torque density due to radial currents of non-thermal particles of the distribution species 
  ...
  real(EUITM_R8),pointer  :: enrg_tot(:) => null()     ! /enrg_tot - Energy content of of a distribution species [J] inside a flux surface; Time-dependent; Vector (npsi)
  real(EUITM_R8),pointer  :: enrg_para(:) => null()     ! /enrg_para - Parallel energy content of a distribution species [J] inside a flux surface; Time-dependent; Vector 
  real(EUITM_R8),pointer  :: pow_coll_i(:,:) => null()     ! /pow_coll_i - Collisional power to ions inside a flux surface [W]; Time-dependent; Matrix(nion, npsi)
  real(EUITM_R8),pointer  :: pow_coll_e(:) => null()     ! /pow_coll_e - Collisional power to the electrons inside a flux surface [W]; Time-dependent; Vector(npsi)
  ...
  real(EUITM_R8),pointer  :: cur_fp(:) => null()     ! /cur_fp - Toroidal current of non-thermal (fast) particles driven inside a flux surface (does not include elec
  real(EUITM_R8),pointer  :: cur_dr(:) => null()     ! /cur_dr - Total toroidal current driven inside a flux surface (including electron back current in the presence
  real(EUITM_R8),pointer  :: trq_i(:,:) => null()     ! /trq_i - Collisional toroidal torque to background ions inside a flux surface [N.m]; Time-dependent; Matrix (
  real(EUITM_R8),pointer  :: trq_e(:) => null()     ! /trq_e - Collisional toroidal torque to electrons inside a flux surface [N.m]; Time-dependent; Vector (npsi)
  real(EUITM_R8),pointer  :: trq_j_rxb(:) => null()     ! /trq_j_rxb - Toroidal torque due to radial currents of non-thermal particles of the distribution species [N.m]; T
...
type type_dist_markers  !    Distribution given as a set of markers (test particles).
  real(EUITM_R8)  :: nvar=-9.0E40_EUITM_R8       ! /nvar - Number of variables associated with a marker (test particle)
  ...
  real(EUITM_R8),pointer  :: var1(:) => null()     ! /var1 - Phase space variables one characterising the markers; Time-dependent; Vector (ntpart)
  real(EUITM_R8),pointer  :: var2(:) => null()     ! /var2 - Phase space variables two characterising the markers; Time-dependent; Vector (ntpart)
  real(EUITM_R8),pointer  :: var3(:) => null()     ! /var3 - Phase space variables three characterising the markers; Time-dependent; Vector (ntpart)
  real(EUITM_R8),pointer  :: var4(:) => null()     ! /var4 - Phase space variables four characterising the markers; Time-dependent; Vector (ntpart)
  real(EUITM_R8),pointer  :: var5(:) => null()     ! /var5 - Phase space variables five characterising the markers; Time-dependent; Vector (ntpart)
  real(EUITM_R8),pointer  :: var6(:) => null()     ! /var6 - Phase space variables six characterising the markers; Time-dependent; Vector (ntpart)
  real(EUITM_R8),pointer  :: var7(:) => null()     ! /var7 - Phase space variables seven characterising the markers; Time-dependent; Vector (ntpart)
  real(EUITM_R8),pointer  :: weight(:) => null()     ! /weight - Weight of the markers; Time-dependent; Vector (ntpart)
endtype
...
type type_source_on_grid  !    Source density of particles in phase space (real space, velocity space, spin state).
  ...
  real(EUITM_R8),pointer  :: dim1(:,:,:,:,:,:) => null()     ! /dim1 - Grid in the first dimension in phase space (as specified in grid_coord). Time-dependent; Array6d (nd
  real(EUITM_R8),pointer  :: dim2(:,:,:,:,:,:) => null()     ! /dim2 - Grid in the second dimension in phase space (as specified in grid_coord). Time-dependent; Array6d (n
  real(EUITM_R8),pointer  :: dim3(:,:,:,:,:,:) => null()     ! /dim3 - Grid in the third dimension in phase space (as specified in grid_coord). Time-dependent; Array6d (nd
  real(EUITM_R8),pointer  :: dim4(:,:,:,:,:,:) => null()     ! /dim4 - Grid in the fourth dimension in phase space (as specified in grid_coord). Time-dependent; Array6d (n
  real(EUITM_R8),pointer  :: dim5(:,:,:,:,:,:) => null()     ! /dim5 - Grid in the fifth dimension in phase space (as specified in grid_coord). Time-dependent; Array6d (nd
  real(EUITM_R8),pointer  :: dim6(:,:,:,:,:,:) => null()     ! /dim6 - Grid in the sixth dimension in phase space (as specified in grid_coord). Time-dependent; Array6d (nd
  real(EUITM_R8),pointer  :: jacobian(:,:,:,:,:,:) => null()     ! /jacobian - Jacobian of the phase space grid coordinate system specified in grid_coord. Time-dependent; Array6d 
  real(EUITM_R8),pointer  :: source(:,:,:,:,:,:) => null()     ! /source - Source rate of particles in phase space. The units depend on the grid_type: [m^-3 s^-1] if the grid 
endtype
...
type type_source_mark  !    Source given as a set of markers (test particles)
  ...
  real(EUITM_R8),pointer  :: var1(:) => null()     ! /var1 - Phase space variable number one characterising the markers. Time-dependent; Vector (n_particles)
  real(EUITM_R8),pointer  :: var2(:) => null()     ! /var2 - Phase space variable number two characterising the markers. Time-dependent; Vector (n_particles)
  real(EUITM_R8),pointer  :: var3(:) => null()     ! /var3 - Phase space variable number three characterising the markers. Time-dependent; Vector (n_particles)
  real(EUITM_R8),pointer  :: var4(:) => null()     ! /var4 - Phase space variable number four characterising the markers. Time-dependent; Vector (n_particles)
  real(EUITM_R8),pointer  :: var5(:) => null()     ! /var5 - Phase space variable number five characterising the markers. Time-dependent; Vector (n_particles)
  real(EUITM_R8),pointer  :: var6(:) => null()     ! /var6 - Phase space variable number six characterising the markers. Time-dependent; Vector (n_particles)
  real(EUITM_R8),pointer  :: var7(:) => null()     ! /var7 - Phase space variable number seven characterising the markers. Time-dependent; Vector (n_particles)
  real(EUITM_R8),pointer  :: weight(:) => null()     ! /weight - Weight of the markers; Time-dependent; Vector (n_particles)
endtype
...
type type_src_snk_tot  !    
  real(EUITM_R8)  :: particles=-9.0E40_EUITM_R8       ! /particles - Source/sink particles [1/s]; Time-dependedent
  real(EUITM_R8)  :: power=-9.0E40_EUITM_R8       ! /power - Power associated with the source/sink of particles [W]; Time-dependent
  real(EUITM_R8)  :: torque=-9.0E40_EUITM_R8       ! /torque - Torque due to the source/sink of particles [Nm]; Time-dependent
endtype

  
type type_src_snk_fav  !    
  real(EUITM_R8),pointer  :: particles(:) => null()     ! /particles - Source/sink particles [s^1 m^-3]; Time-dependedent; Vector (npsi)
  real(EUITM_R8),pointer  :: power(:) => null()     ! /power - Power density associated with the source/sink of particles [W/m^3]; Time-dependent; Vector (npsi)
  real(EUITM_R8),pointer  :: torque(:) => null()     ! /torque - Torque density due to the source/sink of particles [Nm/m^3]; Time-dependent; Vector (npsi)
endtype

  
type type_src_snk_int  !    
  real(EUITM_R8),pointer  :: particles(:) => null()     ! /particles - Source/sink  particles [s^1 m^-3]; Time-dependedent; Vector (npsi)
  real(EUITM_R8),pointer  :: power(:) => null()     ! /power - Power associated with the source/sink of particles [MW/m^3]; Time-dependent; Vector(npsi)
  real(EUITM_R8),pointer  :: torque(:) => null()     ! /torque - Torque due to the source/sink of particles [Nm/m^3]; Time-dependent; Vector (npsi)
endtype
...
type type_ecesetup  !    diagnostic setup information
  ...
  type (type_setup_line) :: los  ! /los - Geometry of the line of sight.
endtype
...
type type_ecesetup_mask  !    diagnostic setup information
  ...
  type (type_setup_line_mask) :: los  ! /los - Geometry of the line of sight.
endtype
...
type type_profiles_1d  !    
  ...
  real(EUITM_R8),pointer  :: phi_flow(:) => null()     ! /profiles_1d/phi_flow - Poloidal flow function phi_flow = rho*v_pol*B_pol[kg/(V.s^2)]; Time-dependent; Vector (npsi)
  real(EUITM_R8),pointer  :: s_flow(:) => null()     ! /profiles_1d/s_flow - Definition to be provided; Time-dependent; Vector (npsi)
...
type type_mhd_vacuum  !    External modes
  real(EUITM_R8),pointer  :: m(:,:,:) => null()     ! /m - Poloidal mode number; Time-dependent; Array3D (npsi,nn,nm)
...
type type_mhd_plasma  !    MHD modes in the confined plasma
  ...
  real(EUITM_R8),pointer  :: tau_resistive(:) => null()     ! /tau_resistive - Resistive time = mu_0 rho*rho/1.22/eta_neo [s]; Source of eta_neo to be clarified. Time-dependent; V
  type (type_coord_sys) :: coord_sys  ! /coord_sys - 
  type (type_mhd_vector) :: a_pert  ! /a_pert - Pertubed vector potential (in Fourier space) [T.m]
  type (type_mhd_vector) :: b_pert  ! /b_pert - Perturbed magnetic field (in Fourier space) [T]
...
type type_mhd  !    
  ...
  real(EUITM_R8),pointer  :: frequency(:) => null()     ! /mhd/frequency - Frequency of the mode [Hz]; Time-dependent; Vector (nn)
  real(EUITM_R8),pointer  :: growthrate(:) => null()     ! /mhd/growthrate - Linear growthrate of the mode [Hz]; Time-dependent; Vector (nn)
...
type type_divergence  !    
  real(EUITM_R8),pointer  :: frac_divcomp(:) => null()     ! /nbi/nbi_unit(i)/setup_inject/divergence/frac_divcomp - Fraction of injected particles. Vector(ndiv_comp)
  real(EUITM_R8),pointer  :: div_vert(:) => null()     ! /nbi/nbi_unit(i)/setup_inject/divergence/div_vert - Beam divergence for a unit in the vertical direction[rad]. Vector(ndiv_comp)
  real(EUITM_R8),pointer  :: div_horiz(:) => null()     ! /nbi/nbi_unit(i)/setup_inject/divergence/div_horiz - Beam divergence for a unit in the horizontal direction[rad]. Vector(ndiv_comp)
endtype

  
type type_beamlets  !    
  type (type_rzphi1D) :: position  ! /nbi/nbi_unit(i)/setup_inject/beamlets/position - Position of beamlets. Vector rzphi1D (nbeamlets)
  real(EUITM_R8),pointer  :: tang_rad_blt(:) => null()     ! /nbi/nbi_unit(i)/setup_inject/beamlets/tang_rad_blt - Tangency radius (major radius where the central line of a beamlet is tangent to a circle around the 
  real(EUITM_R8),pointer  :: angle_blt(:) => null()     ! /nbi/nbi_unit(i)/setup_inject/beamlets/angle_blt - Angle of inclination between a line at the centre of a beamlet and the horiontal plane [rad]; Vector
  real(EUITM_R8),pointer  :: pow_frc_blt(:) => null()     ! /nbi/nbi_unit(i)/setup_inject/beamlets/pow_frc_blt - Fraction of power of a unit injected by a beamlet; Vector(nbeamlets)
endtype
...
type type_setup_inject  !    
  ...
  real(EUITM_R8)  :: tang_rad=-9.0E40_EUITM_R8       ! /nbi/nbi_unit(i)/setup_inject/tang_rad - Tagency radius (major radius where the central line of a NBI unit is tangent to a circle around the 
  real(EUITM_R8)  :: angle=-9.0E40_EUITM_R8       ! /nbi/nbi_unit(i)/setup_inject/angle - Angle of inclination between a line at the centre of the injection unit surface and the horiontal pl
  ...
  real(EUITM_R8)  :: focal_len_hz=-9.0E40_EUITM_R8       ! /nbi/nbi_unit(i)/setup_inject/focal_len_hz - Horizontal focal length along the beam line [m]
  real(EUITM_R8)  :: focal_len_vc=-9.0E40_EUITM_R8       ! /nbi/nbi_unit(i)/setup_inject/focal_len_vc - Vertical focal length along the beam line [m]
...
type type_neoclassic  !    
  ...
  real(EUITM_R8),pointer  :: vpol(:,:) => null()     ! /neoclassic/vpol - Neoclassical poloidal rotation of for each ion species [m/s]. Time-dependent. Matrix(nrho,nion).
...
type type_shape_sph  !    
  real(EUITM_R8)  :: radius=-9.0E40_EUITM_R8       ! /pellets/shape/shape_sph/radius - Pellet radius (m)
endtype

  
type type_shape_cyl  !    
  real(EUITM_R8)  :: radius=-9.0E40_EUITM_R8       ! /pellets/shape/shape_cyl/radius - Pellet radius (m)
  real(EUITM_R8)  :: height=-9.0E40_EUITM_R8       ! /pellets/shape/shape_cyl/height - Pellet height (m)
endtype
...
type type_species  !    
  real(EUITM_R8),pointer  :: amn(:) => null()     ! /pellets/species/amn - Atomic mass number (lumped species are allowed); Vector (nspecies)
  real(EUITM_R8),pointer  :: zn(:) => null()     ! /pellets/species/zn - Nuclear charge (lumped species are allowed); Vector (nspecies)
  real(EUITM_R8),pointer  :: concentr(:) => null()     ! /pellets/species/concentr - Concentration of species on pellet ranging from 0 to 1; Vector (nspecies)
  real(EUITM_R8),pointer  :: subl_energy(:) => null()     ! /pellets/species/subl_energy - Sublimation energy per atom of species on pellet in eV; Vector (nspecies)
endtype
...
type type_pelletpath  !    
  ...
  real(EUITM_R8)  :: horchordang=-9.0E40_EUITM_R8       ! /pellets/pelletpath/horchordang - Angle [rad] of horizontal projection of pellet path line with poloidal cross section (0 for HFS to L
  real(EUITM_R8)  :: verchordang=-9.0E40_EUITM_R8       ! /pellets/pelletpath/verchordang - Angle of pellet path with vertical axis (0 for bottom-top trajectory, Pi for top-bottom trajectory -
...
type type_ablationrate  !    
  real(EUITM_R8),pointer  :: rho_tor(:) => null()     ! /pellets/ablationrate/rho_tor - Toroidal flux coordinate [m], to be used by the ETS and in many CPOs (coreprof, ...). Defined as sqr
  real(EUITM_R8),pointer  :: rate(:) => null()     ! /pellets/ablationrate/rate - Calculated ablation rate; (particle/s)
  type (type_rzphi1D) :: position  ! /pellets/ablationrate/position - Coordinates for abblation rate
endtype

  
type type_deposprofile  !    
  real(EUITM_R8),pointer  :: rho_tor(:) => null()     ! /pellets/deposprofile/rho_tor - Toroidal flux coordinate [m], to be used by the ETS and in many CPOs (coreprof, ...). Defined as sqr
  real(EUITM_R8),pointer  :: density(:) => null()     ! /pellets/deposprofile/density - Density increase (deposition profile); (m^-3)
  type (type_rzphi1D) :: position  ! /pellets/deposprofile/position - Coordinates for abblation rate
  real(EUITM_R8)  :: delay=-9.0E40_EUITM_R8       ! /pellets/deposprofile/delay - Time delay between the deposition profile and the ablation profile; Scalar; Time-dependent (s)
endtype
...
type type_pellets  !    
  ...
  real(EUITM_R8)  :: velocity=-9.0E40_EUITM_R8       ! /pellets/velocity - Pellet injection velocity (m/s). Time-dependent. Scalar
...
type type_ref_nt_0dr_ref  !    a non-timed reference of real type
  real(EUITM_R8)  :: value=-9.0E40_EUITM_R8       ! /value - Value of the reference. Real scalar.
...
type type_ref_nt_1dr_ref  !    a non-timed reference of vecflt type
  real(EUITM_R8),pointer  :: value(:) => null()     ! /value - Value of the reference. Vector.
  character(len=132), dimension(:), pointer ::description => null()       ! /description - Description of the reference. String.
endtype
...
type type_ref_t_0dr_ref  !    a timed reference of real type
  real(EUITM_R8)  :: value=-9.0E40_EUITM_R8       ! /value - Value of the reference. Real scalar. Time-dependent.
  character(len=132), dimension(:), pointer ::description => null()       ! /description - Description of the reference. String.
endtype
...
type type_ref_t_1dr_ref  !    a timed reference of vecflt type
  real(EUITM_R8),pointer  :: value(:) => null()     ! /value - Value of the reference. Vector. Time-dependent.
  character(len=132), dimension(:), pointer ::description => null()       ! /description - Description of the reference. String.
endtype
...
type type_reference  !    
  ...
  real(EUITM_R8)  :: time=-9.0E40_EUITM_R8       ! /reference/time - Time [s]; Time-dependent; Scalar
endtype
...
type type_t_series_real  !    Time series
...
type type_reflectometry_radifield_efield  !    complex electric field at the aperture, given as a 2d grid in the local x and y directions (corresponding to dim1 and dim2)
  type (type_reggrid) :: grid2d  ! /grid2d - Coordinate values for the grid for the electric field arrays. Vector(ndim1) and Vector(ndim2)
  type (type_matcplx_type) :: e1  ! /e1 - Electric field component along local x direction [V/m]. Matrix(ndim1,ndim2)
  type (type_matcplx_type) :: e2  ! /e2 - Electric field component along local y direction [V/m]. Matrix(ndim1,ndim2)
...
type type_reflectometry_antennas  !    Vector of reflectometry antenna descriptions. These include radiation fields as well as material antenna structures (feeds, horns,
  ...
  real(EUITM_R8)  :: origin=-9.0E40_EUITM_R8       ! /origin - To be defined: annotation and type
  ...
  real(EUITM_R8)  :: launchsignal=-9.0E40_EUITM_R8       ! /launchsignal - To be defined: annotation and type
endtype
  
type type_t_series_real_mask  !    Time series
...
type type_reflectometry_radifield_efield_mask  !    complex electric field at the aperture, given as a 2d grid in the local x and y directions (corresponding to dim1 and dim2)
  type (type_reggrid_mask) :: grid2d  ! /grid2d - Coordinate values for the grid for the electric field arrays. Vector(ndim1) and Vector(ndim2)
  type (type_matcplx_type_mask) :: e1  ! /e1 - Electric field component along local x direction [V/m]. Matrix(ndim1,ndim2)
  type (type_matcplx_type_mask) :: e2  ! /e2 - Electric field component along local y direction [V/m]. Matrix(ndim1,ndim2)
...
type type_reflectometry_antennas_mask  !    Vector of reflectometry antenna descriptions. These include radiation fields as well as material antenna structures (feeds, horns,
  ...
  integer  :: origin=0       ! /origin - To be defined: annotation and type
  ...
  integer  :: launchsignal=0       ! /launchsignal - To be defined: annotation and type
endtype
...
type type_refl_receive  !    
  ...
  type (type_t_series_real) :: io_signal  ! /reflectomet/refl_receive(i)/io_signal - Local oscillator signal, for mixing with raw signal; Time series; Vector (ntime_raw)
  ...
  type (type_codeparam) :: codeparam  ! /reflectomet/refl_receive(i)/codeparam - Code parameters of ERC3D used for the production of this signal
endtype
...
type type_refl_receive_mask  !    
  ...
  type (type_t_series_real_mask) :: io_signal  ! /reflectomet/refl_receive(i)/io_signal - Local oscillator signal, for mixing with raw signal; Time series; Vector (ntime_raw)
  ...
  type (type_codeparam_mask) :: codeparam  ! /reflectomet/refl_receive(i)/codeparam - Code parameters of ERC3D used for the production of this signal
endtype
...
type type_scenario_heat_power  !    Power delivred to plasma (thermal an non thermal)
  ...
  type (type_scenario_ref) :: padd_tot  ! /padd_tot - total power input to the plasma, including ohmic power and fusion power [W]. Time-dependent.
...
type type_scenario_heat_power_mask  !    Power delivred to plasma (thermal an non thermal)
  ...
  type (type_scenario_ref_mask) :: padd_tot  ! /padd_tot - total power input to the plasma, including ohmic power and fusion power [W]. Time-dependent.
...
type type_summary  !    
  ...
  real(EUITM_R8)  :: time=-9.0E40_EUITM_R8       ! /summary/time - Time at which the 0D variables of the summary are taken [s]. Scalar
endtype
...
type type_element_desc  !    
  ...
  real(EUITM_R8)  :: amn=-9.0E40_EUITM_R8       ! /amn - Nuclear mass
  real(EUITM_R8)  :: density=-9.0E40_EUITM_R8       ! /density - Material density (atoms/m^3)
endtype
...
type type_compound_desc  !    
  ...
  real(EUITM_R8),pointer  :: stochiometry(:) => null()     ! /stochiometry - Composition of the compound. Float vector, dimensions: 1. element number (numbering as in surface.el
...
type type_ref_wall_typ  !    
  ...
  real(EUITM_R8),pointer  :: thickness(:) => null()     ! /wall/surface/ref_wall_typ(i)/thickness - Thickness(m). Float vector, dimensions: 1. layer index
  real(EUITM_R8),pointer  :: stochiometry(:,:) => null()     ! /wall/surface/ref_wall_typ(i)/stochiometry - Material composition of layer. Float matrix, dimensions: 1. layer index, 2. element number (numberin
  real(EUITM_R8),pointer  :: dx(:,:) => null()     ! /wall/surface/ref_wall_typ(i)/dx - Cell spacings for 1d layer height discretization; Float matrix (max. number of cells for layer, laye
endtype

  
type type_layers  !    
  real(EUITM_R8),pointer  :: density(:,:) => null()     ! /wall/surface/layers/density - Density of the surface layers [kg/m^3]; Time-dependent; Float matrix (max. number of layers, number 
  real(EUITM_R8),pointer  :: thickness(:,:) => null()     ! /wall/surface/layers/thickness - Thickness of surface layer [m]; Time-dependent; Float matrix (max. number of layers, number of eleme
  real(EUITM_R8),pointer  :: roughness(:,:) => null()     ! /wall/surface/layers/roughness - Surface roughness [m] (surface between this layer and the one above it towards the plasma); Time-dep
  real(EUITM_R8),pointer  :: t(:,:,:) => null()     ! /wall/surface/layers/t - Temperature in layer [K]; Time-dependent; 3d float array, dimensions: 1. cell index of 1d layer heig
  real(EUITM_R8),pointer  :: element_frac(:,:,:) => null()     ! /wall/surface/layers/element_frac - Elemental composition; Time-dependent; Float 3d array (max. number of tracked elements, max. number 
  real(EUITM_R8),pointer  :: chem_comp(:,:,:) => null()     ! /wall/surface/layers/chem_comp - Chemical composition, referring to the list surface.compounds; Time-dependent; 3d float array, dimen
endtype

  
type type_h_inventory  !    
  real(EUITM_R8),pointer  :: surf_trap_de(:,:,:,:,:) => null()     ! /wall/surface/h_inventory/surf_trap_de - Density of hydrogen traps on internal surfaces [1/m^2]; Time-dependent; 5d float array; Dimensions: 
  real(EUITM_R8),pointer  :: bulk_trap_de(:,:,:,:,:) => null()     ! /wall/surface/h_inventory/bulk_trap_de - Density of hydrogen traps in bulk material [1/m^3]; Time-dependent; 5d float array; Dimensions: see 
  real(EUITM_R8),pointer  :: bulk_D(:,:,:,:,:) => null()     ! /wall/surface/h_inventory/bulk_D - Diffusivity of hydrogen species in bulks of different compounds; Time-dependent; 5d float array. Dim
  real(EUITM_R8),pointer  :: surface_D(:,:,:,:,:) => null()     ! /wall/surface/h_inventory/surface_D - Diffusivity of hydrogen species of surface of different compounds; Time-dependent; Dimensions: see b
  real(EUITM_R8),pointer  :: bulk_C_s(:,:,:,:,:) => null()     ! /wall/surface/h_inventory/bulk_C_s - Bulk mobile (solute) concentration [atoms/m^3]; Time-dependent; Dimensions: see bulk_D
  real(EUITM_R8),pointer  :: surface_C_s(:,:,:,:,:) => null()     ! /wall/surface/h_inventory/surface_C_s - Surface mobile (solute) concentration [atoms/m^2]; Time-dependent; Dimensions: see bulk_D
  real(EUITM_R8),pointer  :: bulk_C_t(:,:,:,:,:) => null()     ! /wall/surface/h_inventory/bulk_C_t - Bulk trapped concentration [atoms/m^3]; Time-dependent; Dimensions: see bulk_D
  real(EUITM_R8),pointer  :: surface_C_t(:,:,:,:,:) => null()     ! /wall/surface/h_inventory/surface_C_t - Surface trapped concentration [atoms/m^2]; Time-dependent; Dimensions: see bulk_D
  real(EUITM_R8),pointer  :: surf_recrate(:,:,:,:,:) => null()     ! /wall/surface/h_inventory/surf_recrate - Recombination rate on surface (only for pure elements, not compounds) [molecules*m^2/s]; Time-depend
endtype
...
type type_waves_global_param  !    Global wave deposition parameters
  ...
  real(EUITM_R8),pointer  :: pow_i(:) => null()     ! /pow_i - Wave power absorbed by an ion species [W]; Time-dependent; Vector (nion)
  real(EUITM_R8)  :: pow_e=-9.0E40_EUITM_R8       ! /pow_e - Wave power absorbed by the electrons [W]; Time-dependent; Float
  real(EUITM_R8),pointer  :: pow_ntor_i(:,:) => null()     ! /pow_ntor_i - Wave power absorbed by an ion species per toroidal mode number [W]; Time-dependent; Matrix (ntor,nio
  real(EUITM_R8),pointer  :: pow_ntor_e(:) => null()     ! /pow_ntor_e - Wave power absorbed by the electrons per toroidal mode number [W]; Time-dependent; Vector (ntor)
...
type type_waves_grid_1d  !    Grid points for profiles
  real(EUITM_R8),pointer  :: rho_tor_norm(:) => null()     ! /rho_tor_norm - Normalised toroidal flux coordinate at the grid points for 1D profiles; Time-dependent; Vector (npsi
  real(EUITM_R8),pointer  :: rho_tor(:) => null()     ! /rho_tor - Toroidal flux coordinate at the grid points for 1D profiles [m]; Time-dependent; Vector (npsi)
...
type type_waves_grid_2d  !    Grid points for 2D profiles
  ...
  real(EUITM_R8),pointer  :: rho_tor_norm(:,:) => null()     ! /rho_tor_norm - Normalised toroidal flux coordinate at the grid points for the 2D profiles; Time-dependent; Matrix (
  real(EUITM_R8),pointer  :: rho_tor(:,:) => null()     ! /rho_tor - Toroidal flux coordinate at the grid points for the 2D profiles [m]; Time-dependent; Matrix (ndim1, 
  real(EUITM_R8),pointer  :: psi(:,:) => null()     ! /psi - Grid points in poloidal flux function [Wb], without 1/2pi and such that Bp=|grad psi| /R/2/pi. Time-
...
type type_waves_profiles_1d  !    waves 1D radial profiles
  ...
  real(EUITM_R8),pointer  :: powd_e(:) => null()     ! /powd_e - Flux surface averaged absorbed wave power density on electrons [W/m^3]; Time-dependent; Vector (npsi
  real(EUITM_R8),pointer  :: powd_i(:,:) => null()     ! /powd_i - Flux surface averaged absorbed wave power density on ion species [W/m^3]; Time-dependent; Matrix (np
  ...
  real(EUITM_R8),pointer  :: powd_ntor_e(:,:) => null()     ! /powd_ntor_e - Flux surface averaged absorbed power density for each toroidal mode number on electrons [W/m^3]; Tim
  real(EUITM_R8),pointer  :: powd_ntor_i(:,:,:) => null()     ! /powd_ntor_i - Flux surface averaged power density for each toroidal mode number on each ions species [W/m^3]; Time
  ...
  real(EUITM_R8),pointer  :: pow_tot(:) => null()     ! /pow_tot - Volume integrated absorbed wave power density [W]; Time-dependent; Vector (npsi)
  real(EUITM_R8),pointer  :: pow_e(:) => null()     ! /pow_e - Volume integrated absorbed wave power density on electrons [W]; Time-dependent; Vector (npsi)
  real(EUITM_R8),pointer  :: pow_i(:,:) => null()     ! /pow_i - Volume integrated absorbed wave power density on ion species [W]; Time-dependent; Matrix (npsi, nion
  real(EUITM_R8),pointer  :: pow_ntor(:,:,:) => null()     ! /pow_ntor - Volume integrated power density for each toroidal mode number [W]; Time-dependent; Matrix (npsi, nto
  real(EUITM_R8),pointer  :: pow_ntor_e(:,:) => null()     ! /pow_ntor_e - Volume integrated power density for each toroidal mode number on the electrons  [W]; Time-dependent;
  real(EUITM_R8),pointer  :: pow_ntor_i(:,:,:) => null()     ! /pow_ntor_i - Volume integrated power density for each toroidal mode number on each ions species [W]; Time-depende
  real(EUITM_R8),pointer  :: curd_par(:) => null()     ! /curd_par - Flux surface averaged wave driven parallel current density = average(j.B) / B0, where B0 is in  glob
  ...
  real(EUITM_R8),pointer  :: cur_tor(:) => null()     ! /cur_tor - Wave driven toroidal current inside a flux surface from stand alone calculation (not consistent with
  real(EUITM_R8),pointer  :: cur_tor_ntor(:,:) => null()     ! /cur_tor_ntor - Wave driven toroidal current inside a flux surface for each toroidal mode number from a stand alone 
endtype
...
type type_waves_profiles_2d  !    waves 2D profiles in poloidal cross-section
  ...
  real(EUITM_R8),pointer  :: powd_e(:,:) => null()     ! /powd_e - Absorbed wave power density on electrons [W/m^3]; Time-dependent; Matrix (ndim1, ndim2)
  real(EUITM_R8),pointer  :: powd_i(:,:,:) => null()     ! /powd_i - Absorbed wave power density on ion species [W/m^3]; Time-dependent; Array3D (ndim1, ndim2, nion)
  ...
  real(EUITM_R8),pointer  :: powd_ntor_e(:,:,:) => null()     ! /powd_ntor_e - Absorbed power density for each toroidal mode number on electrons [W/m^3]; Time-dependent; Array 3D 
  real(EUITM_R8),pointer  :: powd_ntor_i(:,:,:,:) => null()     ! /powd_ntor_i - Absorbed power density for each toroidal mode number on each ions species [W/m^3]; Time-dependent; A
...
type type_pol_decomp  !    
  ...
  real(EUITM_R8),pointer  :: b_norm_ph(:,:,:) => null()     ! /waves/coherentwave(i)/fullwave/pol_decomp/b_norm_ph - Phase of poloidal Fourier decomposition of parallel wave electric field [rad]; Time-dependent; Array
