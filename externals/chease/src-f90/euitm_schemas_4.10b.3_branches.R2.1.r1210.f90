...
type type_amns_constituentType  !    Contains all of the information to characterize an AMNS constituent.
  ...
  integer  :: multiplicity=-999999999       ! /multiplicity - Multiplicity in the reaction
endtype
...
type type_reacprodType  !    Characterizes a reactant or product in an AMNS reaction.
  ...
  integer  :: relative=-999999999       ! /relative - This is a flag indicating that charges are absolute (if set to 0), or relative (if 1); relative woul
...
type type_amns_processType  !    Contains all of the information to characterize an AMNS process; Vector(nprocs).
  ...
  character(len=132), dimension(:), pointer ::sup_string => null()       ! /sup_string - String to be used if supplementary information is required.
  real(EUITM_R8)  :: sup_real=-9.0E40_EUITM_R8       ! /sup_real - Real to be used if supplementary information is required.
  integer  :: sup_int=-999999999       ! /sup_int - Int to be used if supplementary information is required.
...
type type_reacprodType_mask  !    Characterizes a reactant or product in an AMNS reaction.
  ...
  integer  :: relative=0       ! /relative - This is a flag indicating that charges are absolute (if set to 0), or relative (if 1); relative woul
...
type type_amns_processType_mask  !    Contains all of the information to characterize an AMNS process; Vector(nprocs).
  ...
  integer  :: sup_string=0       ! /sup_string - String to be used if supplementary information is required.
  integer  :: sup_real=0       ! /sup_real - Real to be used if supplementary information is required.
  integer  :: sup_int=0       ! /sup_int - Int to be used if supplementary information is required.
...
type type_dist_state_0d  !    Algebraic moments of the distribution function integrated over the plasma volume, e.g. total number of particles, energy etc. Time
  ...
  real(EUITM_R8),pointer  :: momentm_fast(:) => null()     ! /momentm_fast - Kinetic toroidal angular momentum of the fast ions [Nms]; Time-dependent; Vector (npsi)
...
type type_plasma  !    
  integer,pointer  :: species_index(:,:) => null()     ! /wall/wall1d/plasma/species_index - Index of species into wall/compositions; matrix(nspecies,3); 1st element indicates {1: main ions; 2:
  real(EUITM_R8),pointer  :: flux(:) => null()     ! /wall/wall1d/plasma/flux - flux of species indicated by species_index; array of nspecies; positive implies incoming onto wall; 
  real(EUITM_R8),pointer  :: energy(:) => null()     ! /wall/wall1d/plasma/energy - energy flux of species indicated by species_index; array of nspecies; positive implies incoming onto
endtype
  
type type_plasma_mask  !    
  integer  :: species_index=0       ! /wall/wall1d/plasma/species_index - Index of species into wall/compositions; matrix(nspecies,3); 1st element indicates {1: main ions; 2:
  integer  :: flux=0       ! /wall/wall1d/plasma/flux - flux of species indicated by species_index; array of nspecies; positive implies incoming onto wall; 
  integer  :: energy=0       ! /wall/wall1d/plasma/energy - energy flux of species indicated by species_index; array of nspecies; positive implies incoming onto
endtype
...
type type_wall1d  !    
  real(EUITM_R8),pointer  :: pumping_speed(:) => null()     ! /wall/wall1d/pumping_speed - pumping speed; array of length nneut; [particles/s]
  real(EUITM_R8),pointer  :: gas_puff(:) => null()     ! /wall/wall1d/gas_puff - gas puff; array of length nneut; [particles/s]
  real(EUITM_R8),pointer  :: wall_inventory(:) => null()     ! /wall/wall1d/wall_inventory - wall inventory; array of length nneut; [particles]
  real(EUITM_R8),pointer  :: recycling_coefficient(:) => null()     ! /wall/wall1d/recycling_coefficient - 
  real(EUITM_R8)  :: wall_temperature=-9.0E40_EUITM_R8       ! /wall/wall1d/wall_temperature - 
  real(EUITM_R8)  :: power_from_plasma=-9.0E40_EUITM_R8       ! /wall/wall1d/power_from_plasma - 
  real(EUITM_R8)  :: power_to_cooling=-9.0E40_EUITM_R8       ! /wall/wall1d/power_to_cooling - 
  type (type_plasma) :: plasma  ! /wall/wall1d/plasma - 
endtype
...
type type_wall1d_mask  !    
  integer  :: pumping_speed=0       ! /wall/wall1d/pumping_speed - pumping speed; array of length nneut; [particles/s]
  integer  :: gas_puff=0       ! /wall/wall1d/gas_puff - gas puff; array of length nneut; [particles/s]
  integer  :: wall_inventory=0       ! /wall/wall1d/wall_inventory - wall inventory; array of length nneut; [particles]
  integer  :: recycling_coefficient=0       ! /wall/wall1d/recycling_coefficient - 
  integer  :: wall_temperature=0       ! /wall/wall1d/wall_temperature - 
  integer  :: power_from_plasma=0       ! /wall/wall1d/power_from_plasma - 
  integer  :: power_to_cooling=0       ! /wall/wall1d/power_to_cooling - 
  type (type_plasma_mask) :: plasma  ! /wall/wall1d/plasma - 
endtype
...
type type_wall  !    
  ...
  type (type_wall1d) :: wall1d  ! /wall/wall1d - 
...
type type_wall_mask  !    
  ...
  type (type_wall1d_mask) :: wall1d  ! /wall/wall1d - 
