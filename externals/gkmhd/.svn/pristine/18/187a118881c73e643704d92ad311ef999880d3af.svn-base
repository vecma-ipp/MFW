...
!...  color temperature version, works with IDL only
  ...
  INTEGER :: iu,i,j,i0,j0,j1,j2,mu,ipl,iframe,istr,ieof
...
!...  find the extrema
     ...
     IF (ireads == 1) THEN
        ...
        DO mu=1,npics
           uumax(mu)=MAX(uumax(mu),MAXVAL(ABS(cxa(:,:,mu))))
        END DO
...
!...  put out normalised vars
     ...
     IF (ireads == 2) THEN
        ...
        WRITE (13,100) nx0,ny0
        WRITE (13,220) (xp(i),i=1,nx0)
        WRITE (13,220) (yp(j),j=1,ny0)
        DO mu=1,npics
           cxa(:,:,mu)=uumax(mu)*(cxa(:,:,mu)-uumin(mu))

           WRITE (13,200) mu
           WRITE (13,210) FLOOR(cxa(:,:,mu))
!           WRITE (13,220) cxa(:,:,mu)
