...
!...  poloidal plane 2D ctrs of two vars
...
!...  initialise plotting
  ...
  IF (iptype == 2) THEN
     WRITE (13,112) xx0,xx9,yy0,yy9,aspect,2
  END IF
...
!...  draw contours in x-y plane
  ...
  DO mu=1,2
     IF (mu == 1) nu=ivcor1
     IF (mu == 2) nu=ivcor2
     CALL Ctrs(mu,cxa(1,1,nu+2),nx0,ny0,xx0,xx9,yy0,yy9, &
          cxa,lvars(nu),lvars1(nu),2)
     IF (iptype == 2) THEN
        j0=SIZE(fsbndyr)
