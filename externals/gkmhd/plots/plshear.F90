...
!...  poloidal plane 2D ctrs of magnetic shear scalar
!...  draws ctrs in each of 3 main quadrangles
  ...
  INTEGER :: m0,mm,mp
  REAL :: uur,uuz,uurr,uuzz,uurz,uuzr,div,sigma,shear,thxm

  CHARACTER*32 :: lbl

  INCLUDE 'pages.h'

  INTEGER, PARAMETER :: npics=1
...
!...  evaluate shear -- derivatives of world line field put in 1,2,3,4
     ...
     DO i0=1,nx0
        ...
        DO j0=1,fsfc(i)%ny0
           ...
           DO m0=1,6
              ...
              IF (m0 == 1) mm=6
              puu(1)=puu(1) - thxm*( &
                   (duu(mp,5)-duu(mm,5))*duu(m0,nvars-3) )
              puu(2)=puu(2) + thxm*( &
                   (duu(mp,6)-duu(mm,6))*duu(m0,nvars-4) )
              puu(3)=puu(3) - thxm*( &
                   (duu(mp,6)-duu(mm,6))*duu(m0,nvars-3) )
              puu(4)=puu(4) + thxm*( &
                   (duu(mp,5)-duu(mm,5))*duu(m0,nvars-4) )
           END DO
           fsfc(i)%fuu(1:4,j)=puu(1:4)
        END DO
     END DO
...
!           INCLUDE 'vardefp.h90'
           ...
           uuzr=fsfc(i)%fuu(3,j)
           uurz=fsfc(i)%fuu(4,j)
           uur=fsfc(i)%fuu(5,j)/rtor

           div=(uurr+uuzz+uur)/3.
!           IF (div /= div) Write (0,*) 'NaN in div i0 j0 = ',i0,j0

           sigma=(uurr-div)**2.+(uuzz-div)**2.+(uur-div)**2. &
                +0.5*((uuzr+uurz)**2.)
!           IF (sigma /= sigma) Write (0,*) 'NaN in sigma i0 j0 = ',i0,j0

           shear=sqrt(2*sigma)
           IF (i0 == 1 .OR. i0 == nx0) shear=0.
...
!...  draw contours in x-y plane
  ...
  mu=1
     CALL Ctrs(cxa(1,1,mu+2),nx0,ny0,xx0,xx9,yy0,yy9, &
          cxa,lvars(mu),lvars1(mu))
     IF (iptype == 2) THEN
        j0=SIZE(fsbndyr)
...
SUBROUTINE Ctrs(cx,nx,ny,x1,x2,y1,y2,rth,lvar,lvar1)

!...  plots polar in x-y plane

  IMPLICIT NONE

  INTEGER :: nx,ny,isetinterval=1
  REAL :: x1,x2,y1,y2
  REAL :: cx(nx,ny),rth(nx,ny,2)

  INTEGER :: iptype,ix1,ix2,jy1,jy2,i
  REAL :: zc(13),ztr(6),dh
...
!...  draw and label the map
  ...
  IF (iptype == 1) THEN
     ...
     DO i=1,10
        CALL Pgsci(i)
        zc(1)=dh*i
        CALL Pgconx(cx,nx,ny,1,nx,1,nx,zc,1,pgpolar)
        CALL Pgconx(cx,nx,ny,1,nx,nx+1,2*nx,zc,1,pgpolar)
        CALL Pgconx(cx,nx,ny,1,nx,2*nx+1,3*nx,zc,1,pgpolar)
     END DO
     WRITE (6,110) lvar,dh
  END IF
#endif
  ...
  DO i=1+ngdx,nx0+ngdx
     ...
     DO j=j1,j2
        ...
        DO m0=1,6
           ...
           fsfc(i)%aratio(m0,j)=fsfc(i)%aratio(m0,j)+wwa/2.
           fsfc(i)%aratio(mp,j)=fsfc(i)%aratio(mp,j)+wwb/2.
...
!...  dont divide by 2 on area2 which is used in Arakawa (same factor there)
        fsfc(i)%area2(j)=area2
