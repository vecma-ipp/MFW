...
! +++ Set up coefficients for matrix solver:
  ...
  DO IDIM1 = 1,NDIM
     UU(1,IDIM1,IDIM1)            = - V(IDIM1,1)/H(1)+U(IDIM1,1)
     VV(1,IDIM1,IDIM1)            =   V(IDIM1,1)/H(1)
     UU(2,IDIM1,IDIM1)            =   V(IDIM1,2)/H(NRHO-1)+U(IDIM1,2)
     VV(2,IDIM1,IDIM1)            = - V(IDIM1,2)/H(NRHO-1)      
  ENDDO

  DOLOOP1IRHO1: DO IRHO1 = 2,NRHO-1
     IRHOP                        = IRHO1+1
     IRHOM                        = IRHO1-1
...
! +++ Call the matrix progonka:
  CALL MPROGONKA(NRHO,NDIM,AA,BB,CC,FF,UU,VV,WW,YY)
...
! +++ New solution: 
  ...
  CALL DERIVN3(NRHO,NDIM,RHO,Y,DY)
...
!     BOUNDARY CONDITION 
     DO  I=1,NDIM
        ...
        ENDDO
     ENDDO
     CALL INVMATRIX(NDIM, TEMP1, TEMP2)      
