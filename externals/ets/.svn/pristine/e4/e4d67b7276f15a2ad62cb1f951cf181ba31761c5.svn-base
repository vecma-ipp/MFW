...
'''
Standard version of the input files except for a more peaked source profile (written to 4/102).
'''
...
def write_cpo(shot,run):
    ...
    tbc=numpy.zeros(1, dtype=numpy.int)
    ...
    shape_factor=(RHO[-1]-RHO)**2
    
    cpo.coresourceArray.array[0].j                           = s+0.0                     #j_ni    [A/m^2]
    cpo.coresourceArray.array[0].sigma                       = s+0.0                     #sigma   [1/(Ohm*m)]
    cpo.coresourceArray.array[0].qe.exp                      = s+5.0E4*shape_factor      #Qe_exp  [W/m^3]
    cpo.coresourceArray.array[0].qe.imp                      = s+0.0                     #Qe_imp  [1/m^3/s]

    shape_factor=numpy.tile(numpy.reshape(shape_factor,[NRHO,1]),[1,NION])
    
    cpo.coresourceArray.array[0].si.exp                      = si+5.0E19*shape_factor    #Si_exp  [1/m^3/s]
    cpo.coresourceArray.array[0].si.imp                      = si+0.0                    #Si_imp  [1/s]
    cpo.coresourceArray.array[0].qi.exp                      = si+5.0E4*shape_factor     #Qi_exp  [W/m^3]
