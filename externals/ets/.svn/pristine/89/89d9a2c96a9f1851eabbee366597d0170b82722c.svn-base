...
!> \author Bruce Scott
!>
!> \version "$Id: bdseq.F90 23 2011-04-20 17:48:11Z bscott $"
SUBROUTINE Bdseq (eq_in, eq_out, code_parameters)

!...  simple 1D equilibrium module, actual calculation
...
!...       Phys_Constants which calls ITM_Types
  ...
  REAL(R8) :: hra,heta,rho,dprime,det,coseta,sineta
...
!...  idiot protection
  ...
  IF (neta_eq == 0) then
     if(associated(eq_in(1)%coord_sys%position%R)) then
        neta_eq = SIZE (eq_in(1)%coord_sys%position%R, DIM=2)
        write(*,*) 'neta_eq set on the basis of eq_in%coord_sys%position%R to ', neta_eq
     endif
  endif
...
!...  find the toroidal magnetic potential (little psi)

  psi=jtor*r00*mu_0*(a00*a00)

  CALL Trid(psi,nr_eq)

!...  find the pitch

  qq(1)=2.*(psi(2)-psi(1))/(hra*hra)
  DO i=2,nr_eq-1
     qq(i)=(psi(i+1)-psi(i-1))/(2.*hra*ra(i))
  END DO
  i=nr_eq
  qq(i)=2.*qq(i-1)-qq(i-2)
  qq=b00/qq
...
!...  gm2 still contains the derivatives of the shifts
  ...
  DO j=1,neta_eq
     ...
     g_12(:,j)=g_12(:,j)/(ra+1e-30_R8)
     g_22(:,j)=g_22(:,j)/((ra+1e-30_R8)*(ra+1e-30_R8))
...
SUBROUTINE Trid(u2,nr)
  ...
  INTEGER(ITM_I4) :: nr
  REAL(R8), DIMENSION(nr) :: u2

  INTEGER(ITM_I4) :: i,im
  REAL(R8), DIMENSION(:), ALLOCATABLE :: a2,d2,c2

  LOGICAL, SAVE :: first_flag = .true.

  IF (first_flag) THEN

     ALLOCATE (a2(nr),d2(nr),c2(nr))

  END IF

  u2=u2/REAL((nr-1)*(nr-1))

  DO i=2,nr-1
     c2(i)=-1._R8-1./(2._R8*REAL(i-1))
     a2(i)=-1._R8+1./(2._R8*REAL(i-1))
     d2(i)=2._R8
  END DO
  c2(1)=-4._R8
  d2(1)=4._R8
  ...
  DO i=1,nr-1
     im=MAX(1,i-1)
     d2(i)=1./(d2(i)-a2(i)*c2(im))
     c2(i)=c2(i)*d2(i)
     u2(i)=(u2(i)-a2(i)*u2(im))*d2(i)
  END DO

  u2(nr)=0._R8

  DO i=nr-1,1,-1
     u2(i)=u2(i)-c2(i)*u2(i+1)
  END DO
