...
!...  set the global parameters
  ...
  coreprof(1)%composition%amn = 1.0_R8
  coreprof(1)%composition%zion = 1.0_R8
...
!...  set equilibrium
  ...
  ra => eq(1)%profiles_1d%rho_tor
  qq => eq(1)%profiles_1d%q

  ra=(/ ((1.0_R8/npsi_eq)*(i-1),i=1,npsi_eq) /)
  qq=1.5_R8+2.5_R8*ra*ra

  ra=ra*eq(1)%eqgeometry%a_minor
...
!  CALL Get_Code_Parms(code_parameters, 'bohmgb.xml', '', 'bohmgb.xsd')
  CALL Fill_Param(code_parameters, 'bohmgb.xml', '', 'bohmgb.xsd')

!...  call transport routine

  CALL BohmGB(eq, coreprof, coretransp, code_parameters)

!...  write profiles

  OPEN (10, file = 'prof.dat', form = 'formatted')
  WRITE(10,*) ' i   rho_tor   ne(19)   Te(3)    Ti(3)      D      chi_e    chi_i   chi_phi'
  DO i=1,nrho_prof
     WRITE (10,100) i, &
!.     WRITE (10,*) &
          coreprof(1)%rho_tor(i), &
          coreprof(1)%ne%value(i)/1.e19_R8, &
          coreprof(1)%te%value(i)/1.e3_R8, &
          coreprof(1)%ti%value(i,1)/1.e3_R8, &
          coretransp(1)%values(1)%ne_transp%diff_eff(i,2), &
          coretransp(1)%values(1)%te_transp%diff_eff(i), &
          coretransp(1)%values(1)%ti_transp%diff_eff(i,1), &
          coretransp(1)%values(1)%vtor_transp%diff_eff(i,1)
