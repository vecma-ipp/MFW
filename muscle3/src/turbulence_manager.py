...
from utils import output_value_to_coretransp, coreprof_to_input_value
...
def turbulence_model_selector():
    ...
    rho_ind_s = instance.get_setting('rho_ind_s', '[float]')
    #input_names = instance.get_setting('input_names', '[str]')
    #output_names = instance.get_setting('output_names', '[str]')
    coretransp_default_file_name = instance.get_setting('cortransp_default', 'str')

    input_names = ["te_value", "ti_value", "te_ddrho", "ti_ddrho"]
    output_names = ["te_transp_flux", "ti_transp_flux"]

    n_rhos = len(rho_ind_s)
    ...
    while instance.reuse_instance():
        ...
        dict_outofbounds = coretransp_uncertainty['dict_outofbounds']

        # 3.1. If at least one value is outside of learned support, call a low-fidelity model
        #   Criterion 1: if only a subset of flux tubes have its fluxes values out of leaned bound
        ...
        if bool_call_lofi:
            ...
            for feature,dict_bounds in dict_outofbounds.items():
                for i,bool_coord in enumerate(dict_bounds['within']):
                    bool_coord_inbounds[i] = bool_coord_inbounds[i] and bool_coord
            
            fluxes_out = np.zeros((n_rhos, n_inputs), dtype=float)
        ...
        bool_outofbounds_total = not sum([sum(v['within']) for k,v in dict_outofbounds.items()]) # should be 1 if all values for 'within' were False; could be implemented with AND and without prcessing 'not'
