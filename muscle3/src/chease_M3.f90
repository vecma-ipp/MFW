program chease_M3
  ...
  do while (LIBMUSCLE_Instance_reuse_instance(instance))
     ...
     allocate (equil_in_buf(LIBMUSCLE_DataConstRef_size(rdata)))
     call LIBMUSCLE_DataConstRef_as_byte_array(rdata, equil_in_buf, err_code, err_msg)
     ...
     call chease2buf(equil_in_buf, equil_out_buf)

     sdata = LIBMUSCLE_Data_create_byte_array(equil_out_buf)
     smsg = LIBMUSCLE_Message_create(t_cur, sdata)
     call LIBMUSCLE_Instance_send(instance, 'equilibrium_out', smsg)
     call LIBMUSCLE_Message_free(smsg)
     call LIBMUSCLE_Data_free(sdata)
     
     deallocate(equil_in_buf)
     deallocate(equil_out_buf)
