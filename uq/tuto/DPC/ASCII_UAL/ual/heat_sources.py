...
class heat_sources(KeepInOrder):
	'''
	class heat_sources
	...
	- sources : class sourcesstruct_arraycalorimetry_heat_source: array of sourcesstruct_arraycalorimetry_heat_sourceObj objects
	   Heat sources. Array of structure (nheat_source)
	- sinks : class sinksstruct_arraycalorimetry_heat_source: array of sinksstruct_arraycalorimetry_heat_sourceObj objects
	...
	def putSlice(self, occurrence=0):
		...
		self.sinks.cpoTime = self.cpoTime
		self.sinks.putSlice(path, cpopath)
	...
	def replaceLastSlice(self, occurrence=0):
		...
		self.sinks.replaceLastSlice(path, cpopath)
	...
	def putNonTimed(self, occurrence=0):
		...
		self.sinks.putNonTimed(path, cpopath)
	...
	def getSlice(self, inTime, interpolMode, occurrence=0):
		...
		self.sinks.getSlice(path, cpopath, inTime, interpolMode)
	...
	def build_non_resampled_data(self, occurrence=0):
		...
		if nbslice > 0:
			...
			sinksList = self.sinks.build_non_resampled_data(path, cpopath, nbslice)
	...
	def deleteData(self, occurrence=0):
		...
		ull.deleteData(self.idx, path, cpopath + 'sinks')
		self.codeparam.deleteData(path, cpopath)
		ull.deleteData(self.idx, path, cpopath + 'time')


class heat_sourcesArray:
	'''
	class heat_sourcesArray
	Description of a set of heat sources or sinks. Time-dependent CPO.

	Attributes:
	- array : list of heat_sources
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class heat_sourcesArray nb_cpos=%d\n' %(len(self.array))
		for i in range(len(self.array)):
			ret = ret + space + 'heat_sources cpos=%d\n' %(i) + self.array[i].__str__(depth+1)
		return ret
	...
	def resize(self, nb_cpos):
		...
		for i in range(nb_cpos):
			self.array.append(heat_sources())
...
class sourcesstruct_arraycalorimetry_heat_source:
	'''
	class sourcesstruct_arraycalorimetry_heat_source
	Heat sources. Array of structure (nheat_source)

	Attributes:
	- array : list of sourcesstruct_arraycalorimetry_heat_sourceObj 
	'''
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class sourcesstruct_arraycalorimetry_heat_source\n'
		for i in range(len(self.array)):
			ret = ret + space + 'sourcesstruct_arraycalorimetry_heat_source[%d] = \n' %(i) + self.array[i].__str__(depth+1)
		return ret
	...
	def resize(self, nbelt):
		...
		for i in range(nbelt):
			self.array.append(sourcesstruct_arraycalorimetry_heat_sourceObj(self.base_path))
			self.array[i].setExpIdx(self.idx)

	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type sourcesstruct_arraycalorimetry_heat_source, run function putSlice') 

	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type sourcesstruct_arraycalorimetry_heat_source, run function replaceLastSlice') 

	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type sourcesstruct_arraycalorimetry_heat_source, run function putNonTimed') 
		if (dev()):
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type sourcesstruct_arraycalorimetry_heat_source, run function getSlice') 
		if (dev()):
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type sourcesstruct_arraycalorimetry_heat_source, run function build_non_resampled_data') 
		list=[]
		if nbslice > 0:
			if (dev()):
				print ('getObject('+path+', '+cpopath+self.base_path+', NON_TIMED)')
			...
			if status:
				print ('Failed to get data: ' + path + '/' + cpopath + self.base_path + ' (NON_TIMED)')
				for i in range(nbslice):
					list.append(sourcesstruct_arraycalorimetry_heat_source(self.base_path))
				return list
			obj_size = ull.getObjectDim(self.idx, obj)
			for n in range(nbslice):
				slice = sourcesstruct_arraycalorimetry_heat_source(self.base_path)
				slice.setExpIdx(self.idx)
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type sourcesstruct_arraycalorimetry_heat_source, run function putNonTimedElt') 
		if (dev()):
	...
	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type sourcesstruct_arraycalorimetry_heat_source, run function getNonTimedElt') 
		if (dev()):
...
class sourcesstruct_arraycalorimetry_heat_sourceObj(KeepInOrder):
	'''
	class sourcesstruct_arraycalorimetry_heat_sourceObj
	Heat sources. Array of structure (nheat_source)
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class sourcesstruct_arraycalorimetry_heat_sourceObj\n'
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type sourcesstruct_arraycalorimetry_heat_sourceObj, run function putNonTimedElt') 
		cpopath = cpopath + '/'
		if (dev()):
	...
	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type sourcesstruct_arraycalorimetry_heat_sourceObj, run function getNonTimedElt') 
		cpopath = cpopath + '/'
		if (dev()):
...
class sinksstruct_arraycalorimetry_heat_source:
	'''
	class sinksstruct_arraycalorimetry_heat_source
	Heat sinks. Array of structure (nheat_sink)

	Attributes:
	- array : list of sinksstruct_arraycalorimetry_heat_sourceObj 
	'''

	def __init__(self, base_path_in='sinks'):
		self.base_path = base_path_in
		self.idx = EMPTY_INT
		self.cpoTime = EMPTY_DOUBLE
		self.array = []
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class sinksstruct_arraycalorimetry_heat_source\n'
		for i in range(len(self.array)):
			ret = ret + space + 'sinksstruct_arraycalorimetry_heat_source[%d] = \n' %(i) + self.array[i].__str__(depth+1)
		return ret
	...
	def resize(self, nbelt):
		self.array = []
		for i in range(nbelt):
			self.array.append(sinksstruct_arraycalorimetry_heat_sourceObj(self.base_path))
			self.array[i].setExpIdx(self.idx)

	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type sinksstruct_arraycalorimetry_heat_source, run function putSlice') 

	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type sinksstruct_arraycalorimetry_heat_source, run function replaceLastSlice') 

	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type sinksstruct_arraycalorimetry_heat_source, run function putNonTimed') 
		if (dev()):
			print ('beginObject(None, 0, '+path+'/'+cpopath+self.base_path+', NON_TIMED)')
		obj = ull.beginObject(self.idx, None, 0, path + '/' + cpopath + self.base_path, NON_TIMED)
		for i in range(len(self.array)):
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type sinksstruct_arraycalorimetry_heat_source, run function getSlice') 
		if (dev()):
			print ('getObject('+path+', '+cpopath+self.base_path+', NON_TIMED)')
		status, obj = ull.getObject(self.idx, path, cpopath + self.base_path, NON_TIMED)
		if status:
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type sinksstruct_arraycalorimetry_heat_source, run function build_non_resampled_data') 
		list=[]
		if nbslice > 0:
			if (dev()):
				print ('getObject('+path+', '+cpopath+self.base_path+', NON_TIMED)')
			...
			if status:
				print ('Failed to get data: ' + path + '/' + cpopath + self.base_path + ' (NON_TIMED)')
				for i in range(nbslice):
					list.append(sinksstruct_arraycalorimetry_heat_source(self.base_path))
				return list
			obj_size = ull.getObjectDim(self.idx, obj)
			for n in range(nbslice):
				slice = sinksstruct_arraycalorimetry_heat_source(self.base_path)
				slice.setExpIdx(self.idx)
				slice.resize(obj_size)
				for i in range(obj_size):
					slice.array[i].getNonTimedElt(path, self.base_path, i, obj)
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type sinksstruct_arraycalorimetry_heat_source, run function putNonTimedElt') 
		if (dev()):
			print ('beginObject idx=%d' %(self.idx)) 
		if (dev()):
			print ('beginObject('+str(obj)+', '+str(i)+', '+path+'/'+cpopath+', NON_TIMED)')
	...
	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type sinksstruct_arraycalorimetry_heat_source, run function getNonTimedElt') 
		if (dev()):
			print ('getObjectFromObject('+str(obj)+', '+cpopath+', '+str(i)+')')
		status, obj2 = ull.getObjectFromObject(self.idx, obj, cpopath, i)
		if status:
...
class sinksstruct_arraycalorimetry_heat_sourceObj(KeepInOrder):
	'''
	class sinksstruct_arraycalorimetry_heat_sourceObj
	Heat sinks. Array of structure (nheat_sink)
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class sinksstruct_arraycalorimetry_heat_sourceObj\n'
		ret = ret + space + 'Attribute name: ' + str(self.name) + '\n'
		ret = ret + space + 'Attribute temp_in: ' + str(self.temp_in) + '\n'
		ret = ret + space + 'Attribute temp_out: ' + str(self.temp_out) + '\n'
		ret = ret + space + 'Attribute press_in: ' + str(self.press_in) + '\n'
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type sinksstruct_arraycalorimetry_heat_sourceObj, run function putNonTimedElt') 
		cpopath = cpopath + '/'
		if (dev()):
			print ('putStringInObject : ' + cpopath + 'name') 
			print ('obj = ' + str(obj))
	...
	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type sinksstruct_arraycalorimetry_heat_sourceObj, run function getNonTimedElt') 
		cpopath = cpopath + '/'
		if (dev()):
			print ('getStringInObject : ' + cpopath + 'name') 
			print ('obj = ' + str(obj))
