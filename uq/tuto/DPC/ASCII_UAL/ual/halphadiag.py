...
class halphadiag(KeepInOrder):
	...
	- setup : class setupstructurehalpha_setup
	   setup for the lines of sight of the line integrated measurement
	- intensity : class intensitystructureexp1D
	...
	def putSlice(self, occurrence=0):
		...
		self.intensity.cpoTime = self.cpoTime
		self.intensity.putSlice(path, cpopath)
	...
	def replaceLastSlice(self, occurrence=0):
		...
		self.intensity.replaceLastSlice(path, cpopath)
	...
	def putNonTimed(self, occurrence=0):
		...
		self.intensity.putNonTimed(path, cpopath)
	...
	def getSlice(self, inTime, interpolMode, occurrence=0):
		...
		self.intensity.getSlice(path, cpopath, inTime, interpolMode)
	...
	def build_non_resampled_data(self, occurrence=0):
		...
		if nbslice > 0:
			...
			intensityList = self.intensity.build_non_resampled_data(path, cpopath, nbslice)
			...
			for i in range(nbslice):
				...
				slice.intensity = intensityList[i]
	...
	def deleteData(self, occurrence=0):
		...
		self.intensity.deleteData(path, cpopath)
		self.codeparam.deleteData(path, cpopath)
		ull.deleteData(self.idx, path, cpopath + 'time')


class halphadiagArray:
	'''
	class halphadiagArray
	H/D alpha line integrated diagnostic; Time-dependent CPO

	Attributes:
	- array : list of halphadiag
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class halphadiagArray nb_cpos=%d\n' %(len(self.array))
		for i in range(len(self.array)):
			ret = ret + space + 'halphadiag cpos=%d\n' %(i) + self.array[i].__str__(depth+1)
		return ret
...
class setupstructurehalpha_setup(KeepInOrder):
	'''
	class setupstructurehalpha_setup
	setup for the lines of sight of the line integrated measurement
	...
	- horchordang : numpy.ndarray 1D with float
	...
	- verchordang : numpy.ndarray 1D with float
	...
	- solidangle : class solidanglestructureexp1D
	...
	def __init__(self, base_path_in='setup'):
		...
		self.horchordang = numpy.zeros(0, numpy.float64, order='C')
		self.verchordang = numpy.zeros(0, numpy.float64, order='C')
		self.second_point = second_pointstructurerzphi1D('second_point')
		self.solidangle = solidanglestructureexp1D('solidangle')

	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class setupstructurehalpha_setup\n'
		s = self.name.__str__()
		ret = ret + space + 'Attribute name\n' + space + s.replace('\n', '\n'+space) + '\n'
		ret = ret + space + 'Attribute pivot_point\n ' + self.pivot_point.__str__(depth+1)
		s = self.horchordang.__str__()
		ret = ret + space + 'Attribute horchordang\n' + space + s.replace('\n', '\n'+space) + '\n'
		s = self.verchordang.__str__()
		ret = ret + space + 'Attribute verchordang\n' + space + s.replace('\n', '\n'+space) + '\n'
		ret = ret + space + 'Attribute second_point\n ' + self.second_point.__str__(depth+1)
		ret = ret + space + 'Attribute solidangle\n ' + self.solidangle.__str__(depth+1)
		return ret
	...
	def setExpIdx(self, idx):
		...
		self.solidangle.setExpIdx(idx)

	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type setupstructurehalpha_setup, run function putSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		self.solidangle.cpoTime = self.cpoTime
		self.solidangle.putSlice(path, cpopath)

	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type setupstructurehalpha_setup, run function replaceLastSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		self.solidangle.replaceLastSlice(path, cpopath)

	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type setupstructurehalpha_setup, run function putNonTimed') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		status = ull.putVect1DDouble(self.idx, path, cpopath + 'horchordang', numpy.array(self.horchordang).astype(numpy.float64), False)
		check_status(status)
		status = ull.putVect1DDouble(self.idx, path, cpopath + 'verchordang', numpy.array(self.verchordang).astype(numpy.float64), False)
		check_status(status)
		self.second_point.putNonTimed(path, cpopath)
		self.solidangle.putNonTimed(path, cpopath)

	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type setupstructurehalpha_setup, run function getSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		status, ret_horchordang = ull.getVect1DDouble(self.idx, path, cpopath + 'horchordang')
		check_status(status)
		if not status:
			self.horchordang = ret_horchordang
		status, ret_verchordang = ull.getVect1DDouble(self.idx, path, cpopath + 'verchordang')
		check_status(status)
		if not status:
			self.verchordang = ret_verchordang
		self.second_point.getSlice(path, cpopath, inTime, interpolMode)
		self.solidangle.getSlice(path, cpopath, inTime, interpolMode)

	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type setupstructurehalpha_setup, run function build_non_resampled_data') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		if nbslice > 0:
			...
			status, horchordangVal = ull.getVect1DDouble(self.idx, path, cpopath + 'horchordang')
			check_status(status)
			status, verchordangVal = ull.getVect1DDouble(self.idx, path, cpopath + 'verchordang')
			check_status(status)
			second_pointList = self.second_point.build_non_resampled_data(path, cpopath, nbslice)
			solidangleList = self.solidangle.build_non_resampled_data(path, cpopath, nbslice)
			for i in range(nbslice):
				slice = setupstructurehalpha_setup(self.base_path)
				slice.setExpIdx(self.idx)
				slice.name = nameVal
				...
				slice.solidangle = solidangleList[i]
				list.append(slice)
		else:
			print ('error, nbslice must be > 0 and got:' + str(nbslice))
		return list
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type setupstructurehalpha_setupObj, run function putTimedElt') 
		cpopath = cpopath + '/' 
		obj = self.solidangle.putTimedElt(path, cpopath + 'solidangle', i, obj)
		return obj

	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type setupstructurehalpha_setupObj, run function getTimedElt') 
		cpopath = cpopath + '/' 
		self.solidangle.getTimedElt(path, cpopath + 'solidangle', i, obj)

	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type setupstructurehalpha_setupObj, run function putNonTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
		...
		if (dev()):
			print ('putVect1DDoubleInObject : ' + cpopath + 'horchordang') 
			print ('obj = ' + str(obj))
		obj = ull.putVect1DDoubleInObject(self.idx, obj, cpopath + 'horchordang', i, numpy.array(self.horchordang).astype(numpy.float64))
		if (dev()):
			print ('putVect1DDoubleInObject : ' + cpopath + 'verchordang') 
			print ('obj = ' + str(obj))
		obj = ull.putVect1DDoubleInObject(self.idx, obj, cpopath + 'verchordang', i, numpy.array(self.verchordang).astype(numpy.float64))
		obj = self.second_point.putNonTimedElt(path, cpopath + 'second_point', i, obj)
		obj = self.solidangle.putNonTimedElt(path, cpopath + 'solidangle', i, obj)
		return obj

	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type setupstructurehalpha_setupObj, run function getNonTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
		...
		if (dev()):
			print ('getVect1DDoubleInObject : ' + cpopath + 'horchordang') 
			print ('obj = ' + str(obj))
		status, ret_horchordang = ull.getVect1DDoubleFromObject(self.idx, obj, cpopath + 'horchordang', i)
		check_status(status)
		if not status:
			self.horchordang = ret_horchordang
		if (dev()):
			print ('getVect1DDoubleInObject : ' + cpopath + 'verchordang') 
			print ('obj = ' + str(obj))
		status, ret_verchordang = ull.getVect1DDoubleFromObject(self.idx, obj, cpopath + 'verchordang', i)
		check_status(status)
		if not status:
			self.verchordang = ret_verchordang
		self.second_point.getNonTimedElt(path, cpopath + 'second_point', i, obj)
		self.solidangle.getNonTimedElt(path, cpopath + 'solidangle', i, obj)
	...
	def deleteData(self, path, cpopath):
		...
		self.solidangle.deleteData(path, cpopath)
...
class solidanglestructureexp1D(KeepInOrder):
	'''
	class solidanglestructureexp1D
	Solid angle of the detector; [sr] Vector (nlos)
	...
	def __init__(self, base_path_in='solidangle'):
		self.base_path = base_path_in
		self.idx = EMPTY_INT
		self.cpoTime = EMPTY_DOUBLE
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class solidanglestructureexp1D\n'
	...
	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type solidanglestructureexp1D, run function putSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type solidanglestructureexp1D, run function replaceLastSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type solidanglestructureexp1D, run function putNonTimed') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type solidanglestructureexp1D, run function getSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type solidanglestructureexp1D, run function build_non_resampled_data') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		if nbslice > 0:
			...
			check_status(status)
			for i in range(nbslice):
				slice = solidanglestructureexp1D(self.base_path)
				slice.setExpIdx(self.idx)
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type solidanglestructureexp1DObj, run function putTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
	...
	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type solidanglestructureexp1DObj, run function getTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type solidanglestructureexp1DObj, run function putNonTimedElt') 
		cpopath = cpopath + '/' 
		return obj

	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type solidanglestructureexp1DObj, run function getNonTimedElt') 
		cpopath = cpopath + '/' 
...
class intensitystructureexp1D(KeepInOrder):
	'''
	class intensitystructureexp1D
	Measured light intensity (a.u.).  Time-dependent. Vector (nlos)
	...
	def __init__(self, base_path_in='intensity'):
		self.base_path = base_path_in
		self.idx = EMPTY_INT
		self.cpoTime = EMPTY_DOUBLE
		self.value = numpy.zeros(0, numpy.float64, order='C')
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class intensitystructureexp1D\n'
		s = self.value.__str__()
		ret = ret + space + 'Attribute value\n' + space + s.replace('\n', '\n'+space) + '\n'
		s = self.abserror.__str__()
		ret = ret + space + 'Attribute abserror\n' + space + s.replace('\n', '\n'+space) + '\n'
	...
	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type intensitystructureexp1D, run function putSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type intensitystructureexp1D, run function replaceLastSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type intensitystructureexp1D, run function putNonTimed') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type intensitystructureexp1D, run function getSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type intensitystructureexp1D, run function build_non_resampled_data') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		if nbslice > 0:
			...
			check_status(status)
			for i in range(nbslice):
				slice = intensitystructureexp1D(self.base_path)
				slice.setExpIdx(self.idx)
				slice.value = valueList[:,i]
				slice.abserror = abserrorList[:,i]
				slice.relerror = relerrorList[:,i]
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type intensitystructureexp1DObj, run function putTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
			print ('putVect1DDoubleInObject : ' + cpopath + 'value') 
			print ('obj = ' + str(obj))
	...
	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type intensitystructureexp1DObj, run function getTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
			print ('getVect1DDoubleInObject : ' + cpopath + 'value') 
			print ('obj = ' + str(obj))
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type intensitystructureexp1DObj, run function putNonTimedElt') 
		cpopath = cpopath + '/' 
		return obj

	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type intensitystructureexp1DObj, run function getNonTimedElt') 
		cpopath = cpopath + '/' 
