...
class coredelta(KeepInOrder):
	...
	- values : class valuesstruct_arraycoredelta_values: array of valuesstruct_arraycoredelta_valuesObj objects
	...
	def __init__(self):
		...
		self.values = valuesstruct_arraycoredelta_values('values')
		self.codeparam = codeparamstructurecodeparam('codeparam')
		self.time = EMPTY_DOUBLE

	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class coredelta\n'
...
class coredeltaArray:
	'''
	class coredeltaArray
	Generic instant change of the radial core profiles due to pellet, MHD, ... Time-dependent CPO.

	Attributes:
	- array : list of coredelta
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class coredeltaArray nb_cpos=%d\n' %(len(self.array))
		for i in range(len(self.array)):
			ret = ret + space + 'coredelta cpos=%d\n' %(i) + self.array[i].__str__(depth+1)
		return ret
...
class valuesstruct_arraycoredelta_values:
	'''
	class valuesstruct_arraycoredelta_values
	Description of the delta term for the various origins. Array of structure (ndelta). Time-dependent

	Attributes:
	- array : list of valuesstruct_arraycoredelta_valuesObj 
	'''
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class valuesstruct_arraycoredelta_values\n'
		for i in range(len(self.array)):
			ret = ret + space + 'valuesstruct_arraycoredelta_values[%d] = \n' %(i) + self.array[i].__str__(depth+1)
		return ret
	...
	def resize(self, nbelt):
		self.array = []
		for i in range(nbelt):
			self.array.append(valuesstruct_arraycoredelta_valuesObj(self.base_path))
			self.array[i].setExpIdx(self.idx)

	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type valuesstruct_arraycoredelta_values, run function putSlice') 
		if (dev()):
	...
	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type valuesstruct_arraycoredelta_values, run function replaceLastSlice') 
		if (dev()):
			print ('beginObject(None, 0, '+path+'/'+cpopath+self.base_path+', TIMED)')
	...
	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type valuesstruct_arraycoredelta_values, run function putNonTimed') 
		if (dev()):
			print ('beginObject(None, 0, '+path+'/'+cpopath+self.base_path+', NON_TIMED)')
		obj = ull.beginObject(self.idx, None, 0, path + '/' + cpopath + self.base_path, NON_TIMED)
		for i in range(len(self.array)):
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type valuesstruct_arraycoredelta_values, run function getSlice') 
		if (dev()):
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type valuesstruct_arraycoredelta_values, run function build_non_resampled_data') 
		list=[]
		if nbslice > 0:
			if (dev()):
			...
			if status:
				...
				for i in range(nbslice):
					list.append(valuesstruct_arraycoredelta_values(self.base_path))
				return list
			for n in range(nbslice):
				...
				if status:
					...
					continue
				obj_size = ull.getObjectDim(self.idx, obj)
				slice = valuesstruct_arraycoredelta_values(self.base_path)
				slice.setExpIdx(self.idx)
				slice.resize(obj_size)
				for i in range(obj_size):
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type valuesstruct_arraycoredelta_values, run function putTimedElt') 
		if (dev()):
			print ('beginObject idx=%d' %(self.idx)) 
		if (dev()):
	...
	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type valuesstruct_arraycoredelta_values, run function getTimedElt') 
		if (dev()):
			print ('getObjectFromObject('+str(obj)+', '+cpopath+', '+str(i)+')')
		status, obj2 = ull.getObjectFromObject(self.idx, obj, cpopath, i)
		if status:
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type valuesstruct_arraycoredelta_values, run function putNonTimedElt') 
		if (dev()):
			print ('beginObject idx=%d' %(self.idx)) 
		if (dev()):
			print ('beginObject('+str(obj)+', '+str(i)+', '+path+'/'+cpopath+', NON_TIMED)')
	...
	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type valuesstruct_arraycoredelta_values, run function getNonTimedElt') 
		if (dev()):
			print ('getObjectFromObject('+str(obj)+', '+cpopath+', '+str(i)+')')
		status, obj2 = ull.getObjectFromObject(self.idx, obj, cpopath, i)
		if status:
...
class valuesstruct_arraycoredelta_valuesObj(KeepInOrder):
	'''
	class valuesstruct_arraycoredelta_valuesObj
	Description of the delta term for the various origins. Array of structure (ndelta). Time-dependent

	Attributes:
	- deltaid : class deltaidstructureidentifier
	...
	- delta_psi : numpy.ndarray 1D with float
	...
	- impurity : class impuritystruct_arraycoredelta_values_impurity: array of impuritystruct_arraycoredelta_values_impurityObj objects
	...
	def __init__(self, base_path_in='values'):
		...
		self.idx = EMPTY_INT
		self.cpoTime = EMPTY_DOUBLE
		self.deltaid = deltaidstructureidentifier('deltaid')
		...
		self.delta_psi = numpy.zeros(0, numpy.float64, order='C')
		...
		self.impurity = impuritystruct_arraycoredelta_values_impurity('impurity')
		self.delta_vtor = numpy.zeros((0,0), numpy.float64, order='C')
		self.codeparam = codeparamstructurecodeparam('codeparam')

	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class valuesstruct_arraycoredelta_valuesObj\n'
		ret = ret + space + 'Attribute deltaid\n ' + self.deltaid.__str__(depth+1)
		...
		s = self.delta_psi.__str__()
		ret = ret + space + 'Attribute delta_psi\n' + space + s.replace('\n', '\n'+space) + '\n'
	...
	def setExpIdx(self, idx):
		self.idx = idx
		self.deltaid.setExpIdx(idx)
		self.impurity.setExpIdx(idx)
		self.codeparam.setExpIdx(idx)

	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type valuesstruct_arraycoredelta_valuesObj, run function putTimedElt') 
		cpopath = cpopath + '/'
		if (dev()):
		...
		if (dev()):
			print ('putVect1DDoubleInObject : ' + cpopath + 'delta_psi') 
			print ('obj = ' + str(obj))
		obj = ull.putVect1DDoubleInObject(self.idx, obj, cpopath + 'delta_psi', i, numpy.array(self.delta_psi).astype(numpy.float64))
		if (dev()):
	...
	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type valuesstruct_arraycoredelta_valuesObj, run function getTimedElt') 
		cpopath = cpopath + '/'
		if (dev()):
		...
		if not status:
			self.area = ret_area
		if (dev()):
			print ('getVect1DDoubleInObject : ' + cpopath + 'delta_psi') 
			print ('obj = ' + str(obj))
		status, ret_delta_psi = ull.getVect1DDoubleFromObject(self.idx, obj, cpopath + 'delta_psi', i)
		check_status(status)
		if not status:
			self.delta_psi = ret_delta_psi
		if (dev()):
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type valuesstruct_arraycoredelta_valuesObj, run function putNonTimedElt') 
		cpopath = cpopath + '/'
		obj = self.deltaid.putNonTimedElt(path, cpopath + 'deltaid', i, obj)
	...
	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type valuesstruct_arraycoredelta_valuesObj, run function getNonTimedElt') 
		cpopath = cpopath + '/'
		self.deltaid.getNonTimedElt(path, cpopath + 'deltaid', i, obj)
		self.impurity.getNonTimedElt(path, cpopath + 'impurity', i, obj)
		self.codeparam.getNonTimedElt(path, cpopath + 'codeparam', i, obj)


class deltaidstructureidentifier(KeepInOrder):
	'''
	class deltaidstructureidentifier
	Identifier for the origin of the delta terms (see conventions in the ITM website)
	...
	def __init__(self, base_path_in='deltaid'):
		self.base_path = base_path_in
		self.idx = EMPTY_INT
		self.cpoTime = EMPTY_DOUBLE
		self.id = ''
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class deltaidstructureidentifier\n'
		ret = ret + space + 'Attribute id: ' + str(self.id) + '\n'
		ret = ret + space + 'Attribute flag: ' + str(self.flag) + '\n'
		ret = ret + space + 'Attribute description: ' + str(self.description) + '\n'
		return ret
	...
	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type deltaidstructureidentifier, run function putSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type deltaidstructureidentifier, run function replaceLastSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type deltaidstructureidentifier, run function putNonTimed') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type deltaidstructureidentifier, run function getSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type deltaidstructureidentifier, run function build_non_resampled_data') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		if nbslice > 0:
			...
			check_status(status)
			for i in range(nbslice):
				slice = deltaidstructureidentifier(self.base_path)
				slice.setExpIdx(self.idx)
				slice.id = idVal
				slice.flag = flagVal
				slice.description = descriptionVal
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type deltaidstructureidentifierObj, run function putTimedElt') 
		cpopath = cpopath + '/' 
		return obj

	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type deltaidstructureidentifierObj, run function getTimedElt') 
		cpopath = cpopath + '/' 

	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type deltaidstructureidentifierObj, run function putNonTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
			print ('putStringInObject : ' + cpopath + 'id') 
			print ('obj = ' + str(obj))
	...
	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type deltaidstructureidentifierObj, run function getNonTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
			print ('getStringInObject : ' + cpopath + 'id') 
			print ('obj = ' + str(obj))
...
class impuritystruct_arraycoredelta_values_impurity:
	'''
	class impuritystruct_arraycoredelta_values_impurity
	Array(nimp). Time-dependent

	Attributes:
	- array : list of impuritystruct_arraycoredelta_values_impurityObj 
	'''
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class impuritystruct_arraycoredelta_values_impurity\n'
		for i in range(len(self.array)):
			ret = ret + space + 'impuritystruct_arraycoredelta_values_impurity[%d] = \n' %(i) + self.array[i].__str__(depth+1)
		return ret
	...
	def resize(self, nbelt):
		self.array = []
		for i in range(nbelt):
			self.array.append(impuritystruct_arraycoredelta_values_impurityObj(self.base_path))
			self.array[i].setExpIdx(self.idx)

	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type impuritystruct_arraycoredelta_values_impurity, run function putSlice') 
		if (dev()):
			print ('beginObject(None, 0, '+path+'/'+cpopath+self.base_path+', TIMED)')
		obj_time = ull.beginObject(self.idx, None, 0, path + '/' + cpopath + self.base_path, TIMED)
		obj = ull.beginObject(self.idx, obj_time, 0, 'ALLTIMES', TIMED)
	...
	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type impuritystruct_arraycoredelta_values_impurity, run function replaceLastSlice') 
		if (dev()):
			print ('beginObject(None, 0, '+path+'/'+cpopath+self.base_path+', TIMED)')
		obj = ull.beginObject(self.idx, None, 0, path + '/' + cpopath + self.base_path, TIMED)
		for i in range(len(self.array)):
	...
	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type impuritystruct_arraycoredelta_values_impurity, run function putNonTimed') 
		if (dev()):
			print ('beginObject(None, 0, '+path+'/'+cpopath+self.base_path+', NON_TIMED)')
		obj = ull.beginObject(self.idx, None, 0, path + '/' + cpopath + self.base_path, NON_TIMED)
		for i in range(len(self.array)):
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type impuritystruct_arraycoredelta_values_impurity, run function getSlice') 
		if (dev()):
			print ('getObjectSlice('+path+', '+cpopath+self.base_path+', '+str(inTime)+')')
		status, obj_time = ull.getObjectSlice(self.idx, path, cpopath + self.base_path, inTime)
		if status:
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type impuritystruct_arraycoredelta_values_impurity, run function build_non_resampled_data') 
		list=[]
		if nbslice > 0:
			if (dev()):
				print ('getObject('+path+', '+cpopath+self.base_path+', TIMED)')
			...
			if status:
				print ('Failed to get data: ' + path + '/' + cpopath + self.base_path + ' (TIMED)')
				for i in range(nbslice):
					list.append(impuritystruct_arraycoredelta_values_impurity(self.base_path))
				return list
			for n in range(nbslice):
				status, obj = ull.getObjectFromObject(self.idx, obj_time, 'ALLTIMES', n)
				if status:
					...
					continue
				obj_size = ull.getObjectDim(self.idx, obj)
				slice = impuritystruct_arraycoredelta_values_impurity(self.base_path)
				slice.setExpIdx(self.idx)
				slice.resize(obj_size)
				for i in range(obj_size):
					slice.array[i].getTimedElt(path, self.base_path, i, obj)
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type impuritystruct_arraycoredelta_values_impurity, run function putTimedElt') 
		if (dev()):
			print ('beginObject idx=%d' %(self.idx)) 
		if (dev()):
			print ('beginObject('+str(obj)+', '+str(i)+', '+path+'/'+cpopath+', TIMED)')
	...
	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type impuritystruct_arraycoredelta_values_impurity, run function getTimedElt') 
		if (dev()):
			print ('getObjectFromObject('+str(obj)+', '+cpopath+', '+str(i)+')')
		status, obj2 = ull.getObjectFromObject(self.idx, obj, cpopath, i)
		if status:
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type impuritystruct_arraycoredelta_values_impurity, run function putNonTimedElt') 
		if (dev()):
			print ('beginObject idx=%d' %(self.idx)) 
		if (dev()):
			print ('beginObject('+str(obj)+', '+str(i)+', '+path+'/'+cpopath+', NON_TIMED)')
	...
	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type impuritystruct_arraycoredelta_values_impurity, run function getNonTimedElt') 
		if (dev()):
			print ('getObjectFromObject('+str(obj)+', '+cpopath+', '+str(i)+')')
		status, obj2 = ull.getObjectFromObject(self.idx, obj, cpopath, i)
		if status:
...
class impuritystruct_arraycoredelta_values_impurityObj(KeepInOrder):
	'''
	class impuritystruct_arraycoredelta_values_impurityObj
	Array(nimp). Time-dependent

	Attributes:
	- delta_tz : numpy.ndarray 2D with float
	...
	def __init__(self, base_path_in='impurity'):
		...
		self.idx = EMPTY_INT
		self.cpoTime = EMPTY_DOUBLE
		self.delta_tz = numpy.zeros((0,0), numpy.float64, order='C')
		self.delta_nz = numpy.zeros((0,0), numpy.float64, order='C')

	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class impuritystruct_arraycoredelta_values_impurityObj\n'
		s = self.delta_tz.__str__()
		ret = ret + space + 'Attribute delta_tz\n' + space + s.replace('\n', '\n'+space) + '\n'
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type impuritystruct_arraycoredelta_values_impurityObj, run function putTimedElt') 
		cpopath = cpopath + '/'
		if (dev()):
			print ('putVect2DDoubleInObject : ' + cpopath + 'delta_tz') 
			print ('obj = ' + str(obj))
		obj = ull.putVect2DDoubleInObject(self.idx, obj, cpopath + 'delta_tz', i, numpy.array(self.delta_tz).astype(numpy.float64))
		if (dev()):
	...
	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type impuritystruct_arraycoredelta_values_impurityObj, run function getTimedElt') 
		cpopath = cpopath + '/'
		if (dev()):
			print ('getVect2DDoubleInObject : ' + cpopath + 'delta_tz') 
			print ('obj = ' + str(obj))
		status, ret_delta_tz = ull.getVect2DDoubleFromObject(self.idx, obj, cpopath + 'delta_tz', i)
		check_status(status)
		if not status:
			self.delta_tz = ret_delta_tz
		if (dev()):
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type impuritystruct_arraycoredelta_values_impurityObj, run function putNonTimedElt') 
		cpopath = cpopath + '/'
		return obj

	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type impuritystruct_arraycoredelta_values_impurityObj, run function getNonTimedElt') 
		cpopath = cpopath + '/'
