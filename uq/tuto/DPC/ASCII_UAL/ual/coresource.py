...
class coresource(KeepInOrder):
	...
	- values : class valuesstruct_arraycoresource_values: array of valuesstruct_arraycoresource_valuesObj objects
	...
	def __init__(self):
		self.base_path = 'coresource'
		self.idx = EMPTY_INT
		self.cpoTime = EMPTY_DOUBLE
		self.maxOccurrences = 15
		...
		self.values = valuesstruct_arraycoresource_values('values')
		self.codeparam = codeparamstructurecodeparam('codeparam')
		self.time = EMPTY_DOUBLE

	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class coresource\n'
...
class coresourceArray:
	'''
	class coresourceArray
	Generic source term for the core transport equations (radial profile). Time-dependent CPO.

	Attributes:
	- array : list of coresource
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class coresourceArray nb_cpos=%d\n' %(len(self.array))
		for i in range(len(self.array)):
			ret = ret + space + 'coresource cpos=%d\n' %(i) + self.array[i].__str__(depth+1)
		return ret
...
class valuesstruct_arraycoresource_values:
	'''
	class valuesstruct_arraycoresource_values
	Description of the source terms of various origins. Array of structure (nsource). Time-dependent.

	Attributes:
	- array : list of valuesstruct_arraycoresource_valuesObj 
	'''
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class valuesstruct_arraycoresource_values\n'
		for i in range(len(self.array)):
			ret = ret + space + 'valuesstruct_arraycoresource_values[%d] = \n' %(i) + self.array[i].__str__(depth+1)
		return ret
	...
	def resize(self, nbelt):
		self.array = []
		for i in range(nbelt):
			self.array.append(valuesstruct_arraycoresource_valuesObj(self.base_path))
			self.array[i].setExpIdx(self.idx)

	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type valuesstruct_arraycoresource_values, run function putSlice') 
		if (dev()):
	...
	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type valuesstruct_arraycoresource_values, run function replaceLastSlice') 
		if (dev()):
			print ('beginObject(None, 0, '+path+'/'+cpopath+self.base_path+', TIMED)')
	...
	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type valuesstruct_arraycoresource_values, run function putNonTimed') 
		if (dev()):
			print ('beginObject(None, 0, '+path+'/'+cpopath+self.base_path+', NON_TIMED)')
		obj = ull.beginObject(self.idx, None, 0, path + '/' + cpopath + self.base_path, NON_TIMED)
		for i in range(len(self.array)):
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type valuesstruct_arraycoresource_values, run function getSlice') 
		if (dev()):
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type valuesstruct_arraycoresource_values, run function build_non_resampled_data') 
		list=[]
		if nbslice > 0:
			if (dev()):
			...
			if status:
				...
				for i in range(nbslice):
					list.append(valuesstruct_arraycoresource_values(self.base_path))
				return list
			for n in range(nbslice):
				...
				if status:
					...
					continue
				obj_size = ull.getObjectDim(self.idx, obj)
				slice = valuesstruct_arraycoresource_values(self.base_path)
				slice.setExpIdx(self.idx)
				slice.resize(obj_size)
				for i in range(obj_size):
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type valuesstruct_arraycoresource_values, run function putTimedElt') 
		if (dev()):
			print ('beginObject idx=%d' %(self.idx)) 
		if (dev()):
	...
	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type valuesstruct_arraycoresource_values, run function getTimedElt') 
		if (dev()):
			print ('getObjectFromObject('+str(obj)+', '+cpopath+', '+str(i)+')')
		status, obj2 = ull.getObjectFromObject(self.idx, obj, cpopath, i)
		if status:
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type valuesstruct_arraycoresource_values, run function putNonTimedElt') 
		if (dev()):
			print ('beginObject idx=%d' %(self.idx)) 
		if (dev()):
			print ('beginObject('+str(obj)+', '+str(i)+', '+path+'/'+cpopath+', NON_TIMED)')
	...
	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type valuesstruct_arraycoresource_values, run function getNonTimedElt') 
		if (dev()):
			print ('getObjectFromObject('+str(obj)+', '+cpopath+', '+str(i)+')')
		status, obj2 = ull.getObjectFromObject(self.idx, obj, cpopath, i)
		if status:
...
class valuesstruct_arraycoresource_valuesObj(KeepInOrder):
	'''
	class valuesstruct_arraycoresource_valuesObj
	Description of the source terms of various origins. Array of structure (nsource). Time-dependent.

	Attributes:
	- sourceid : class sourceidstructureidentifier
	...
	def __init__(self, base_path_in='values'):
		...
		self.idx = EMPTY_INT
		self.cpoTime = EMPTY_DOUBLE
		self.sourceid = sourceidstructureidentifier('sourceid')
		...
		self.j = numpy.zeros(0, numpy.float64, order='C')
		self.sigma = numpy.zeros(0, numpy.float64, order='C')
		self.si = sistructuresource_ion('si')
		self.se = sestructuresource_vec('se')
		self.sz = szstruct_arraysource_imp('sz')
		self.qi = qistructuresource_ion('qi')
		self.qe = qestructuresource_vec('qe')
		self.qz = qzstruct_arraysource_imp('qz')
		self.ui = uistructuresource_ion('ui')
		self.ujxb = ujxbstructuresource_vec('ujxb')
		self.codeparam = codeparamstructurecodeparam('codeparam')

	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class valuesstruct_arraycoresource_valuesObj\n'
		ret = ret + space + 'Attribute sourceid\n ' + self.sourceid.__str__(depth+1)
		...
		ret = ret + space + 'Attribute si\n ' + self.si.__str__(depth+1)
		ret = ret + space + 'Attribute se\n ' + self.se.__str__(depth+1)
		ret = ret + space + 'Attribute sz\n ' + self.sz.__str__(depth+1)
		ret = ret + space + 'Attribute qi\n ' + self.qi.__str__(depth+1)
		ret = ret + space + 'Attribute qe\n ' + self.qe.__str__(depth+1)
		ret = ret + space + 'Attribute qz\n ' + self.qz.__str__(depth+1)
		ret = ret + space + 'Attribute ui\n ' + self.ui.__str__(depth+1)
		ret = ret + space + 'Attribute ujxb\n ' + self.ujxb.__str__(depth+1)
		ret = ret + space + 'Attribute codeparam\n ' + self.codeparam.__str__(depth+1)
		return ret

	def setExpIdx(self, idx):
		self.idx = idx
		self.sourceid.setExpIdx(idx)
		self.si.setExpIdx(idx)
		self.se.setExpIdx(idx)
		self.sz.setExpIdx(idx)
		self.qi.setExpIdx(idx)
		self.qe.setExpIdx(idx)
		self.qz.setExpIdx(idx)
		self.ui.setExpIdx(idx)
		self.ujxb.setExpIdx(idx)
		self.codeparam.setExpIdx(idx)

	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type valuesstruct_arraycoresource_valuesObj, run function putTimedElt') 
		cpopath = cpopath + '/'
		if (dev()):
		...
		if (dev()):
			print ('putVect1DDoubleInObject : ' + cpopath + 'j') 
			print ('obj = ' + str(obj))
		obj = ull.putVect1DDoubleInObject(self.idx, obj, cpopath + 'j', i, numpy.array(self.j).astype(numpy.float64))
		if (dev()):
		...
		obj = self.si.putTimedElt(path, cpopath + 'si', i, obj)
		obj = self.se.putTimedElt(path, cpopath + 'se', i, obj)
		obj = self.sz.putTimedElt(path, cpopath + 'sz', i, obj)
		obj = self.qi.putTimedElt(path, cpopath + 'qi', i, obj)
		obj = self.qe.putTimedElt(path, cpopath + 'qe', i, obj)
		obj = self.qz.putTimedElt(path, cpopath + 'qz', i, obj)
		obj = self.ui.putTimedElt(path, cpopath + 'ui', i, obj)
		obj = self.ujxb.putTimedElt(path, cpopath + 'ujxb', i, obj)
		obj = self.codeparam.putTimedElt(path, cpopath + 'codeparam', i, obj)
		return obj

	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type valuesstruct_arraycoresource_valuesObj, run function getTimedElt') 
		cpopath = cpopath + '/'
		if (dev()):
		...
		if not status:
			self.area = ret_area
		if (dev()):
			print ('getVect1DDoubleInObject : ' + cpopath + 'j') 
			print ('obj = ' + str(obj))
		status, ret_j = ull.getVect1DDoubleFromObject(self.idx, obj, cpopath + 'j', i)
		check_status(status)
		if not status:
			self.j = ret_j
		if (dev()):
		...
		self.si.getTimedElt(path, cpopath + 'si', i, obj)
		self.se.getTimedElt(path, cpopath + 'se', i, obj)
		self.sz.getTimedElt(path, cpopath + 'sz', i, obj)
		self.qi.getTimedElt(path, cpopath + 'qi', i, obj)
		self.qe.getTimedElt(path, cpopath + 'qe', i, obj)
		self.qz.getTimedElt(path, cpopath + 'qz', i, obj)
		self.ui.getTimedElt(path, cpopath + 'ui', i, obj)
		self.ujxb.getTimedElt(path, cpopath + 'ujxb', i, obj)
		self.codeparam.getTimedElt(path, cpopath + 'codeparam', i, obj)

	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type valuesstruct_arraycoresource_valuesObj, run function putNonTimedElt') 
		cpopath = cpopath + '/'
		obj = self.sourceid.putNonTimedElt(path, cpopath + 'sourceid', i, obj)
		obj = self.si.putNonTimedElt(path, cpopath + 'si', i, obj)
		obj = self.se.putNonTimedElt(path, cpopath + 'se', i, obj)
		obj = self.sz.putNonTimedElt(path, cpopath + 'sz', i, obj)
		obj = self.qi.putNonTimedElt(path, cpopath + 'qi', i, obj)
		obj = self.qe.putNonTimedElt(path, cpopath + 'qe', i, obj)
		obj = self.qz.putNonTimedElt(path, cpopath + 'qz', i, obj)
		obj = self.ui.putNonTimedElt(path, cpopath + 'ui', i, obj)
		obj = self.ujxb.putNonTimedElt(path, cpopath + 'ujxb', i, obj)
		obj = self.codeparam.putNonTimedElt(path, cpopath + 'codeparam', i, obj)
		return obj

	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type valuesstruct_arraycoresource_valuesObj, run function getNonTimedElt') 
		cpopath = cpopath + '/'
		self.sourceid.getNonTimedElt(path, cpopath + 'sourceid', i, obj)
		self.si.getNonTimedElt(path, cpopath + 'si', i, obj)
		self.se.getNonTimedElt(path, cpopath + 'se', i, obj)
		self.sz.getNonTimedElt(path, cpopath + 'sz', i, obj)
		self.qi.getNonTimedElt(path, cpopath + 'qi', i, obj)
		self.qe.getNonTimedElt(path, cpopath + 'qe', i, obj)
		self.qz.getNonTimedElt(path, cpopath + 'qz', i, obj)
		self.ui.getNonTimedElt(path, cpopath + 'ui', i, obj)
		self.ujxb.getNonTimedElt(path, cpopath + 'ujxb', i, obj)
		self.codeparam.getNonTimedElt(path, cpopath + 'codeparam', i, obj)


class sourceidstructureidentifier(KeepInOrder):
	'''
	class sourceidstructureidentifier
	Identifier for the origin of the source terms (see conventions in the ITM website)
	...
	def __init__(self, base_path_in='sourceid'):
		self.base_path = base_path_in
		self.idx = EMPTY_INT
		self.cpoTime = EMPTY_DOUBLE
		self.id = ''
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class sourceidstructureidentifier\n'
		ret = ret + space + 'Attribute id: ' + str(self.id) + '\n'
		ret = ret + space + 'Attribute flag: ' + str(self.flag) + '\n'
		ret = ret + space + 'Attribute description: ' + str(self.description) + '\n'
		return ret
	...
	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type sourceidstructureidentifier, run function putSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type sourceidstructureidentifier, run function replaceLastSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type sourceidstructureidentifier, run function putNonTimed') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type sourceidstructureidentifier, run function getSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type sourceidstructureidentifier, run function build_non_resampled_data') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		if nbslice > 0:
			...
			check_status(status)
			for i in range(nbslice):
				slice = sourceidstructureidentifier(self.base_path)
				slice.setExpIdx(self.idx)
				slice.id = idVal
				slice.flag = flagVal
				slice.description = descriptionVal
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type sourceidstructureidentifierObj, run function putTimedElt') 
		cpopath = cpopath + '/' 
		return obj

	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type sourceidstructureidentifierObj, run function getTimedElt') 
		cpopath = cpopath + '/' 

	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type sourceidstructureidentifierObj, run function putNonTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
			print ('putStringInObject : ' + cpopath + 'id') 
			print ('obj = ' + str(obj))
	...
	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type sourceidstructureidentifierObj, run function getNonTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
			print ('getStringInObject : ' + cpopath + 'id') 
			print ('obj = ' + str(obj))
...
class sistructuresource_ion(KeepInOrder):
	'''
	class sistructuresource_ion
	Particle source for ion density transport equation [m^-3.s^-1]. Time-dependent.

	Attributes:
	- exp : numpy.ndarray 2D with float
	   Explicit source term [same unit as root quantity]. Time-dependent. Matrix (nrho,nion)
	- imp : numpy.ndarray 2D with float
	   Implicit source term [s^-1.m^-3]. Time-dependent. Matrix (nrho,nion)
	'''

	def __init__(self, base_path_in='si'):
		self.base_path = base_path_in
		self.idx = EMPTY_INT
		self.cpoTime = EMPTY_DOUBLE
		self.exp = numpy.zeros((0,0), numpy.float64, order='C')
		self.imp = numpy.zeros((0,0), numpy.float64, order='C')

	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class sistructuresource_ion\n'
		s = self.exp.__str__()
		ret = ret + space + 'Attribute exp\n' + space + s.replace('\n', '\n'+space) + '\n'
		s = self.imp.__str__()
		ret = ret + space + 'Attribute imp\n' + space + s.replace('\n', '\n'+space) + '\n'
		return ret
	...
	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type sistructuresource_ion, run function putSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		if (verb()):
			print ('path = ' + path + '  and cpopath = ' + cpopath) 
		status = ull.putVect2DDoubleSlice(self.idx, path, cpopath + 'exp', numpy.array(self.exp).astype(numpy.float64), self.cpoTime)
		check_status(status)
		status = ull.putVect2DDoubleSlice(self.idx, path, cpopath + 'imp', numpy.array(self.imp).astype(numpy.float64), self.cpoTime)
		check_status(status)

	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type sistructuresource_ion, run function replaceLastSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		if (verb()):
			print ('path = ' + path + '  and cpopath = ' + cpopath) 
		status = ull.replaceLastVect2DDoubleSlice(self.idx, path, cpopath + 'exp', numpy.array(self.exp).astype(numpy.float64))
		check_status(status)
		status = ull.replaceLastVect2DDoubleSlice(self.idx, path, cpopath + 'imp', numpy.array(self.imp).astype(numpy.float64))
		check_status(status)

	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type sistructuresource_ion, run function putNonTimed') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type sistructuresource_ion, run function getSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		if (verb()):
			print ('path = ' + path + '  and cpopath = ' + cpopath) 
		status, ret_exp, retTime = ull.getVect2DDoubleSlice(self.idx, path, cpopath + 'exp', inTime, interpolMode)
		check_status(status)
		if not status:
			self.exp = ret_exp
			self.cpoTime = retTime
		status, ret_imp, retTime = ull.getVect2DDoubleSlice(self.idx, path, cpopath + 'imp', inTime, interpolMode)
		check_status(status)
		if not status:
			self.imp = ret_imp
			self.cpoTime = retTime
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type sistructuresource_ion, run function build_non_resampled_data') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		list=[]
		if nbslice > 0:
			status, expList = ull.getVect3DDouble(self.idx, path, cpopath + 'exp')
			if len(expList) == 0:
				expList = numpy.resize(expList, (0,0,nbslice))
			check_status(status)
			status, impList = ull.getVect3DDouble(self.idx, path, cpopath + 'imp')
			if len(impList) == 0:
				impList = numpy.resize(impList, (0,0,nbslice))
			check_status(status)
			for i in range(nbslice):
				slice = sistructuresource_ion(self.base_path)
				slice.setExpIdx(self.idx)
				slice.exp = expList[:,:,i]
				slice.imp = impList[:,:,i]
				list.append(slice)
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type sistructuresource_ionObj, run function putTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
			print ('putVect2DDoubleInObject : ' + cpopath + 'exp') 
			print ('obj = ' + str(obj))
		obj = ull.putVect2DDoubleInObject(self.idx, obj, cpopath + 'exp', i, numpy.array(self.exp).astype(numpy.float64))
		if (dev()):
			print ('putVect2DDoubleInObject : ' + cpopath + 'imp') 
			print ('obj = ' + str(obj))
		obj = ull.putVect2DDoubleInObject(self.idx, obj, cpopath + 'imp', i, numpy.array(self.imp).astype(numpy.float64))
		return obj

	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type sistructuresource_ionObj, run function getTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
			print ('getVect2DDoubleInObject : ' + cpopath + 'exp') 
			print ('obj = ' + str(obj))
		status, ret_exp = ull.getVect2DDoubleFromObject(self.idx, obj, cpopath + 'exp', i)
		check_status(status)
		if not status:
			self.exp = ret_exp
		if (dev()):
			print ('getVect2DDoubleInObject : ' + cpopath + 'imp') 
			print ('obj = ' + str(obj))
		status, ret_imp = ull.getVect2DDoubleFromObject(self.idx, obj, cpopath + 'imp', i)
		check_status(status)
		if not status:
			self.imp = ret_imp
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type sistructuresource_ionObj, run function putNonTimedElt') 
		cpopath = cpopath + '/' 
		return obj

	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type sistructuresource_ionObj, run function getNonTimedElt') 
		cpopath = cpopath + '/' 
	...
	def deleteData(self, path, cpopath):
		...
		if (verb()):
			print ('path = ' + path + '  and cpopath = ' + cpopath) 
		ull.deleteData(self.idx, path, cpopath + 'exp')
		ull.deleteData(self.idx, path, cpopath + 'imp')


class sestructuresource_vec(KeepInOrder):
	'''
	class sestructuresource_vec
	Particle source for electron density transport equation [m^-3.s^-1]. Time-dependent.

	Attributes:
	- exp : numpy.ndarray 1D with float
	   Explicit source term [same unit as root quantity]. Time-dependent. Vector (nrho)
	- imp : numpy.ndarray 1D with float
	   Implicit source term [s^-1.m^-3]. Time-dependent. Vector (nrho)
	'''

	def __init__(self, base_path_in='se'):
		self.base_path = base_path_in
		self.idx = EMPTY_INT
		self.cpoTime = EMPTY_DOUBLE
		self.exp = numpy.zeros(0, numpy.float64, order='C')
		self.imp = numpy.zeros(0, numpy.float64, order='C')

	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class sestructuresource_vec\n'
		s = self.exp.__str__()
		ret = ret + space + 'Attribute exp\n' + space + s.replace('\n', '\n'+space) + '\n'
		s = self.imp.__str__()
		ret = ret + space + 'Attribute imp\n' + space + s.replace('\n', '\n'+space) + '\n'
	...
	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type sestructuresource_vec, run function putSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		if (verb()):
			print ('path = ' + path + '  and cpopath = ' + cpopath) 
		status = ull.putVect1DDoubleSlice(self.idx, path, cpopath + 'exp', numpy.array(self.exp).astype(numpy.float64), self.cpoTime)
		check_status(status)
		status = ull.putVect1DDoubleSlice(self.idx, path, cpopath + 'imp', numpy.array(self.imp).astype(numpy.float64), self.cpoTime)
		check_status(status)

	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type sestructuresource_vec, run function replaceLastSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		if (verb()):
			print ('path = ' + path + '  and cpopath = ' + cpopath) 
		status = ull.replaceLastVect1DDoubleSlice(self.idx, path, cpopath + 'exp', numpy.array(self.exp).astype(numpy.float64))
		check_status(status)
		status = ull.replaceLastVect1DDoubleSlice(self.idx, path, cpopath + 'imp', numpy.array(self.imp).astype(numpy.float64))
		check_status(status)

	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type sestructuresource_vec, run function putNonTimed') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type sestructuresource_vec, run function getSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		if (verb()):
			print ('path = ' + path + '  and cpopath = ' + cpopath) 
		status, ret_exp, retTime = ull.getVect1DDoubleSlice(self.idx, path, cpopath + 'exp', inTime, interpolMode)
		check_status(status)
		if not status:
			self.exp = ret_exp
			self.cpoTime = retTime
		status, ret_imp, retTime = ull.getVect1DDoubleSlice(self.idx, path, cpopath + 'imp', inTime, interpolMode)
		check_status(status)
		if not status:
			self.imp = ret_imp
			self.cpoTime = retTime
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type sestructuresource_vec, run function build_non_resampled_data') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		list=[]
		if nbslice > 0:
			status, expList = ull.getVect2DDouble(self.idx, path, cpopath + 'exp')
			if len(expList) == 0:
				expList = numpy.resize(expList, (0,nbslice))
			check_status(status)
			status, impList = ull.getVect2DDouble(self.idx, path, cpopath + 'imp')
			if len(impList) == 0:
				impList = numpy.resize(impList, (0,nbslice))
			check_status(status)
			for i in range(nbslice):
				slice = sestructuresource_vec(self.base_path)
				slice.setExpIdx(self.idx)
				slice.exp = expList[:,i]
				slice.imp = impList[:,i]
				list.append(slice)
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type sestructuresource_vecObj, run function putTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
			print ('putVect1DDoubleInObject : ' + cpopath + 'exp') 
			print ('obj = ' + str(obj))
		obj = ull.putVect1DDoubleInObject(self.idx, obj, cpopath + 'exp', i, numpy.array(self.exp).astype(numpy.float64))
		if (dev()):
			print ('putVect1DDoubleInObject : ' + cpopath + 'imp') 
			print ('obj = ' + str(obj))
		obj = ull.putVect1DDoubleInObject(self.idx, obj, cpopath + 'imp', i, numpy.array(self.imp).astype(numpy.float64))
		return obj

	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type sestructuresource_vecObj, run function getTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
			print ('getVect1DDoubleInObject : ' + cpopath + 'exp') 
			print ('obj = ' + str(obj))
		status, ret_exp = ull.getVect1DDoubleFromObject(self.idx, obj, cpopath + 'exp', i)
		check_status(status)
		if not status:
			self.exp = ret_exp
		if (dev()):
			print ('getVect1DDoubleInObject : ' + cpopath + 'imp') 
			print ('obj = ' + str(obj))
		status, ret_imp = ull.getVect1DDoubleFromObject(self.idx, obj, cpopath + 'imp', i)
		check_status(status)
		if not status:
			self.imp = ret_imp
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type sestructuresource_vecObj, run function putNonTimedElt') 
		cpopath = cpopath + '/' 
		return obj

	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type sestructuresource_vecObj, run function getNonTimedElt') 
		cpopath = cpopath + '/' 
...
class szstruct_arraysource_imp:
	'''
	class szstruct_arraysource_imp
	Particle source for impurity density transport equation [m^-3.s^-1]. Vector(nimpur). Time-dependent.

	Attributes:
	- array : list of szstruct_arraysource_impObj 
	'''

	def __init__(self, base_path_in='sz'):
		self.base_path = base_path_in
		self.idx = EMPTY_INT
		self.cpoTime = EMPTY_DOUBLE
		self.array = []
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class szstruct_arraysource_imp\n'
		for i in range(len(self.array)):
			ret = ret + space + 'szstruct_arraysource_imp[%d] = \n' %(i) + self.array[i].__str__(depth+1)
		return ret
	...
	def resize(self, nbelt):
		self.array = []
		for i in range(nbelt):
			self.array.append(szstruct_arraysource_impObj(self.base_path))
			self.array[i].setExpIdx(self.idx)

	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type szstruct_arraysource_imp, run function putSlice') 
		if (dev()):
			print ('beginObject(None, 0, '+path+'/'+cpopath+self.base_path+', TIMED)')
		obj_time = ull.beginObject(self.idx, None, 0, path + '/' + cpopath + self.base_path, TIMED)
		obj = ull.beginObject(self.idx, obj_time, 0, 'ALLTIMES', TIMED)
	...
	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type szstruct_arraysource_imp, run function replaceLastSlice') 
		if (dev()):
			print ('beginObject(None, 0, '+path+'/'+cpopath+self.base_path+', TIMED)')
		obj = ull.beginObject(self.idx, None, 0, path + '/' + cpopath + self.base_path, TIMED)
		for i in range(len(self.array)):
	...
	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type szstruct_arraysource_imp, run function putNonTimed') 
		if (dev()):
			print ('beginObject(None, 0, '+path+'/'+cpopath+self.base_path+', NON_TIMED)')
		obj = ull.beginObject(self.idx, None, 0, path + '/' + cpopath + self.base_path, NON_TIMED)
		for i in range(len(self.array)):
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type szstruct_arraysource_imp, run function getSlice') 
		if (dev()):
			print ('getObjectSlice('+path+', '+cpopath+self.base_path+', '+str(inTime)+')')
		status, obj_time = ull.getObjectSlice(self.idx, path, cpopath + self.base_path, inTime)
		if status:
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type szstruct_arraysource_imp, run function build_non_resampled_data') 
		list=[]
		if nbslice > 0:
			if (dev()):
				print ('getObject('+path+', '+cpopath+self.base_path+', TIMED)')
			...
			if status:
				print ('Failed to get data: ' + path + '/' + cpopath + self.base_path + ' (TIMED)')
				for i in range(nbslice):
					list.append(szstruct_arraysource_imp(self.base_path))
				return list
			for n in range(nbslice):
				status, obj = ull.getObjectFromObject(self.idx, obj_time, 'ALLTIMES', n)
				if status:
					...
					continue
				obj_size = ull.getObjectDim(self.idx, obj)
				slice = szstruct_arraysource_imp(self.base_path)
				slice.setExpIdx(self.idx)
				slice.resize(obj_size)
				for i in range(obj_size):
					slice.array[i].getTimedElt(path, self.base_path, i, obj)
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type szstruct_arraysource_imp, run function putTimedElt') 
		if (dev()):
			print ('beginObject idx=%d' %(self.idx)) 
		if (dev()):
			print ('beginObject('+str(obj)+', '+str(i)+', '+path+'/'+cpopath+', TIMED)')
	...
	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type szstruct_arraysource_imp, run function getTimedElt') 
		if (dev()):
			print ('getObjectFromObject('+str(obj)+', '+cpopath+', '+str(i)+')')
		status, obj2 = ull.getObjectFromObject(self.idx, obj, cpopath, i)
		if status:
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type szstruct_arraysource_imp, run function putNonTimedElt') 
		if (dev()):
			print ('beginObject idx=%d' %(self.idx)) 
		if (dev()):
			print ('beginObject('+str(obj)+', '+str(i)+', '+path+'/'+cpopath+', NON_TIMED)')
	...
	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type szstruct_arraysource_imp, run function getNonTimedElt') 
		if (dev()):
			print ('getObjectFromObject('+str(obj)+', '+cpopath+', '+str(i)+')')
		status, obj2 = ull.getObjectFromObject(self.idx, obj, cpopath, i)
		if status:
...
class szstruct_arraysource_impObj(KeepInOrder):
	'''
	class szstruct_arraysource_impObj
	Particle source for impurity density transport equation [m^-3.s^-1]. Vector(nimpur). Time-dependent.
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class szstruct_arraysource_impObj\n'
		s = self.exp.__str__()
		ret = ret + space + 'Attribute exp\n' + space + s.replace('\n', '\n'+space) + '\n'
		s = self.imp.__str__()
		ret = ret + space + 'Attribute imp\n' + space + s.replace('\n', '\n'+space) + '\n'
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type szstruct_arraysource_impObj, run function putTimedElt') 
		cpopath = cpopath + '/'
		if (dev()):
			print ('putVect2DDoubleInObject : ' + cpopath + 'exp') 
			print ('obj = ' + str(obj))
	...
	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type szstruct_arraysource_impObj, run function getTimedElt') 
		cpopath = cpopath + '/'
		if (dev()):
			print ('getVect2DDoubleInObject : ' + cpopath + 'exp') 
			print ('obj = ' + str(obj))
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type szstruct_arraysource_impObj, run function putNonTimedElt') 
		cpopath = cpopath + '/'
		return obj

	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type szstruct_arraysource_impObj, run function getNonTimedElt') 
		cpopath = cpopath + '/'


class qistructuresource_ion(KeepInOrder):
	'''
	class qistructuresource_ion
	Heat source for ion heat transport equations [W.m^-3]. Time-dependent.
	...
	def __init__(self, base_path_in='qi'):
		self.base_path = base_path_in
		self.idx = EMPTY_INT
		self.cpoTime = EMPTY_DOUBLE
		self.exp = numpy.zeros((0,0), numpy.float64, order='C')
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class qistructuresource_ion\n'
		s = self.exp.__str__()
		ret = ret + space + 'Attribute exp\n' + space + s.replace('\n', '\n'+space) + '\n'
		s = self.imp.__str__()
		ret = ret + space + 'Attribute imp\n' + space + s.replace('\n', '\n'+space) + '\n'
	...
	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type qistructuresource_ion, run function putSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type qistructuresource_ion, run function replaceLastSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type qistructuresource_ion, run function putNonTimed') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type qistructuresource_ion, run function getSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type qistructuresource_ion, run function build_non_resampled_data') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		if nbslice > 0:
			...
			check_status(status)
			for i in range(nbslice):
				slice = qistructuresource_ion(self.base_path)
				slice.setExpIdx(self.idx)
				slice.exp = expList[:,:,i]
				slice.imp = impList[:,:,i]
				list.append(slice)
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type qistructuresource_ionObj, run function putTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
			print ('putVect2DDoubleInObject : ' + cpopath + 'exp') 
			print ('obj = ' + str(obj))
	...
	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type qistructuresource_ionObj, run function getTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
			print ('getVect2DDoubleInObject : ' + cpopath + 'exp') 
			print ('obj = ' + str(obj))
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type qistructuresource_ionObj, run function putNonTimedElt') 
		cpopath = cpopath + '/' 
		return obj

	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type qistructuresource_ionObj, run function getNonTimedElt') 
		cpopath = cpopath + '/' 
...
class qestructuresource_vec(KeepInOrder):
	'''
	class qestructuresource_vec
	Heat source for electron heat transport equation [W.m^-3]. Time-dependent.
	...
	def __init__(self, base_path_in='qe'):
		self.base_path = base_path_in
		self.idx = EMPTY_INT
		self.cpoTime = EMPTY_DOUBLE
		self.exp = numpy.zeros(0, numpy.float64, order='C')
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class qestructuresource_vec\n'
		s = self.exp.__str__()
		ret = ret + space + 'Attribute exp\n' + space + s.replace('\n', '\n'+space) + '\n'
		s = self.imp.__str__()
		ret = ret + space + 'Attribute imp\n' + space + s.replace('\n', '\n'+space) + '\n'
	...
	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type qestructuresource_vec, run function putSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type qestructuresource_vec, run function replaceLastSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type qestructuresource_vec, run function putNonTimed') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type qestructuresource_vec, run function getSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type qestructuresource_vec, run function build_non_resampled_data') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		if nbslice > 0:
			...
			check_status(status)
			for i in range(nbslice):
				slice = qestructuresource_vec(self.base_path)
				slice.setExpIdx(self.idx)
				slice.exp = expList[:,i]
				slice.imp = impList[:,i]
				list.append(slice)
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type qestructuresource_vecObj, run function putTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
			print ('putVect1DDoubleInObject : ' + cpopath + 'exp') 
			print ('obj = ' + str(obj))
	...
	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type qestructuresource_vecObj, run function getTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
			print ('getVect1DDoubleInObject : ' + cpopath + 'exp') 
			print ('obj = ' + str(obj))
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type qestructuresource_vecObj, run function putNonTimedElt') 
		cpopath = cpopath + '/' 
		return obj

	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type qestructuresource_vecObj, run function getNonTimedElt') 
		cpopath = cpopath + '/' 
...
class qzstruct_arraysource_imp:
	'''
	class qzstruct_arraysource_imp
	Heat source for impurity heat transport equations [W.m^-3]. Vector(nimpur). Time-dependent.

	Attributes:
	- array : list of qzstruct_arraysource_impObj 
	'''

	def __init__(self, base_path_in='qz'):
		self.base_path = base_path_in
		self.idx = EMPTY_INT
		self.cpoTime = EMPTY_DOUBLE
		self.array = []
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class qzstruct_arraysource_imp\n'
		for i in range(len(self.array)):
			ret = ret + space + 'qzstruct_arraysource_imp[%d] = \n' %(i) + self.array[i].__str__(depth+1)
		return ret
	...
	def resize(self, nbelt):
		self.array = []
		for i in range(nbelt):
			self.array.append(qzstruct_arraysource_impObj(self.base_path))
			self.array[i].setExpIdx(self.idx)

	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type qzstruct_arraysource_imp, run function putSlice') 
		if (dev()):
			print ('beginObject(None, 0, '+path+'/'+cpopath+self.base_path+', TIMED)')
		obj_time = ull.beginObject(self.idx, None, 0, path + '/' + cpopath + self.base_path, TIMED)
		obj = ull.beginObject(self.idx, obj_time, 0, 'ALLTIMES', TIMED)
	...
	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type qzstruct_arraysource_imp, run function replaceLastSlice') 
		if (dev()):
			print ('beginObject(None, 0, '+path+'/'+cpopath+self.base_path+', TIMED)')
		obj = ull.beginObject(self.idx, None, 0, path + '/' + cpopath + self.base_path, TIMED)
		for i in range(len(self.array)):
	...
	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type qzstruct_arraysource_imp, run function putNonTimed') 
		if (dev()):
			print ('beginObject(None, 0, '+path+'/'+cpopath+self.base_path+', NON_TIMED)')
		obj = ull.beginObject(self.idx, None, 0, path + '/' + cpopath + self.base_path, NON_TIMED)
		for i in range(len(self.array)):
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type qzstruct_arraysource_imp, run function getSlice') 
		if (dev()):
			print ('getObjectSlice('+path+', '+cpopath+self.base_path+', '+str(inTime)+')')
		status, obj_time = ull.getObjectSlice(self.idx, path, cpopath + self.base_path, inTime)
		if status:
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type qzstruct_arraysource_imp, run function build_non_resampled_data') 
		list=[]
		if nbslice > 0:
			if (dev()):
				print ('getObject('+path+', '+cpopath+self.base_path+', TIMED)')
			...
			if status:
				print ('Failed to get data: ' + path + '/' + cpopath + self.base_path + ' (TIMED)')
				for i in range(nbslice):
					list.append(qzstruct_arraysource_imp(self.base_path))
				return list
			for n in range(nbslice):
				status, obj = ull.getObjectFromObject(self.idx, obj_time, 'ALLTIMES', n)
				if status:
					...
					continue
				obj_size = ull.getObjectDim(self.idx, obj)
				slice = qzstruct_arraysource_imp(self.base_path)
				slice.setExpIdx(self.idx)
				slice.resize(obj_size)
				for i in range(obj_size):
					slice.array[i].getTimedElt(path, self.base_path, i, obj)
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type qzstruct_arraysource_imp, run function putTimedElt') 
		if (dev()):
			print ('beginObject idx=%d' %(self.idx)) 
		if (dev()):
			print ('beginObject('+str(obj)+', '+str(i)+', '+path+'/'+cpopath+', TIMED)')
	...
	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type qzstruct_arraysource_imp, run function getTimedElt') 
		if (dev()):
			print ('getObjectFromObject('+str(obj)+', '+cpopath+', '+str(i)+')')
		status, obj2 = ull.getObjectFromObject(self.idx, obj, cpopath, i)
		if status:
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type qzstruct_arraysource_imp, run function putNonTimedElt') 
		if (dev()):
			print ('beginObject idx=%d' %(self.idx)) 
		if (dev()):
			print ('beginObject('+str(obj)+', '+str(i)+', '+path+'/'+cpopath+', NON_TIMED)')
	...
	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type qzstruct_arraysource_imp, run function getNonTimedElt') 
		if (dev()):
			print ('getObjectFromObject('+str(obj)+', '+cpopath+', '+str(i)+')')
		status, obj2 = ull.getObjectFromObject(self.idx, obj, cpopath, i)
		if status:
...
class qzstruct_arraysource_impObj(KeepInOrder):
	'''
	class qzstruct_arraysource_impObj
	Heat source for impurity heat transport equations [W.m^-3]. Vector(nimpur). Time-dependent.
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class qzstruct_arraysource_impObj\n'
		s = self.exp.__str__()
		ret = ret + space + 'Attribute exp\n' + space + s.replace('\n', '\n'+space) + '\n'
		s = self.imp.__str__()
		ret = ret + space + 'Attribute imp\n' + space + s.replace('\n', '\n'+space) + '\n'
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type qzstruct_arraysource_impObj, run function putTimedElt') 
		cpopath = cpopath + '/'
		if (dev()):
			print ('putVect2DDoubleInObject : ' + cpopath + 'exp') 
			print ('obj = ' + str(obj))
	...
	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type qzstruct_arraysource_impObj, run function getTimedElt') 
		cpopath = cpopath + '/'
		if (dev()):
			print ('getVect2DDoubleInObject : ' + cpopath + 'exp') 
			print ('obj = ' + str(obj))
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type qzstruct_arraysource_impObj, run function putNonTimedElt') 
		cpopath = cpopath + '/'
		return obj

	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type qzstruct_arraysource_impObj, run function getNonTimedElt') 
		cpopath = cpopath + '/'


class uistructuresource_ion(KeepInOrder):
	'''
	class uistructuresource_ion
	Toroidal torque on individual ion species; for toroidal momentum transport equation [kg.m^-1.s^-2]. Time-dependent.
	...
	def __init__(self, base_path_in='ui'):
		self.base_path = base_path_in
		self.idx = EMPTY_INT
		self.cpoTime = EMPTY_DOUBLE
		self.exp = numpy.zeros((0,0), numpy.float64, order='C')
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class uistructuresource_ion\n'
		s = self.exp.__str__()
		ret = ret + space + 'Attribute exp\n' + space + s.replace('\n', '\n'+space) + '\n'
		s = self.imp.__str__()
		ret = ret + space + 'Attribute imp\n' + space + s.replace('\n', '\n'+space) + '\n'
	...
	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type uistructuresource_ion, run function putSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type uistructuresource_ion, run function replaceLastSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type uistructuresource_ion, run function putNonTimed') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type uistructuresource_ion, run function getSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type uistructuresource_ion, run function build_non_resampled_data') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		if nbslice > 0:
			...
			check_status(status)
			for i in range(nbslice):
				slice = uistructuresource_ion(self.base_path)
				slice.setExpIdx(self.idx)
				slice.exp = expList[:,:,i]
				slice.imp = impList[:,:,i]
				list.append(slice)
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type uistructuresource_ionObj, run function putTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
			print ('putVect2DDoubleInObject : ' + cpopath + 'exp') 
			print ('obj = ' + str(obj))
	...
	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type uistructuresource_ionObj, run function getTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
			print ('getVect2DDoubleInObject : ' + cpopath + 'exp') 
			print ('obj = ' + str(obj))
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type uistructuresource_ionObj, run function putNonTimedElt') 
		cpopath = cpopath + '/' 
		return obj

	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type uistructuresource_ionObj, run function getNonTimedElt') 
		cpopath = cpopath + '/' 
...
class ujxbstructuresource_vec(KeepInOrder):
	'''
	class ujxbstructuresource_vec
	Toroidal JxB torque on bulk plasma; for toroidal momentum transport equation [kg.m^-1.s^-2]. Here J is the return current from fast ion radial currents Jfast=-J. Time-dependent.
	...
	def __init__(self, base_path_in='ujxb'):
		self.base_path = base_path_in
		self.idx = EMPTY_INT
		self.cpoTime = EMPTY_DOUBLE
		self.exp = numpy.zeros(0, numpy.float64, order='C')
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class ujxbstructuresource_vec\n'
		s = self.exp.__str__()
		ret = ret + space + 'Attribute exp\n' + space + s.replace('\n', '\n'+space) + '\n'
		s = self.imp.__str__()
		ret = ret + space + 'Attribute imp\n' + space + s.replace('\n', '\n'+space) + '\n'
	...
	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type ujxbstructuresource_vec, run function putSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type ujxbstructuresource_vec, run function replaceLastSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type ujxbstructuresource_vec, run function putNonTimed') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type ujxbstructuresource_vec, run function getSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type ujxbstructuresource_vec, run function build_non_resampled_data') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		if nbslice > 0:
			...
			check_status(status)
			for i in range(nbslice):
				slice = ujxbstructuresource_vec(self.base_path)
				slice.setExpIdx(self.idx)
				slice.exp = expList[:,i]
				slice.imp = impList[:,i]
				list.append(slice)
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type ujxbstructuresource_vecObj, run function putTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
			print ('putVect1DDoubleInObject : ' + cpopath + 'exp') 
			print ('obj = ' + str(obj))
	...
	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type ujxbstructuresource_vecObj, run function getTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
			print ('getVect1DDoubleInObject : ' + cpopath + 'exp') 
			print ('obj = ' + str(obj))
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type ujxbstructuresource_vecObj, run function putNonTimedElt') 
		cpopath = cpopath + '/' 
		return obj

	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type ujxbstructuresource_vecObj, run function getNonTimedElt') 
		cpopath = cpopath + '/' 
