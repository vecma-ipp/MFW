...
class tsdiag(KeepInOrder):
	...
	- setup : class setupstructuretssetup
	   diagnostic setup information
	- measure : class measurestructuretsmeasure
	   Measured values
...
class tsdiagArray:
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class tsdiagArray nb_cpos=%d\n' %(len(self.array))
		for i in range(len(self.array)):
			ret = ret + space + 'tsdiag cpos=%d\n' %(i) + self.array[i].__str__(depth+1)
		return ret
...
class setupstructuretssetup(KeepInOrder):
	'''
	class setupstructuretssetup
	diagnostic setup information

	Attributes:
	- position : class positionstructurerzphi1D
	   Position of intersection between laser and line of sight; Vector (nchords)
	'''
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class setupstructuretssetup\n'
	...
	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type setupstructuretssetup, run function putSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type setupstructuretssetup, run function replaceLastSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type setupstructuretssetup, run function putNonTimed') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type setupstructuretssetup, run function getSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type setupstructuretssetup, run function build_non_resampled_data') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		if nbslice > 0:
			...
			for i in range(nbslice):
				slice = setupstructuretssetup(self.base_path)
				slice.setExpIdx(self.idx)
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type setupstructuretssetupObj, run function putTimedElt') 
		cpopath = cpopath + '/' 
		return obj

	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type setupstructuretssetupObj, run function getTimedElt') 
		cpopath = cpopath + '/' 

	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type setupstructuretssetupObj, run function putNonTimedElt') 
		cpopath = cpopath + '/' 
	...
	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type setupstructuretssetupObj, run function getNonTimedElt') 
		cpopath = cpopath + '/' 
...
class positionstructurerzphi1D(KeepInOrder):
	'''
...
class measurestructuretsmeasure(KeepInOrder):
	'''
	class measurestructuretsmeasure
	Measured values

	Attributes:
	- te : class testructureexp1D
	   Electron temperature [eV]. Vector (nchords)
	- ne : class nestructureexp1D
	   Electron density [m^-3]. Vector (nchords)
	'''
	...
	def __init__(self, base_path_in='measure'):
		...
		self.ne = nestructureexp1D('ne')

	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class measurestructuretsmeasure\n'
	...
	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type measurestructuretsmeasure, run function putSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type measurestructuretsmeasure, run function replaceLastSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type measurestructuretsmeasure, run function putNonTimed') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type measurestructuretsmeasure, run function getSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type measurestructuretsmeasure, run function build_non_resampled_data') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		if nbslice > 0:
			...
			for i in range(nbslice):
				slice = measurestructuretsmeasure(self.base_path)
				slice.setExpIdx(self.idx)
				slice.te = teList[i]
				slice.ne = neList[i]
				list.append(slice)
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type measurestructuretsmeasureObj, run function putTimedElt') 
		cpopath = cpopath + '/' 
	...
	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type measurestructuretsmeasureObj, run function getTimedElt') 
		cpopath = cpopath + '/' 
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type measurestructuretsmeasureObj, run function putNonTimedElt') 
		cpopath = cpopath + '/' 
	...
	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type measurestructuretsmeasureObj, run function getNonTimedElt') 
		cpopath = cpopath + '/' 
...
class nestructureexp1D(KeepInOrder):
	'''
	class nestructureexp1D
	Electron density [m^-3]. Vector (nchords)
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class nestructureexp1D\n'
		s = self.value.__str__()
		ret = ret + space + 'Attribute value\n' + space + s.replace('\n', '\n'+space) + '\n'
		s = self.abserror.__str__()
		ret = ret + space + 'Attribute abserror\n' + space + s.replace('\n', '\n'+space) + '\n'
	...
	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type nestructureexp1D, run function putSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type nestructureexp1D, run function replaceLastSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type nestructureexp1D, run function putNonTimed') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type nestructureexp1D, run function getSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type nestructureexp1D, run function build_non_resampled_data') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		if nbslice > 0:
			...
			check_status(status)
			for i in range(nbslice):
				slice = nestructureexp1D(self.base_path)
				slice.setExpIdx(self.idx)
				slice.value = valueList[:,i]
				slice.abserror = abserrorList[:,i]
				slice.relerror = relerrorList[:,i]
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type nestructureexp1DObj, run function putTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
			print ('putVect1DDoubleInObject : ' + cpopath + 'value') 
			print ('obj = ' + str(obj))
	...
	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type nestructureexp1DObj, run function getTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
			print ('getVect1DDoubleInObject : ' + cpopath + 'value') 
			print ('obj = ' + str(obj))
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type nestructureexp1DObj, run function putNonTimedElt') 
		cpopath = cpopath + '/' 
		return obj

	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type nestructureexp1DObj, run function getNonTimedElt') 
		cpopath = cpopath + '/' 
