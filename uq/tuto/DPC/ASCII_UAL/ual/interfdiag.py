...
class interfdiagArray:
	'''
	class interfdiagArray
	Interferometry; measures line-integrated electron density [m^-2]; Time-dependent CPO

	Attributes:
	- array : list of interfdiag
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class interfdiagArray nb_cpos=%d\n' %(len(self.array))
		for i in range(len(self.array)):
			ret = ret + space + 'interfdiag cpos=%d\n' %(i) + self.array[i].__str__(depth+1)
		return ret
