...
class corefast(KeepInOrder):
	...
	- values : class valuesstruct_arraycorefast_values: array of valuesstruct_arraycorefast_valuesObj objects
	   Description of the fast particle terms of various origins. Array of structure (nfast). Time-dependent.
...
class corefastArray:
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class corefastArray nb_cpos=%d\n' %(len(self.array))
		for i in range(len(self.array)):
			ret = ret + space + 'corefast cpos=%d\n' %(i) + self.array[i].__str__(depth+1)
		return ret
...
class valuesstruct_arraycorefast_values:
	'''
	class valuesstruct_arraycorefast_values
	Description of the fast particle terms of various origins. Array of structure (nfast). Time-dependent.

	Attributes:
	- array : list of valuesstruct_arraycorefast_valuesObj 
	'''
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class valuesstruct_arraycorefast_values\n'
		for i in range(len(self.array)):
			ret = ret + space + 'valuesstruct_arraycorefast_values[%d] = \n' %(i) + self.array[i].__str__(depth+1)
		return ret
	...
	def resize(self, nbelt):
		self.array = []
		for i in range(nbelt):
			self.array.append(valuesstruct_arraycorefast_valuesObj(self.base_path))
			self.array[i].setExpIdx(self.idx)

	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type valuesstruct_arraycorefast_values, run function putSlice') 
		if (dev()):
	...
	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type valuesstruct_arraycorefast_values, run function replaceLastSlice') 
		if (dev()):
			print ('beginObject(None, 0, '+path+'/'+cpopath+self.base_path+', TIMED)')
	...
	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type valuesstruct_arraycorefast_values, run function putNonTimed') 
		if (dev()):
			print ('beginObject(None, 0, '+path+'/'+cpopath+self.base_path+', NON_TIMED)')
		obj = ull.beginObject(self.idx, None, 0, path + '/' + cpopath + self.base_path, NON_TIMED)
		for i in range(len(self.array)):
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type valuesstruct_arraycorefast_values, run function getSlice') 
		if (dev()):
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type valuesstruct_arraycorefast_values, run function build_non_resampled_data') 
		list=[]
		if nbslice > 0:
			if (dev()):
			...
			if status:
				...
				for i in range(nbslice):
					list.append(valuesstruct_arraycorefast_values(self.base_path))
				return list
			for n in range(nbslice):
				...
				if status:
					...
					continue
				obj_size = ull.getObjectDim(self.idx, obj)
				slice = valuesstruct_arraycorefast_values(self.base_path)
				slice.setExpIdx(self.idx)
				slice.resize(obj_size)
				for i in range(obj_size):
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type valuesstruct_arraycorefast_values, run function putTimedElt') 
		if (dev()):
			print ('beginObject idx=%d' %(self.idx)) 
		if (dev()):
	...
	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type valuesstruct_arraycorefast_values, run function getTimedElt') 
		if (dev()):
			print ('getObjectFromObject('+str(obj)+', '+cpopath+', '+str(i)+')')
		status, obj2 = ull.getObjectFromObject(self.idx, obj, cpopath, i)
		if status:
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type valuesstruct_arraycorefast_values, run function putNonTimedElt') 
		if (dev()):
			print ('beginObject idx=%d' %(self.idx)) 
		if (dev()):
			print ('beginObject('+str(obj)+', '+str(i)+', '+path+'/'+cpopath+', NON_TIMED)')
	...
	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type valuesstruct_arraycorefast_values, run function getNonTimedElt') 
		if (dev()):
			print ('getObjectFromObject('+str(obj)+', '+cpopath+', '+str(i)+')')
		status, obj2 = ull.getObjectFromObject(self.idx, obj, cpopath, i)
		if status:
...
class valuesstruct_arraycorefast_valuesObj(KeepInOrder):
	'''
	class valuesstruct_arraycorefast_valuesObj
	Description of the fast particle terms of various origins. Array of structure (nfast). Time-dependent.

	Attributes:
	- fastid : class fastidstructureidentifier
	   Identifier for the origin of the non-thermal contributions (see fast_particle_origin_identifier in the Documentation website under Conventions/Enumerated_datatypes).  Time-dependent.
	- filter : class filterstructurefast_thermal_separation_filter
	   Description of how the fast and the thermal particle populations were separated. Time-dependent.
	...
	- rho_tor_norm : numpy.ndarray 1D with float
	   Normalised toroidal flux coordinate values (= rho_tor normalised to the value at the last grid point). Vector (nrho). Time-dependent.
	- psi : numpy.ndarray 1D with float
	   Poloidal flux at the grid points for 1D profiles [Wb], without 1/2pi and such that Bp=|grad psi| /R/2/pi. Vector (nrho). Time-dependent.
	- volume : numpy.ndarray 1D with float
	   Volume enclosed in the flux surface [m^3]. Vector (nrho). Time-dependent.
	- area : numpy.ndarray 1D with float
	   Cross-sectional area of the flux surface [m^2]. Vector (nrho). Time-dependent.
	- j : numpy.ndarray 1D with float
	   Non thermal current, = average(j.B) / B0, where B0 = corefast/toroid_field/b0 [A.m^-2]. Vector(nrho). Time-dependent.
	- sigma : numpy.ndarray 1D with float
	   Non-thermal induced parallel conductivity [ohm^-1.m^-1]. EXACT DEFINITION PENDING. Vector(nrho). Time-dependent.
	- ni : numpy.ndarray 2D with float
	   Non-thermal ion density [m^-3]. Matrix(nrho,nions). Time-dependent.
	- ne : numpy.ndarray 1D with float
	   Non-thermal electron density [m^-3]. Vector(nrho). Time-dependent.
	- nz : numpy.ndarray 2D with float
	   Non-thermal impurity density [m^-3]. Matrix(nrho,nimpur). Time-dependent.
	- pi : numpy.ndarray 2D with float
	   Non-thermal ion pressure; the flux surface average of the m*v^2/3 moment of the fast particle distribution function [Pa]. Matrix(nrho,nions). Time-dependent.
	- pe : numpy.ndarray 1D with float
	   Non-thermal electron pressure; the flux surface average of the m*v^2/3 moment of the fast particle distribution function [Pa]. Vector(nrho). Time-dependent.
	- pz : numpy.ndarray 2D with float
	   Non-thermal impurity total pressure; the flux surface average of the m*v^2/3 moment of the fast particle distribution function [Pa]. Matrix(nrho,nimpur). Time-dependent.
	- pi_para : numpy.ndarray 2D with float
	   Non-thermal ion parallel pressure; the flux surface average of the m*v_parallel^2 moment of the fast particle distribution function [Pa]. Matrix(nrho,nions). Time-dependent.
	- pe_para : numpy.ndarray 1D with float
	   Non-thermal electron parallel pressure; the flux surface average of the m*v_parallel^2 moment of the fast particle distribution function [Pa]. Vector(nrho). Time-dependent.
	- pz_para : numpy.ndarray 2D with float
	   Non-thermal impurity parallel pressure; the flux surface average of the m*v_parallel^2 moment of the fast particle distribution function [Pa]. Matrix(nrho,nimpur). Time-dependent.
	- ui : numpy.ndarray 2D with float
	   Non-thermal ion toroidal velocity [m.s^-1]. Matrix(nrho,nions). Time-dependent.
	- uz : numpy.ndarray 2D with float
	   Non-thermal impurity toroidal velocity [m.s^-1]. Matrix(nrho,nimpur). Time-dependent.
	- codeparam : class codeparamstructurecodeparam
	   Code parameters
	'''
	...
	def __init__(self, base_path_in='values'):
		...
		self.idx = EMPTY_INT
		self.cpoTime = EMPTY_DOUBLE
		self.fastid = fastidstructureidentifier('fastid')
		self.filter = filterstructurefast_thermal_separation_filter('filter')
		...
		self.pi = numpy.zeros((0,0), numpy.float64, order='C')
		self.pe = numpy.zeros(0, numpy.float64, order='C')
		self.pz = numpy.zeros((0,0), numpy.float64, order='C')
		self.pi_para = numpy.zeros((0,0), numpy.float64, order='C')
		self.pe_para = numpy.zeros(0, numpy.float64, order='C')
		self.pz_para = numpy.zeros((0,0), numpy.float64, order='C')
		self.ui = numpy.zeros((0,0), numpy.float64, order='C')
		self.uz = numpy.zeros((0,0), numpy.float64, order='C')
		self.codeparam = codeparamstructurecodeparam('codeparam')

	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class valuesstruct_arraycorefast_valuesObj\n'
		ret = ret + space + 'Attribute fastid\n ' + self.fastid.__str__(depth+1)
		...
		s = self.pz.__str__()
		ret = ret + space + 'Attribute pz\n' + space + s.replace('\n', '\n'+space) + '\n'
		s = self.pi_para.__str__()
		ret = ret + space + 'Attribute pi_para\n' + space + s.replace('\n', '\n'+space) + '\n'
		s = self.pe_para.__str__()
		ret = ret + space + 'Attribute pe_para\n' + space + s.replace('\n', '\n'+space) + '\n'
		s = self.pz_para.__str__()
		ret = ret + space + 'Attribute pz_para\n' + space + s.replace('\n', '\n'+space) + '\n'
		s = self.ui.__str__()
		ret = ret + space + 'Attribute ui\n' + space + s.replace('\n', '\n'+space) + '\n'
		s = self.uz.__str__()
		ret = ret + space + 'Attribute uz\n' + space + s.replace('\n', '\n'+space) + '\n'
		ret = ret + space + 'Attribute codeparam\n ' + self.codeparam.__str__(depth+1)
		return ret

	def setExpIdx(self, idx):
		self.idx = idx
		self.fastid.setExpIdx(idx)
		self.filter.setExpIdx(idx)
		self.codeparam.setExpIdx(idx)

	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type valuesstruct_arraycorefast_valuesObj, run function putTimedElt') 
		cpopath = cpopath + '/'
		obj = self.filter.putTimedElt(path, cpopath + 'filter', i, obj)
		if (dev()):
		...
		if (dev()):
			print ('putVect2DDoubleInObject : ' + cpopath + 'pi') 
			print ('obj = ' + str(obj))
		obj = ull.putVect2DDoubleInObject(self.idx, obj, cpopath + 'pi', i, numpy.array(self.pi).astype(numpy.float64))
		if (dev()):
			print ('putVect1DDoubleInObject : ' + cpopath + 'pe') 
			print ('obj = ' + str(obj))
		obj = ull.putVect1DDoubleInObject(self.idx, obj, cpopath + 'pe', i, numpy.array(self.pe).astype(numpy.float64))
		if (dev()):
			print ('putVect2DDoubleInObject : ' + cpopath + 'pz') 
			print ('obj = ' + str(obj))
		obj = ull.putVect2DDoubleInObject(self.idx, obj, cpopath + 'pz', i, numpy.array(self.pz).astype(numpy.float64))
		if (dev()):
			print ('putVect2DDoubleInObject : ' + cpopath + 'pi_para') 
			print ('obj = ' + str(obj))
		obj = ull.putVect2DDoubleInObject(self.idx, obj, cpopath + 'pi_para', i, numpy.array(self.pi_para).astype(numpy.float64))
		if (dev()):
			print ('putVect1DDoubleInObject : ' + cpopath + 'pe_para') 
			print ('obj = ' + str(obj))
		obj = ull.putVect1DDoubleInObject(self.idx, obj, cpopath + 'pe_para', i, numpy.array(self.pe_para).astype(numpy.float64))
		if (dev()):
			print ('putVect2DDoubleInObject : ' + cpopath + 'pz_para') 
			print ('obj = ' + str(obj))
		obj = ull.putVect2DDoubleInObject(self.idx, obj, cpopath + 'pz_para', i, numpy.array(self.pz_para).astype(numpy.float64))
		if (dev()):
		...
		if (dev()):
			print ('putVect2DDoubleInObject : ' + cpopath + 'uz') 
			print ('obj = ' + str(obj))
		obj = ull.putVect2DDoubleInObject(self.idx, obj, cpopath + 'uz', i, numpy.array(self.uz).astype(numpy.float64))
		obj = self.codeparam.putTimedElt(path, cpopath + 'codeparam', i, obj)
		return obj

	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type valuesstruct_arraycorefast_valuesObj, run function getTimedElt') 
		cpopath = cpopath + '/'
		self.filter.getTimedElt(path, cpopath + 'filter', i, obj)
		if (dev()):
		...
		if not status:
			self.nz = ret_nz
		if (dev()):
			print ('getVect2DDoubleInObject : ' + cpopath + 'pi') 
			print ('obj = ' + str(obj))
		status, ret_pi = ull.getVect2DDoubleFromObject(self.idx, obj, cpopath + 'pi', i)
		check_status(status)
		if not status:
			self.pi = ret_pi
		if (dev()):
			print ('getVect1DDoubleInObject : ' + cpopath + 'pe') 
			print ('obj = ' + str(obj))
		status, ret_pe = ull.getVect1DDoubleFromObject(self.idx, obj, cpopath + 'pe', i)
		check_status(status)
		if not status:
			self.pe = ret_pe
		if (dev()):
			print ('getVect2DDoubleInObject : ' + cpopath + 'pz') 
			print ('obj = ' + str(obj))
		status, ret_pz = ull.getVect2DDoubleFromObject(self.idx, obj, cpopath + 'pz', i)
		check_status(status)
		if not status:
			self.pz = ret_pz
		if (dev()):
			print ('getVect2DDoubleInObject : ' + cpopath + 'pi_para') 
			print ('obj = ' + str(obj))
		status, ret_pi_para = ull.getVect2DDoubleFromObject(self.idx, obj, cpopath + 'pi_para', i)
		check_status(status)
		if not status:
			self.pi_para = ret_pi_para
		if (dev()):
			print ('getVect1DDoubleInObject : ' + cpopath + 'pe_para') 
			print ('obj = ' + str(obj))
		status, ret_pe_para = ull.getVect1DDoubleFromObject(self.idx, obj, cpopath + 'pe_para', i)
		check_status(status)
		if not status:
			self.pe_para = ret_pe_para
		if (dev()):
			print ('getVect2DDoubleInObject : ' + cpopath + 'pz_para') 
			print ('obj = ' + str(obj))
		status, ret_pz_para = ull.getVect2DDoubleFromObject(self.idx, obj, cpopath + 'pz_para', i)
		check_status(status)
		if not status:
			self.pz_para = ret_pz_para
		if (dev()):
		...
		if not status:
			self.ui = ret_ui
		if (dev()):
			print ('getVect2DDoubleInObject : ' + cpopath + 'uz') 
			print ('obj = ' + str(obj))
		status, ret_uz = ull.getVect2DDoubleFromObject(self.idx, obj, cpopath + 'uz', i)
		check_status(status)
		if not status:
			self.uz = ret_uz
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type valuesstruct_arraycorefast_valuesObj, run function putNonTimedElt') 
		cpopath = cpopath + '/'
		obj = self.fastid.putNonTimedElt(path, cpopath + 'fastid', i, obj)
	...
	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type valuesstruct_arraycorefast_valuesObj, run function getNonTimedElt') 
		cpopath = cpopath + '/'
		self.fastid.getNonTimedElt(path, cpopath + 'fastid', i, obj)
		self.filter.getNonTimedElt(path, cpopath + 'filter', i, obj)
		self.codeparam.getNonTimedElt(path, cpopath + 'codeparam', i, obj)


class fastidstructureidentifier(KeepInOrder):
	'''
	class fastidstructureidentifier
	Identifier for the origin of the non-thermal contributions (see fast_particle_origin_identifier in the Documentation website under Conventions/Enumerated_datatypes).  Time-dependent.
	...
	def __init__(self, base_path_in='fastid'):
		self.base_path = base_path_in
		self.idx = EMPTY_INT
		self.cpoTime = EMPTY_DOUBLE
		self.id = ''
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class fastidstructureidentifier\n'
		ret = ret + space + 'Attribute id: ' + str(self.id) + '\n'
		ret = ret + space + 'Attribute flag: ' + str(self.flag) + '\n'
		ret = ret + space + 'Attribute description: ' + str(self.description) + '\n'
		return ret
	...
	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type fastidstructureidentifier, run function putSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type fastidstructureidentifier, run function replaceLastSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type fastidstructureidentifier, run function putNonTimed') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type fastidstructureidentifier, run function getSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type fastidstructureidentifier, run function build_non_resampled_data') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		if nbslice > 0:
			...
			check_status(status)
			for i in range(nbslice):
				slice = fastidstructureidentifier(self.base_path)
				slice.setExpIdx(self.idx)
				slice.id = idVal
				slice.flag = flagVal
				slice.description = descriptionVal
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type fastidstructureidentifierObj, run function putTimedElt') 
		cpopath = cpopath + '/' 
		return obj

	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type fastidstructureidentifierObj, run function getTimedElt') 
		cpopath = cpopath + '/' 

	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type fastidstructureidentifierObj, run function putNonTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
			print ('putStringInObject : ' + cpopath + 'id') 
			print ('obj = ' + str(obj))
	...
	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type fastidstructureidentifierObj, run function getNonTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
			print ('getStringInObject : ' + cpopath + 'id') 
			print ('obj = ' + str(obj))
...
class filterstructurefast_thermal_separation_filter(KeepInOrder):
	'''
	class filterstructurefast_thermal_separation_filter
	Description of how the fast and the thermal particle populations were separated. Time-dependent.
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class filterstructurefast_thermal_separation_filter\n'
	...
	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type filterstructurefast_thermal_separation_filter, run function putSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type filterstructurefast_thermal_separation_filter, run function replaceLastSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type filterstructurefast_thermal_separation_filter, run function putNonTimed') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type filterstructurefast_thermal_separation_filter, run function getSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type filterstructurefast_thermal_separation_filter, run function build_non_resampled_data') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		if nbslice > 0:
			...
			check_status(status)
			for i in range(nbslice):
				slice = filterstructurefast_thermal_separation_filter(self.base_path)
				slice.setExpIdx(self.idx)
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type filterstructurefast_thermal_separation_filterObj, run function putTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
	...
	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type filterstructurefast_thermal_separation_filterObj, run function getTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type filterstructurefast_thermal_separation_filterObj, run function putNonTimedElt') 
		cpopath = cpopath + '/' 
	...
	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type filterstructurefast_thermal_separation_filterObj, run function getNonTimedElt') 
		cpopath = cpopath + '/' 
...
class methodstructureidentifier(KeepInOrder):
	'''
