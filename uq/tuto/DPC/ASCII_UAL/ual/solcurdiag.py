...
class solcurdiag(KeepInOrder):
	...
	- sol_current : class sol_currentstruct_arraysolcurdiag_sol_current: array of sol_currentstruct_arraysolcurdiag_sol_currentObj objects
	   Vector of toroidal rings of divertor tiles.  Structure array(nrings). Time-dependent
	- clusters : class clustersstruct_arrayclusters: array of clustersstruct_arrayclustersObj objects
	...
	def __init__(self):
		self.base_path = 'solcurdiag'
		self.idx = EMPTY_INT
		self.cpoTime = EMPTY_DOUBLE
		self.maxOccurrences = 3
		self.datainfo = datainfostructuredatainfo('datainfo')
		self.sol_current = sol_currentstruct_arraysolcurdiag_sol_current('sol_current')
		self.clusters = clustersstruct_arrayclusters('clusters')
		self.time = EMPTY_DOUBLE
		self.codeparam = codeparamstructurecodeparam('codeparam')

	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class solcurdiag\n'
		ret = ret + space + 'Attribute datainfo\n ' + self.datainfo.__str__(depth+1)
		ret = ret + space + 'Attribute sol_current\n ' + self.sol_current.__str__(depth+1)
		ret = ret + space + 'Attribute clusters\n ' + self.clusters.__str__(depth+1)
	...
	def setExpIdx(self, idx):
		...
		self.sol_current.setExpIdx(idx)
		self.clusters.setExpIdx(idx)
	...
	def putSlice(self, occurrence=0):
		...
		self.sol_current.cpoTime = self.cpoTime
		self.sol_current.putSlice(path, cpopath)
		self.clusters.cpoTime = self.cpoTime
		self.clusters.putSlice(path, cpopath)
	...
	def replaceLastSlice(self, occurrence=0):
		...
		self.sol_current.replaceLastSlice(path, cpopath)
		self.clusters.replaceLastSlice(path, cpopath)
	...
	def putNonTimed(self, occurrence=0):
		...
		self.sol_current.putNonTimed(path, cpopath)
		self.clusters.putNonTimed(path, cpopath)
	...
	def getSlice(self, inTime, interpolMode, occurrence=0):
		...
		self.sol_current.getSlice(path, cpopath, inTime, interpolMode)
		self.clusters.getSlice(path, cpopath, inTime, interpolMode)
	...
	def build_non_resampled_data(self, occurrence=0):
		...
		if nbslice > 0:
			...
			sol_currentList = self.sol_current.build_non_resampled_data(path, cpopath, nbslice)
			clustersList = self.clusters.build_non_resampled_data(path, cpopath, nbslice)
			...
			for i in range(nbslice):
				...
				slice.sol_current = sol_currentList[i]
				slice.clusters = clustersList[i]
	...
	def deleteData(self, occurrence=0):
		...
		ull.deleteData(self.idx, path, cpopath + 'sol_current')
		ull.deleteData(self.idx, path, cpopath + 'clusters')
		ull.deleteData(self.idx, path, cpopath + 'time')
		self.codeparam.deleteData(path, cpopath)


class solcurdiagArray:
	'''
	class solcurdiagArray
	SOL current diagnostic. Time-dependent CPO

	Attributes:
	- array : list of solcurdiag
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class solcurdiagArray nb_cpos=%d\n' %(len(self.array))
		for i in range(len(self.array)):
			ret = ret + space + 'solcurdiag cpos=%d\n' %(i) + self.array[i].__str__(depth+1)
		return ret
...
class sol_currentstruct_arraysolcurdiag_sol_current:
	'''
	class sol_currentstruct_arraysolcurdiag_sol_current
	Vector of toroidal rings of divertor tiles.  Structure array(nrings). Time-dependent

	Attributes:
	- array : list of sol_currentstruct_arraysolcurdiag_sol_currentObj 
	'''

	def __init__(self, base_path_in='sol_current'):
		self.base_path = base_path_in
		self.idx = EMPTY_INT
		self.cpoTime = EMPTY_DOUBLE
		self.array = []
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class sol_currentstruct_arraysolcurdiag_sol_current\n'
		for i in range(len(self.array)):
			ret = ret + space + 'sol_currentstruct_arraysolcurdiag_sol_current[%d] = \n' %(i) + self.array[i].__str__(depth+1)
		return ret
	...
	def resize(self, nbelt):
		...
		for i in range(nbelt):
			self.array.append(sol_currentstruct_arraysolcurdiag_sol_currentObj(self.base_path))
			self.array[i].setExpIdx(self.idx)

	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type sol_currentstruct_arraysolcurdiag_sol_current, run function putSlice') 
		if (dev()):
	...
	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type sol_currentstruct_arraysolcurdiag_sol_current, run function replaceLastSlice') 
		if (dev()):
			print ('beginObject(None, 0, '+path+'/'+cpopath+self.base_path+', TIMED)')
	...
	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type sol_currentstruct_arraysolcurdiag_sol_current, run function putNonTimed') 
		if (dev()):
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type sol_currentstruct_arraysolcurdiag_sol_current, run function getSlice') 
		if (dev()):
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type sol_currentstruct_arraysolcurdiag_sol_current, run function build_non_resampled_data') 
		list=[]
		if nbslice > 0:
			if (dev()):
			...
			if status:
				...
				for i in range(nbslice):
					list.append(sol_currentstruct_arraysolcurdiag_sol_current(self.base_path))
				return list
			for n in range(nbslice):
				...
				if status:
					...
					continue
				obj_size = ull.getObjectDim(self.idx, obj)
				slice = sol_currentstruct_arraysolcurdiag_sol_current(self.base_path)
				slice.setExpIdx(self.idx)
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type sol_currentstruct_arraysolcurdiag_sol_current, run function putTimedElt') 
		if (dev()):
	...
	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type sol_currentstruct_arraysolcurdiag_sol_current, run function getTimedElt') 
		if (dev()):
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type sol_currentstruct_arraysolcurdiag_sol_current, run function putNonTimedElt') 
		if (dev()):
			print ('beginObject idx=%d' %(self.idx)) 
		if (dev()):
	...
	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type sol_currentstruct_arraysolcurdiag_sol_current, run function getNonTimedElt') 
		if (dev()):
			print ('getObjectFromObject('+str(obj)+', '+cpopath+', '+str(i)+')')
		status, obj2 = ull.getObjectFromObject(self.idx, obj, cpopath, i)
		if status:
...
class sol_currentstruct_arraysolcurdiag_sol_currentObj(KeepInOrder):
	'''
	class sol_currentstruct_arraysolcurdiag_sol_currentObj
	Vector of toroidal rings of divertor tiles.  Structure array(nrings). Time-dependent

	Attributes:
	- setup : class setupstructuresolcurdiag_sol_current_setup
	   diagnostic setup information
	- measure : class measurestructureexp0D
	...
	def __init__(self, base_path_in='sol_current'):
		...
		self.idx = EMPTY_INT
		self.cpoTime = EMPTY_DOUBLE
		self.setup = setupstructuresolcurdiag_sol_current_setup('setup')
		self.measure = measurestructureexp0D('measure')

	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class sol_currentstruct_arraysolcurdiag_sol_currentObj\n'
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type sol_currentstruct_arraysolcurdiag_sol_currentObj, run function putTimedElt') 
		cpopath = cpopath + '/'
	...
	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type sol_currentstruct_arraysolcurdiag_sol_currentObj, run function getTimedElt') 
		cpopath = cpopath + '/'
		self.measure.getTimedElt(path, cpopath + 'measure', i, obj)

	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type sol_currentstruct_arraysolcurdiag_sol_currentObj, run function putNonTimedElt') 
		cpopath = cpopath + '/'
	...
	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type sol_currentstruct_arraysolcurdiag_sol_currentObj, run function getNonTimedElt') 
		cpopath = cpopath + '/'
		self.setup.getNonTimedElt(path, cpopath + 'setup', i, obj)
		self.measure.getNonTimedElt(path, cpopath + 'measure', i, obj)


class setupstructuresolcurdiag_sol_current_setup(KeepInOrder):
	'''
	class setupstructuresolcurdiag_sol_current_setup
	diagnostic setup information
	...
	def __init__(self, base_path_in='setup'):
		...
		self.tiles_turn = EMPTY_INT

	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class setupstructuresolcurdiag_sol_current_setup\n'
		ret = ret + space + 'Attribute name: ' + str(self.name) + '\n'
		ret = ret + space + 'Attribute id: ' + str(self.id) + '\n'
		ret = ret + space + 'Attribute position\n ' + self.position.__str__(depth+1)
		ret = ret + space + 'Attribute tiles_turn: ' + str(self.tiles_turn) + '\n'
		return ret
	...
	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type setupstructuresolcurdiag_sol_current_setup, run function putSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type setupstructuresolcurdiag_sol_current_setup, run function replaceLastSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type setupstructuresolcurdiag_sol_current_setup, run function putNonTimed') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		status = ull.putInt(self.idx, path, cpopath + 'tiles_turn', self.tiles_turn)
		check_status(status)

	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type setupstructuresolcurdiag_sol_current_setup, run function getSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		status, ret_tiles_turn = ull.getInt(self.idx, path, cpopath + 'tiles_turn')
		check_status(status)
		if not status:
			self.tiles_turn = ret_tiles_turn

	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type setupstructuresolcurdiag_sol_current_setup, run function build_non_resampled_data') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		if nbslice > 0:
			...
			status, tiles_turnVal = ull.getInt(self.idx, path, cpopath + 'tiles_turn')
			check_status(status)
			for i in range(nbslice):
				slice = setupstructuresolcurdiag_sol_current_setup(self.base_path)
				slice.setExpIdx(self.idx)
				slice.name = nameVal
				slice.id = idVal
				slice.position = positionList[i]
				slice.tiles_turn = tiles_turnVal
				list.append(slice)
		else:
			print ('error, nbslice must be > 0 and got:' + str(nbslice))
		return list
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type setupstructuresolcurdiag_sol_current_setupObj, run function putTimedElt') 
		cpopath = cpopath + '/' 
		return obj

	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type setupstructuresolcurdiag_sol_current_setupObj, run function getTimedElt') 
		cpopath = cpopath + '/' 

	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type setupstructuresolcurdiag_sol_current_setupObj, run function putNonTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
		...
		if (dev()):
			print ('putIntInObject : ' + cpopath + 'tiles_turn') 
			print ('obj = ' + str(obj))
		obj = ull.putIntInObject(self.idx, obj, cpopath + 'tiles_turn', i, self.tiles_turn)
		return obj

	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type setupstructuresolcurdiag_sol_current_setupObj, run function getNonTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
		...
		if (dev()):
			print ('getIntInObject : ' + cpopath + 'tiles_turn') 
			print ('obj = ' + str(obj))
		status, ret_tiles_turn = ull.getIntFromObject(self.idx, obj, cpopath + 'tiles_turn', i)
		check_status(status)
		if not status:
			self.tiles_turn = ret_tiles_turn
	...
	def deleteData(self, path, cpopath):
		...
		ull.deleteData(self.idx, path, cpopath + 'tiles_turn')
...
class measurestructureexp0D(KeepInOrder):
	'''
	class measurestructureexp0D
	Measured value for the current through the toroidal ring of tiles [A]; Time-dependent; Scalar
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class measurestructureexp0D\n'
	...
	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type measurestructureexp0D, run function putSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type measurestructureexp0D, run function replaceLastSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type measurestructureexp0D, run function putNonTimed') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type measurestructureexp0D, run function getSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type measurestructureexp0D, run function build_non_resampled_data') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		if nbslice > 0:
			...
			check_status(status)
			for i in range(nbslice):
				slice = measurestructureexp0D(self.base_path)
				slice.setExpIdx(self.idx)
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type measurestructureexp0DObj, run function putTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
	...
	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type measurestructureexp0DObj, run function getTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type measurestructureexp0DObj, run function putNonTimedElt') 
		cpopath = cpopath + '/' 
		return obj

	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type measurestructureexp0DObj, run function getNonTimedElt') 
		cpopath = cpopath + '/' 
...
class clustersstruct_arrayclusters:
	'''
	class clustersstruct_arrayclusters
	Cluster of tile rings to define and reference superset structures using the individual tile rings. A coil ring can coexist on two top level structures. Structure array (ncluster).

	Attributes:
	- array : list of clustersstruct_arrayclustersObj 
	'''

	def __init__(self, base_path_in='clusters'):
		self.base_path = base_path_in
		self.idx = EMPTY_INT
		self.cpoTime = EMPTY_DOUBLE
		self.array = []
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class clustersstruct_arrayclusters\n'
		for i in range(len(self.array)):
			ret = ret + space + 'clustersstruct_arrayclusters[%d] = \n' %(i) + self.array[i].__str__(depth+1)
		return ret
	...
	def resize(self, nbelt):
		self.array = []
		for i in range(nbelt):
			self.array.append(clustersstruct_arrayclustersObj(self.base_path))
			self.array[i].setExpIdx(self.idx)

	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type clustersstruct_arrayclusters, run function putSlice') 

	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type clustersstruct_arrayclusters, run function replaceLastSlice') 

	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type clustersstruct_arrayclusters, run function putNonTimed') 
		if (dev()):
			print ('beginObject(None, 0, '+path+'/'+cpopath+self.base_path+', NON_TIMED)')
		obj = ull.beginObject(self.idx, None, 0, path + '/' + cpopath + self.base_path, NON_TIMED)
		for i in range(len(self.array)):
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type clustersstruct_arrayclusters, run function getSlice') 
		if (dev()):
			print ('getObject('+path+', '+cpopath+self.base_path+', NON_TIMED)')
		status, obj = ull.getObject(self.idx, path, cpopath + self.base_path, NON_TIMED)
		if status:
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type clustersstruct_arrayclusters, run function build_non_resampled_data') 
		list=[]
		if nbslice > 0:
			if (dev()):
				print ('getObject('+path+', '+cpopath+self.base_path+', NON_TIMED)')
			...
			if status:
				print ('Failed to get data: ' + path + '/' + cpopath + self.base_path + ' (NON_TIMED)')
				for i in range(nbslice):
					list.append(clustersstruct_arrayclusters(self.base_path))
				return list
			obj_size = ull.getObjectDim(self.idx, obj)
			for n in range(nbslice):
				slice = clustersstruct_arrayclusters(self.base_path)
				slice.setExpIdx(self.idx)
				slice.resize(obj_size)
				for i in range(obj_size):
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type clustersstruct_arrayclusters, run function putNonTimedElt') 
		if (dev()):
			print ('beginObject idx=%d' %(self.idx)) 
		if (dev()):
			print ('beginObject('+str(obj)+', '+str(i)+', '+path+'/'+cpopath+', NON_TIMED)')
	...
	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type clustersstruct_arrayclusters, run function getNonTimedElt') 
		if (dev()):
			print ('getObjectFromObject('+str(obj)+', '+cpopath+', '+str(i)+')')
		status, obj2 = ull.getObjectFromObject(self.idx, obj, cpopath, i)
		if status:
...
class clustersstruct_arrayclustersObj(KeepInOrder):
	'''
	class clustersstruct_arrayclustersObj
	Cluster of tile rings to define and reference superset structures using the individual tile rings. A coil ring can coexist on two top level structures. Structure array (ncluster).
	...
	def __init__(self, base_path_in='clusters'):
		...
		self.cpoTime = EMPTY_DOUBLE
		self.name = ''
		self.start = EMPTY_INT
		self.finish = EMPTY_INT

	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class clustersstruct_arrayclustersObj\n'
		ret = ret + space + 'Attribute name: ' + str(self.name) + '\n'
		ret = ret + space + 'Attribute start: ' + str(self.start) + '\n'
		ret = ret + space + 'Attribute finish: ' + str(self.finish) + '\n'
		return ret
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type clustersstruct_arrayclustersObj, run function putNonTimedElt') 
		cpopath = cpopath + '/'
		if (dev()):
			print ('putStringInObject : ' + cpopath + 'name') 
			print ('obj = ' + str(obj))
		obj = ull.putStringInObject(self.idx, obj, cpopath + 'name', i, self.name)
		if (dev()):
			print ('putIntInObject : ' + cpopath + 'start') 
			print ('obj = ' + str(obj))
		obj = ull.putIntInObject(self.idx, obj, cpopath + 'start', i, self.start)
		if (dev()):
			print ('putIntInObject : ' + cpopath + 'finish') 
			print ('obj = ' + str(obj))
		obj = ull.putIntInObject(self.idx, obj, cpopath + 'finish', i, self.finish)
		return obj

	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type clustersstruct_arrayclustersObj, run function getNonTimedElt') 
		cpopath = cpopath + '/'
		if (dev()):
			print ('getStringInObject : ' + cpopath + 'name') 
			print ('obj = ' + str(obj))
		...
		if not status:
			self.name = ret_name
		if (dev()):
			print ('getIntInObject : ' + cpopath + 'start') 
			print ('obj = ' + str(obj))
		status, ret_start = ull.getIntFromObject(self.idx, obj, cpopath + 'start', i)
		check_status(status)
		if not status:
			self.start = ret_start
		if (dev()):
			print ('getIntInObject : ' + cpopath + 'finish') 
			print ('obj = ' + str(obj))
		status, ret_finish = ull.getIntFromObject(self.idx, obj, cpopath + 'finish', i)
		check_status(status)
		if not status:
			self.finish = ret_finish
