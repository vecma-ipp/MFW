...
class ecediag(KeepInOrder):
	...
	- setup : class setupstructureecesetup
	   diagnostic setup information
	- measure : class measurestructureecemeasure
	   Measured values
...
class ecediagArray:
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class ecediagArray nb_cpos=%d\n' %(len(self.array))
		for i in range(len(self.array)):
			ret = ret + space + 'ecediag cpos=%d\n' %(i) + self.array[i].__str__(depth+1)
		return ret
...
class setupstructureecesetup(KeepInOrder):
	'''
	class setupstructureecesetup
	diagnostic setup information

	Attributes:
	- frequency : numpy.ndarray 1D with float
	   Frequency of the ECE channels. Vector (nchannels)
	- los : class losstructuresetup_line_exp
	...
	def __init__(self, base_path_in='setup'):
		...
		self.los = losstructuresetup_line_exp('los')

	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class setupstructureecesetup\n'
		s = self.frequency.__str__()
		ret = ret + space + 'Attribute frequency\n' + space + s.replace('\n', '\n'+space) + '\n'
		ret = ret + space + 'Attribute los\n ' + self.los.__str__(depth+1)
		return ret

	def setExpIdx(self, idx):
		self.idx = idx
		self.los.setExpIdx(idx)

	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type setupstructureecesetup, run function putSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		if (verb()):
			print ('path = ' + path + '  and cpopath = ' + cpopath) 
		self.los.cpoTime = self.cpoTime
		self.los.putSlice(path, cpopath)

	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type setupstructureecesetup, run function replaceLastSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		if (verb()):
			print ('path = ' + path + '  and cpopath = ' + cpopath) 
		self.los.replaceLastSlice(path, cpopath)

	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type setupstructureecesetup, run function putNonTimed') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		check_status(status)
		self.los.putNonTimed(path, cpopath)

	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type setupstructureecesetup, run function getSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		self.los.getSlice(path, cpopath, inTime, interpolMode)

	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type setupstructureecesetup, run function build_non_resampled_data') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		if nbslice > 0:
			...
			check_status(status)
			losList = self.los.build_non_resampled_data(path, cpopath, nbslice)
			for i in range(nbslice):
				slice = setupstructureecesetup(self.base_path)
				slice.setExpIdx(self.idx)
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type setupstructureecesetupObj, run function putTimedElt') 
		cpopath = cpopath + '/' 
		return obj

	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type setupstructureecesetupObj, run function getTimedElt') 
		cpopath = cpopath + '/' 

	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type setupstructureecesetupObj, run function putNonTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
		...
		obj = self.los.putNonTimedElt(path, cpopath + 'los', i, obj)
		return obj

	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type setupstructureecesetupObj, run function getNonTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
		...
		if not status:
			self.frequency = ret_frequency
		self.los.getNonTimedElt(path, cpopath + 'los', i, obj)
	...
	def deleteData(self, path, cpopath):
		...
		self.los.deleteData(path, cpopath)


class losstructuresetup_line_exp(KeepInOrder):
	'''
	class losstructuresetup_line_exp
	Geometry of the line of sight.

	Attributes:
	- pivot_point : class pivot_pointstructurerzphi1Dexperimental
	...
	- second_point : class second_pointstructurerzphi1Dexperimental
	...
	- third_point : class third_pointstructurerzphi1Dexperimental
	...
	def __init__(self, base_path_in='los'):
		self.base_path = base_path_in
		self.idx = EMPTY_INT
		self.cpoTime = EMPTY_DOUBLE
		self.pivot_point = pivot_pointstructurerzphi1Dexperimental('pivot_point')
		...
		self.second_point = second_pointstructurerzphi1Dexperimental('second_point')
		self.horchordang2 = numpy.zeros(0, numpy.float64, order='C')
		self.verchordang2 = numpy.zeros(0, numpy.float64, order='C')
		self.third_point = third_pointstructurerzphi1Dexperimental('third_point')
		self.nchordpoints = EMPTY_INT

	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class losstructuresetup_line_exp\n'
	...
	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type losstructuresetup_line_exp, run function putSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type losstructuresetup_line_exp, run function replaceLastSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type losstructuresetup_line_exp, run function putNonTimed') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type losstructuresetup_line_exp, run function getSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type losstructuresetup_line_exp, run function build_non_resampled_data') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		if nbslice > 0:
			...
			check_status(status)
			for i in range(nbslice):
				slice = losstructuresetup_line_exp(self.base_path)
				slice.setExpIdx(self.idx)
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type losstructuresetup_line_expObj, run function putTimedElt') 
		cpopath = cpopath + '/' 
		return obj

	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type losstructuresetup_line_expObj, run function getTimedElt') 
		cpopath = cpopath + '/' 

	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type losstructuresetup_line_expObj, run function putNonTimedElt') 
		cpopath = cpopath + '/' 
	...
	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type losstructuresetup_line_expObj, run function getNonTimedElt') 
		cpopath = cpopath + '/' 
...
class pivot_pointstructurerzphi1Dexperimental(KeepInOrder):
	'''
	class pivot_pointstructurerzphi1Dexperimental
	Pivot point of each line of sight; Vector (nchords)
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class pivot_pointstructurerzphi1Dexperimental\n'
		s = self.r.__str__()
	...
	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type pivot_pointstructurerzphi1Dexperimental, run function putSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type pivot_pointstructurerzphi1Dexperimental, run function replaceLastSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type pivot_pointstructurerzphi1Dexperimental, run function putNonTimed') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type pivot_pointstructurerzphi1Dexperimental, run function getSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type pivot_pointstructurerzphi1Dexperimental, run function build_non_resampled_data') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		if nbslice > 0:
			...
			check_status(status)
			for i in range(nbslice):
				slice = pivot_pointstructurerzphi1Dexperimental(self.base_path)
				slice.setExpIdx(self.idx)
				slice.r = rVal
				slice.z = zVal
				slice.phi = phiVal
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type pivot_pointstructurerzphi1DexperimentalObj, run function putTimedElt') 
		cpopath = cpopath + '/' 
		return obj

	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type pivot_pointstructurerzphi1DexperimentalObj, run function getTimedElt') 
		cpopath = cpopath + '/' 

	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type pivot_pointstructurerzphi1DexperimentalObj, run function putNonTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
	...
	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type pivot_pointstructurerzphi1DexperimentalObj, run function getNonTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
...
class second_pointstructurerzphi1Dexperimental(KeepInOrder):
	'''
	class second_pointstructurerzphi1Dexperimental
	Second point defining the line of sight together with the pivot_point. In case the probing wave is reflected, this should be the position of the mirror. This data is redundant with horchordang1 and verchordang1. Vector (nchords).
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class second_pointstructurerzphi1Dexperimental\n'
		s = self.r.__str__()
		ret = ret + space + 'Attribute r\n' + space + s.replace('\n', '\n'+space) + '\n'
		s = self.z.__str__()
		ret = ret + space + 'Attribute z\n' + space + s.replace('\n', '\n'+space) + '\n'
	...
	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type second_pointstructurerzphi1Dexperimental, run function putSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type second_pointstructurerzphi1Dexperimental, run function replaceLastSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type second_pointstructurerzphi1Dexperimental, run function putNonTimed') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type second_pointstructurerzphi1Dexperimental, run function getSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type second_pointstructurerzphi1Dexperimental, run function build_non_resampled_data') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		if nbslice > 0:
			...
			check_status(status)
			for i in range(nbslice):
				slice = second_pointstructurerzphi1Dexperimental(self.base_path)
				slice.setExpIdx(self.idx)
				slice.r = rVal
				slice.z = zVal
				slice.phi = phiVal
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type second_pointstructurerzphi1DexperimentalObj, run function putTimedElt') 
		cpopath = cpopath + '/' 
		return obj

	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type second_pointstructurerzphi1DexperimentalObj, run function getTimedElt') 
		cpopath = cpopath + '/' 

	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type second_pointstructurerzphi1DexperimentalObj, run function putNonTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
			print ('putVect1DDoubleInObject : ' + cpopath + 'r') 
			print ('obj = ' + str(obj))
	...
	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type second_pointstructurerzphi1DexperimentalObj, run function getNonTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
			print ('getVect1DDoubleInObject : ' + cpopath + 'r') 
			print ('obj = ' + str(obj))
...
class third_pointstructurerzphi1Dexperimental(KeepInOrder):
	'''
	class third_pointstructurerzphi1Dexperimental
	Third point defining the reflected line of sight together with the second_point (undefined if the probing wave is not reflected). This data is redundant with horchordang2 and verchordang2. Vector (nchords).
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class third_pointstructurerzphi1Dexperimental\n'
		s = self.r.__str__()
		ret = ret + space + 'Attribute r\n' + space + s.replace('\n', '\n'+space) + '\n'
		s = self.z.__str__()
		ret = ret + space + 'Attribute z\n' + space + s.replace('\n', '\n'+space) + '\n'
	...
	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type third_pointstructurerzphi1Dexperimental, run function putSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type third_pointstructurerzphi1Dexperimental, run function replaceLastSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type third_pointstructurerzphi1Dexperimental, run function putNonTimed') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type third_pointstructurerzphi1Dexperimental, run function getSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type third_pointstructurerzphi1Dexperimental, run function build_non_resampled_data') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		if nbslice > 0:
			...
			check_status(status)
			for i in range(nbslice):
				slice = third_pointstructurerzphi1Dexperimental(self.base_path)
				slice.setExpIdx(self.idx)
				slice.r = rVal
				slice.z = zVal
				slice.phi = phiVal
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type third_pointstructurerzphi1DexperimentalObj, run function putTimedElt') 
		cpopath = cpopath + '/' 
		return obj

	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type third_pointstructurerzphi1DexperimentalObj, run function getTimedElt') 
		cpopath = cpopath + '/' 

	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type third_pointstructurerzphi1DexperimentalObj, run function putNonTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
			print ('putVect1DDoubleInObject : ' + cpopath + 'r') 
			print ('obj = ' + str(obj))
	...
	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type third_pointstructurerzphi1DexperimentalObj, run function getNonTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
			print ('getVect1DDoubleInObject : ' + cpopath + 'r') 
			print ('obj = ' + str(obj))
...
class measurestructureecemeasure(KeepInOrder):
	'''
	class measurestructureecemeasure
	Measured values

	Attributes:
	- harmonic : int
	   Harmonic detected by the  ECE channels. Time-dependent.
	- position : class positionstructurerzphi1Dexp
	   Position of the measurement. Time-dependent. Vector (nchannels)
	- te : class testructureexp1D
	   Electron temperature [eV]. Time-dependent. Vector (nchannels)
	'''
	...
	def __init__(self, base_path_in='measure'):
		...
		self.idx = EMPTY_INT
		self.cpoTime = EMPTY_DOUBLE
		self.harmonic = EMPTY_INT
		self.position = positionstructurerzphi1Dexp('position')
		self.te = testructureexp1D('te')

	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class measurestructureecemeasure\n'
		ret = ret + space + 'Attribute harmonic: ' + str(self.harmonic) + '\n'
	...
	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type measurestructureecemeasure, run function putSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		if (verb()):
			print ('path = ' + path + '  and cpopath = ' + cpopath) 
		status = ull.putIntSlice(self.idx, path, cpopath + 'harmonic', self.harmonic, self.cpoTime)
		check_status(status)
	...
	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type measurestructureecemeasure, run function replaceLastSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		if (verb()):
			print ('path = ' + path + '  and cpopath = ' + cpopath) 
		status = ull.replaceLastIntSlice(self.idx, path, cpopath + 'harmonic', self.harmonic)
		check_status(status)
	...
	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type measurestructureecemeasure, run function putNonTimed') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type measurestructureecemeasure, run function getSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		if (verb()):
			print ('path = ' + path + '  and cpopath = ' + cpopath) 
		status, ret_harmonic, retTime = ull.getIntSlice(self.idx, path, cpopath + 'harmonic', inTime, interpolMode)
		check_status(status)
		if not status:
			self.harmonic = ret_harmonic
			self.cpoTime = retTime
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type measurestructureecemeasure, run function build_non_resampled_data') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		list=[]
		if nbslice > 0:
			status, harmonicList = ull.getVect1DInt(self.idx, path, cpopath + 'harmonic')
			if len(harmonicList) == 0:
				harmonicList = numpy.resize(harmonicList, (nbslice))
			check_status(status)
			...
			for i in range(nbslice):
				slice = measurestructureecemeasure(self.base_path)
				slice.setExpIdx(self.idx)
				slice.harmonic = int(harmonicList[i].copy())
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type measurestructureecemeasureObj, run function putTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
			print ('putIntInObject : ' + cpopath + 'harmonic') 
			print ('obj = ' + str(obj))
		obj = ull.putIntInObject(self.idx, obj, cpopath + 'harmonic', i, self.harmonic)
	...
	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type measurestructureecemeasureObj, run function getTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
			print ('getIntInObject : ' + cpopath + 'harmonic') 
			print ('obj = ' + str(obj))
		status, ret_harmonic = ull.getIntFromObject(self.idx, obj, cpopath + 'harmonic', i)
		check_status(status)
		if not status:
			self.harmonic = ret_harmonic
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type measurestructureecemeasureObj, run function putNonTimedElt') 
		cpopath = cpopath + '/' 
	...
	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type measurestructureecemeasureObj, run function getNonTimedElt') 
		cpopath = cpopath + '/' 
	...
	def deleteData(self, path, cpopath):
		...
		if (verb()):
			print ('path = ' + path + '  and cpopath = ' + cpopath) 
		ull.deleteData(self.idx, path, cpopath + 'harmonic')
...
class testructureexp1D(KeepInOrder):
	'''
	class testructureexp1D
	Electron temperature [eV]. Time-dependent. Vector (nchannels)
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class testructureexp1D\n'
		s = self.value.__str__()
		ret = ret + space + 'Attribute value\n' + space + s.replace('\n', '\n'+space) + '\n'
		s = self.abserror.__str__()
		ret = ret + space + 'Attribute abserror\n' + space + s.replace('\n', '\n'+space) + '\n'
	...
	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type testructureexp1D, run function putSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type testructureexp1D, run function replaceLastSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type testructureexp1D, run function putNonTimed') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type testructureexp1D, run function getSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type testructureexp1D, run function build_non_resampled_data') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		if nbslice > 0:
			...
			check_status(status)
			for i in range(nbslice):
				slice = testructureexp1D(self.base_path)
				slice.setExpIdx(self.idx)
				slice.value = valueList[:,i]
				slice.abserror = abserrorList[:,i]
				slice.relerror = relerrorList[:,i]
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type testructureexp1DObj, run function putTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
			print ('putVect1DDoubleInObject : ' + cpopath + 'value') 
			print ('obj = ' + str(obj))
	...
	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type testructureexp1DObj, run function getTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
			print ('getVect1DDoubleInObject : ' + cpopath + 'value') 
			print ('obj = ' + str(obj))
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type testructureexp1DObj, run function putNonTimedElt') 
		cpopath = cpopath + '/' 
		return obj

	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type testructureexp1DObj, run function getNonTimedElt') 
		cpopath = cpopath + '/' 
