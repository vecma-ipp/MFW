...
class power_conv(KeepInOrder):
	...
	- circuits : class circuitsstruct_arraycircuits: array of circuitsstruct_arraycircuitsObj objects
	...
	def __init__(self):
		self.base_path = 'power_conv'
		self.idx = EMPTY_INT
		self.cpoTime = EMPTY_DOUBLE
		self.maxOccurrences = 3
		self.datainfo = datainfostructuredatainfo('datainfo')
		self.cycle_type = ''
		self.circuits = circuitsstruct_arraycircuits('circuits')
		self.power_recirc = EMPTY_DOUBLE
		self.power_net = EMPTY_DOUBLE
		self.power_int = EMPTY_DOUBLE
		self.efficiency = EMPTY_DOUBLE
		self.codeparam = codeparamstructurecodeparam('codeparam')
		self.time = EMPTY_DOUBLE

	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class power_conv\n'
		ret = ret + space + 'Attribute datainfo\n ' + self.datainfo.__str__(depth+1)
		ret = ret + space + 'Attribute cycle_type: ' + str(self.cycle_type) + '\n'
		ret = ret + space + 'Attribute circuits\n ' + self.circuits.__str__(depth+1)
		ret = ret + space + 'Attribute power_recirc: ' + str(self.power_recirc) + '\n'
		ret = ret + space + 'Attribute power_net: ' + str(self.power_net) + '\n'
		ret = ret + space + 'Attribute power_int: ' + str(self.power_int) + '\n'
		ret = ret + space + 'Attribute efficiency: ' + str(self.efficiency) + '\n'
	...
	def putSlice(self, occurrence=0):
		...
		self.circuits.cpoTime = self.cpoTime
		self.circuits.putSlice(path, cpopath)
	...
	def replaceLastSlice(self, occurrence=0):
		...
		self.circuits.replaceLastSlice(path, cpopath)
	...
	def putNonTimed(self, occurrence=0):
		...
		status = ull.putString(self.idx, path, cpopath + 'cycle_type', self.cycle_type)
		check_status(status)
		self.circuits.putNonTimed(path, cpopath)
		status = ull.putDouble(self.idx, path, cpopath + 'power_recirc', self.power_recirc)
		check_status(status)
		status = ull.putDouble(self.idx, path, cpopath + 'power_net', self.power_net)
		check_status(status)
		status = ull.putDouble(self.idx, path, cpopath + 'power_int', self.power_int)
		check_status(status)
		status = ull.putDouble(self.idx, path, cpopath + 'efficiency', self.efficiency)
		check_status(status)
	...
	def getSlice(self, inTime, interpolMode, occurrence=0):
		...
		status, ret_cycle_type = ull.getString(self.idx, path, cpopath + 'cycle_type')
		check_status(status)
		if not status:
			self.cycle_type = ret_cycle_type
		self.circuits.getSlice(path, cpopath, inTime, interpolMode)
		status, ret_power_recirc = ull.getDouble(self.idx, path, cpopath + 'power_recirc')
		check_status(status)
		if not status:
			self.power_recirc = ret_power_recirc
		status, ret_power_net = ull.getDouble(self.idx, path, cpopath + 'power_net')
		check_status(status)
		if not status:
			self.power_net = ret_power_net
		status, ret_power_int = ull.getDouble(self.idx, path, cpopath + 'power_int')
		check_status(status)
		if not status:
			self.power_int = ret_power_int
		status, ret_efficiency = ull.getDouble(self.idx, path, cpopath + 'efficiency')
		check_status(status)
		if not status:
			self.efficiency = ret_efficiency
	...
	def build_non_resampled_data(self, occurrence=0):
		...
		if nbslice > 0:
			...
			status, cycle_typeVal = ull.getString(self.idx, path, cpopath + 'cycle_type')
			check_status(status)
			circuitsList = self.circuits.build_non_resampled_data(path, cpopath, nbslice)
			status, power_recircVal = ull.getDouble(self.idx, path, cpopath + 'power_recirc')
			check_status(status)
			status, power_netVal = ull.getDouble(self.idx, path, cpopath + 'power_net')
			check_status(status)
			status, power_intVal = ull.getDouble(self.idx, path, cpopath + 'power_int')
			check_status(status)
			status, efficiencyVal = ull.getDouble(self.idx, path, cpopath + 'efficiency')
			check_status(status)
			...
			for i in range(nbslice):
				...
				slice.cycle_type = cycle_typeVal
				slice.circuits = circuitsList[i]
				slice.power_recirc = power_recircVal
				slice.power_net = power_netVal
				slice.power_int = power_intVal
				slice.efficiency = efficiencyVal
	...
	def deleteData(self, occurrence=0):
		...
		ull.deleteData(self.idx, path, cpopath + 'cycle_type')
		ull.deleteData(self.idx, path, cpopath + 'circuits')
		ull.deleteData(self.idx, path, cpopath + 'power_recirc')
		ull.deleteData(self.idx, path, cpopath + 'power_net')
		ull.deleteData(self.idx, path, cpopath + 'power_int')
		ull.deleteData(self.idx, path, cpopath + 'efficiency')
		self.codeparam.deleteData(path, cpopath)
		ull.deleteData(self.idx, path, cpopath + 'time')


class power_convArray:
	'''
	class power_convArray
	Power conversion system. Time-dependent CPO.

	Attributes:
	- array : list of power_conv
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class power_convArray nb_cpos=%d\n' %(len(self.array))
		for i in range(len(self.array)):
			ret = ret + space + 'power_conv cpos=%d\n' %(i) + self.array[i].__str__(depth+1)
		return ret
...
class circuitsstruct_arraycircuits:
	'''
	class circuitsstruct_arraycircuits
	Description of the circuit of the power conversion system. Array of structure. (ncircuits).

	Attributes:
	- array : list of circuitsstruct_arraycircuitsObj 
	'''

	def __init__(self, base_path_in='circuits'):
		self.base_path = base_path_in
		self.idx = EMPTY_INT
		self.cpoTime = EMPTY_DOUBLE
		self.array = []
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class circuitsstruct_arraycircuits\n'
		for i in range(len(self.array)):
			ret = ret + space + 'circuitsstruct_arraycircuits[%d] = \n' %(i) + self.array[i].__str__(depth+1)
		return ret
	...
	def resize(self, nbelt):
		...
		for i in range(nbelt):
			self.array.append(circuitsstruct_arraycircuitsObj(self.base_path))
			self.array[i].setExpIdx(self.idx)

	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type circuitsstruct_arraycircuits, run function putSlice') 

	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type circuitsstruct_arraycircuits, run function replaceLastSlice') 

	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type circuitsstruct_arraycircuits, run function putNonTimed') 
		if (dev()):
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type circuitsstruct_arraycircuits, run function getSlice') 
		if (dev()):
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type circuitsstruct_arraycircuits, run function build_non_resampled_data') 
		list=[]
		if nbslice > 0:
			if (dev()):
				print ('getObject('+path+', '+cpopath+self.base_path+', NON_TIMED)')
			...
			if status:
				print ('Failed to get data: ' + path + '/' + cpopath + self.base_path + ' (NON_TIMED)')
				for i in range(nbslice):
					list.append(circuitsstruct_arraycircuits(self.base_path))
				return list
			obj_size = ull.getObjectDim(self.idx, obj)
			for n in range(nbslice):
				slice = circuitsstruct_arraycircuits(self.base_path)
				slice.setExpIdx(self.idx)
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type circuitsstruct_arraycircuits, run function putNonTimedElt') 
		if (dev()):
	...
	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type circuitsstruct_arraycircuits, run function getNonTimedElt') 
		if (dev()):
...
class circuitsstruct_arraycircuitsObj(KeepInOrder):
	'''
	class circuitsstruct_arraycircuitsObj
	Description of the circuit of the power conversion system. Array of structure. (ncircuits).

	Attributes:
	- component : class componentstruct_arraypower_conv_component: array of componentstruct_arraypower_conv_componentObj objects
	...
	def __init__(self, base_path_in='circuits'):
		...
		self.idx = EMPTY_INT
		self.cpoTime = EMPTY_DOUBLE
		self.component = componentstruct_arraypower_conv_component('component')
		self.power_net = EMPTY_DOUBLE
		self.power_int = EMPTY_DOUBLE
		self.efficiency = EMPTY_DOUBLE
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class circuitsstruct_arraycircuitsObj\n'
		ret = ret + space + 'Attribute component\n ' + self.component.__str__(depth+1)
		ret = ret + space + 'Attribute power_net: ' + str(self.power_net) + '\n'
		ret = ret + space + 'Attribute power_int: ' + str(self.power_int) + '\n'
		ret = ret + space + 'Attribute efficiency: ' + str(self.efficiency) + '\n'
		return ret
	...
	def setExpIdx(self, idx):
		self.idx = idx
		self.component.setExpIdx(idx)

	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type circuitsstruct_arraycircuitsObj, run function putNonTimedElt') 
		cpopath = cpopath + '/'
		obj = self.component.putNonTimedElt(path, cpopath + 'component', i, obj)
		if (dev()):
			print ('putDoubleInObject : ' + cpopath + 'power_net') 
			print ('obj = ' + str(obj))
		obj = ull.putDoubleInObject(self.idx, obj, cpopath + 'power_net', i, self.power_net)
		if (dev()):
			print ('putDoubleInObject : ' + cpopath + 'power_int') 
			print ('obj = ' + str(obj))
		obj = ull.putDoubleInObject(self.idx, obj, cpopath + 'power_int', i, self.power_int)
		if (dev()):
			print ('putDoubleInObject : ' + cpopath + 'efficiency') 
			print ('obj = ' + str(obj))
		obj = ull.putDoubleInObject(self.idx, obj, cpopath + 'efficiency', i, self.efficiency)
		return obj

	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type circuitsstruct_arraycircuitsObj, run function getNonTimedElt') 
		cpopath = cpopath + '/'
		self.component.getNonTimedElt(path, cpopath + 'component', i, obj)
		if (dev()):
			print ('getDoubleInObject : ' + cpopath + 'power_net') 
			print ('obj = ' + str(obj))
		status, ret_power_net = ull.getDoubleFromObject(self.idx, obj, cpopath + 'power_net', i)
		check_status(status)
		if not status:
			self.power_net = ret_power_net
		if (dev()):
			print ('getDoubleInObject : ' + cpopath + 'power_int') 
			print ('obj = ' + str(obj))
		status, ret_power_int = ull.getDoubleFromObject(self.idx, obj, cpopath + 'power_int', i)
		check_status(status)
		if not status:
			self.power_int = ret_power_int
		if (dev()):
			print ('getDoubleInObject : ' + cpopath + 'efficiency') 
			print ('obj = ' + str(obj))
		status, ret_efficiency = ull.getDoubleFromObject(self.idx, obj, cpopath + 'efficiency', i)
		check_status(status)
		if not status:
			self.efficiency = ret_efficiency


class componentstruct_arraypower_conv_component:
	'''
	class componentstruct_arraypower_conv_component
	Description of the components of the power conversion system. Array of structure (ncomp).

	Attributes:
	- array : list of componentstruct_arraypower_conv_componentObj 
	'''

	def __init__(self, base_path_in='component'):
		self.base_path = base_path_in
		self.idx = EMPTY_INT
		self.cpoTime = EMPTY_DOUBLE
		self.array = []
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class componentstruct_arraypower_conv_component\n'
		for i in range(len(self.array)):
			ret = ret + space + 'componentstruct_arraypower_conv_component[%d] = \n' %(i) + self.array[i].__str__(depth+1)
		return ret
	...
	def resize(self, nbelt):
		self.array = []
		for i in range(nbelt):
			self.array.append(componentstruct_arraypower_conv_componentObj(self.base_path))
			self.array[i].setExpIdx(self.idx)

	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type componentstruct_arraypower_conv_component, run function putSlice') 

	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type componentstruct_arraypower_conv_component, run function replaceLastSlice') 

	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type componentstruct_arraypower_conv_component, run function putNonTimed') 
		if (dev()):
			print ('beginObject(None, 0, '+path+'/'+cpopath+self.base_path+', NON_TIMED)')
		obj = ull.beginObject(self.idx, None, 0, path + '/' + cpopath + self.base_path, NON_TIMED)
		for i in range(len(self.array)):
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type componentstruct_arraypower_conv_component, run function getSlice') 
		if (dev()):
			print ('getObject('+path+', '+cpopath+self.base_path+', NON_TIMED)')
		status, obj = ull.getObject(self.idx, path, cpopath + self.base_path, NON_TIMED)
		if status:
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type componentstruct_arraypower_conv_component, run function build_non_resampled_data') 
		list=[]
		if nbslice > 0:
			if (dev()):
				print ('getObject('+path+', '+cpopath+self.base_path+', NON_TIMED)')
			...
			if status:
				print ('Failed to get data: ' + path + '/' + cpopath + self.base_path + ' (NON_TIMED)')
				for i in range(nbslice):
					list.append(componentstruct_arraypower_conv_component(self.base_path))
				return list
			obj_size = ull.getObjectDim(self.idx, obj)
			for n in range(nbslice):
				slice = componentstruct_arraypower_conv_component(self.base_path)
				slice.setExpIdx(self.idx)
				slice.resize(obj_size)
				for i in range(obj_size):
					slice.array[i].getNonTimedElt(path, self.base_path, i, obj)
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type componentstruct_arraypower_conv_component, run function putNonTimedElt') 
		if (dev()):
			print ('beginObject idx=%d' %(self.idx)) 
		if (dev()):
			print ('beginObject('+str(obj)+', '+str(i)+', '+path+'/'+cpopath+', NON_TIMED)')
	...
	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type componentstruct_arraypower_conv_component, run function getNonTimedElt') 
		if (dev()):
			print ('getObjectFromObject('+str(obj)+', '+cpopath+', '+str(i)+')')
		status, obj2 = ull.getObjectFromObject(self.idx, obj, cpopath, i)
		if status:
...
class componentstruct_arraypower_conv_componentObj(KeepInOrder):
	'''
	class componentstruct_arraypower_conv_componentObj
	Description of the components of the power conversion system. Array of structure (ncomp).
	...
	def __init__(self, base_path_in='component'):
		...
		self.cpoTime = EMPTY_DOUBLE
		self.name = ''
		self.temp_in = EMPTY_DOUBLE
		self.temp_out = EMPTY_DOUBLE
		self.press_in = EMPTY_DOUBLE
		self.press_out = EMPTY_DOUBLE
		self.power = EMPTY_DOUBLE
		self.flow = EMPTY_DOUBLE

	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class componentstruct_arraypower_conv_componentObj\n'
		ret = ret + space + 'Attribute name: ' + str(self.name) + '\n'
		ret = ret + space + 'Attribute temp_in: ' + str(self.temp_in) + '\n'
		ret = ret + space + 'Attribute temp_out: ' + str(self.temp_out) + '\n'
		ret = ret + space + 'Attribute press_in: ' + str(self.press_in) + '\n'
		ret = ret + space + 'Attribute press_out: ' + str(self.press_out) + '\n'
		ret = ret + space + 'Attribute power: ' + str(self.power) + '\n'
		ret = ret + space + 'Attribute flow: ' + str(self.flow) + '\n'
		return ret
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type componentstruct_arraypower_conv_componentObj, run function putNonTimedElt') 
		cpopath = cpopath + '/'
		if (dev()):
		...
		if (dev()):
			print ('putDoubleInObject : ' + cpopath + 'temp_in') 
			print ('obj = ' + str(obj))
		obj = ull.putDoubleInObject(self.idx, obj, cpopath + 'temp_in', i, self.temp_in)
		if (dev()):
			print ('putDoubleInObject : ' + cpopath + 'temp_out') 
			print ('obj = ' + str(obj))
		obj = ull.putDoubleInObject(self.idx, obj, cpopath + 'temp_out', i, self.temp_out)
		if (dev()):
			print ('putDoubleInObject : ' + cpopath + 'press_in') 
			print ('obj = ' + str(obj))
		obj = ull.putDoubleInObject(self.idx, obj, cpopath + 'press_in', i, self.press_in)
		if (dev()):
			print ('putDoubleInObject : ' + cpopath + 'press_out') 
			print ('obj = ' + str(obj))
		obj = ull.putDoubleInObject(self.idx, obj, cpopath + 'press_out', i, self.press_out)
		if (dev()):
			print ('putDoubleInObject : ' + cpopath + 'power') 
			print ('obj = ' + str(obj))
		obj = ull.putDoubleInObject(self.idx, obj, cpopath + 'power', i, self.power)
		if (dev()):
			print ('putDoubleInObject : ' + cpopath + 'flow') 
			print ('obj = ' + str(obj))
		obj = ull.putDoubleInObject(self.idx, obj, cpopath + 'flow', i, self.flow)
		return obj

	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type componentstruct_arraypower_conv_componentObj, run function getNonTimedElt') 
		cpopath = cpopath + '/'
		if (dev()):
		...
		if not status:
			self.name = ret_name
		if (dev()):
			print ('getDoubleInObject : ' + cpopath + 'temp_in') 
			print ('obj = ' + str(obj))
		status, ret_temp_in = ull.getDoubleFromObject(self.idx, obj, cpopath + 'temp_in', i)
		check_status(status)
		if not status:
			self.temp_in = ret_temp_in
		if (dev()):
			print ('getDoubleInObject : ' + cpopath + 'temp_out') 
			print ('obj = ' + str(obj))
		status, ret_temp_out = ull.getDoubleFromObject(self.idx, obj, cpopath + 'temp_out', i)
		check_status(status)
		if not status:
			self.temp_out = ret_temp_out
		if (dev()):
			print ('getDoubleInObject : ' + cpopath + 'press_in') 
			print ('obj = ' + str(obj))
		status, ret_press_in = ull.getDoubleFromObject(self.idx, obj, cpopath + 'press_in', i)
		check_status(status)
		if not status:
			self.press_in = ret_press_in
		if (dev()):
			print ('getDoubleInObject : ' + cpopath + 'press_out') 
			print ('obj = ' + str(obj))
		status, ret_press_out = ull.getDoubleFromObject(self.idx, obj, cpopath + 'press_out', i)
		check_status(status)
		if not status:
			self.press_out = ret_press_out
		if (dev()):
		...
		if (dev()):
			print ('getDoubleInObject : ' + cpopath + 'flow') 
			print ('obj = ' + str(obj))
		status, ret_flow = ull.getDoubleFromObject(self.idx, obj, cpopath + 'flow', i)
		check_status(status)
		if not status:
			self.flow = ret_flow
