...
class efcc(KeepInOrder):
	...
	- coil : class coilstruct_arraycoil: array of coilstruct_arraycoilObj objects
	...
	def putSlice(self, occurrence=0):
		...
		self.coil.cpoTime = self.cpoTime
		self.coil.putSlice(path, cpopath)
	...
	def replaceLastSlice(self, occurrence=0):
		...
		self.coil.replaceLastSlice(path, cpopath)
	...
	def putNonTimed(self, occurrence=0):
		...
		self.coil.putNonTimed(path, cpopath)
	...
	def getSlice(self, inTime, interpolMode, occurrence=0):
		...
		self.coil.getSlice(path, cpopath, inTime, interpolMode)
	...
	def build_non_resampled_data(self, occurrence=0):
		...
		if nbslice > 0:
			...
			coilList = self.coil.build_non_resampled_data(path, cpopath, nbslice)
	...
	def deleteData(self, occurrence=0):
		...
		ull.deleteData(self.idx, path, cpopath + 'coil')
...
class efccArray:
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class efccArray nb_cpos=%d\n' %(len(self.array))
		for i in range(len(self.array)):
			ret = ret + space + 'efcc cpos=%d\n' %(i) + self.array[i].__str__(depth+1)
		return ret
...
class coilstruct_arraycoil:
	'''
	class coilstruct_arraycoil
	Individual coil. Time-dependent. Structure array. Replicate this coil structure for coil element in the efcc array.

	Attributes:
	- array : list of coilstruct_arraycoilObj 
	'''

	def __init__(self, base_path_in='coil'):
		self.base_path = base_path_in
		self.idx = EMPTY_INT
		self.cpoTime = EMPTY_DOUBLE
		self.array = []
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class coilstruct_arraycoil\n'
		for i in range(len(self.array)):
			ret = ret + space + 'coilstruct_arraycoil[%d] = \n' %(i) + self.array[i].__str__(depth+1)
		return ret
	...
	def resize(self, nbelt):
		...
		for i in range(nbelt):
			self.array.append(coilstruct_arraycoilObj(self.base_path))
			self.array[i].setExpIdx(self.idx)

	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type coilstruct_arraycoil, run function putSlice') 
		if (dev()):
	...
	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type coilstruct_arraycoil, run function replaceLastSlice') 
		if (dev()):
			print ('beginObject(None, 0, '+path+'/'+cpopath+self.base_path+', TIMED)')
	...
	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type coilstruct_arraycoil, run function putNonTimed') 
		if (dev()):
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type coilstruct_arraycoil, run function getSlice') 
		if (dev()):
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type coilstruct_arraycoil, run function build_non_resampled_data') 
		list=[]
		if nbslice > 0:
			if (dev()):
			...
			if status:
				...
				for i in range(nbslice):
					list.append(coilstruct_arraycoil(self.base_path))
				return list
			for n in range(nbslice):
				...
				if status:
					...
					continue
				obj_size = ull.getObjectDim(self.idx, obj)
				slice = coilstruct_arraycoil(self.base_path)
				slice.setExpIdx(self.idx)
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type coilstruct_arraycoil, run function putTimedElt') 
		if (dev()):
	...
	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type coilstruct_arraycoil, run function getTimedElt') 
		if (dev()):
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type coilstruct_arraycoil, run function putNonTimedElt') 
		if (dev()):
			print ('beginObject idx=%d' %(self.idx)) 
		if (dev()):
	...
	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type coilstruct_arraycoil, run function getNonTimedElt') 
		if (dev()):
			print ('getObjectFromObject('+str(obj)+', '+cpopath+', '+str(i)+')')
		status, obj2 = ull.getObjectFromObject(self.idx, obj, cpopath, i)
		if status:
...
class coilstruct_arraycoilObj(KeepInOrder):
	'''
	class coilstruct_arraycoilObj
	Individual coil. Time-dependent. Structure array. Replicate this coil structure for coil element in the efcc array.

	Attributes:
	- desc_coils : class desc_coilsstructuredesc_coils
	...
	def __init__(self, base_path_in='coil'):
		...
		self.idx = EMPTY_INT
		self.cpoTime = EMPTY_DOUBLE
		self.desc_coils = desc_coilsstructuredesc_coils('desc_coils')
		self.coilcurrent = coilcurrentstructureexp1D('coilcurrent')
		self.coilvoltage = coilvoltagestructureexp1D('coilvoltage')

	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class coilstruct_arraycoilObj\n'
		ret = ret + space + 'Attribute desc_coils\n ' + self.desc_coils.__str__(depth+1)
	...
	def setExpIdx(self, idx):
		self.idx = idx
		self.desc_coils.setExpIdx(idx)
		self.coilcurrent.setExpIdx(idx)
		self.coilvoltage.setExpIdx(idx)

	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type coilstruct_arraycoilObj, run function putTimedElt') 
		cpopath = cpopath + '/'
	...
	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type coilstruct_arraycoilObj, run function getTimedElt') 
		cpopath = cpopath + '/'
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type coilstruct_arraycoilObj, run function putNonTimedElt') 
		cpopath = cpopath + '/'
		obj = self.desc_coils.putNonTimedElt(path, cpopath + 'desc_coils', i, obj)
	...
	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type coilstruct_arraycoilObj, run function getNonTimedElt') 
		cpopath = cpopath + '/'
		self.desc_coils.getNonTimedElt(path, cpopath + 'desc_coils', i, obj)
...
class desc_coilsstructuredesc_coils(KeepInOrder):
	'''
	class desc_coilsstructuredesc_coils
	Description of the coils
	...
	- edges : class edgesstruct_arrayedges: array of edgesstruct_arrayedgesObj objects
	...
	def __init__(self, base_path_in='desc_coils'):
		self.base_path = base_path_in
		self.idx = EMPTY_INT
		self.cpoTime = EMPTY_DOUBLE
		self.name = ''
		self.res = EMPTY_DOUBLE
		self.nturns = EMPTY_INT
		self.closed = ''
		self.edges = edgesstruct_arrayedges('edges')

	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class desc_coilsstructuredesc_coils\n'
		ret = ret + space + 'Attribute name: ' + str(self.name) + '\n'
		ret = ret + space + 'Attribute res: ' + str(self.res) + '\n'
		ret = ret + space + 'Attribute nturns: ' + str(self.nturns) + '\n'
		ret = ret + space + 'Attribute closed: ' + str(self.closed) + '\n'
		ret = ret + space + 'Attribute edges\n ' + self.edges.__str__(depth+1)
		return ret

	def setExpIdx(self, idx):
		self.idx = idx
		self.edges.setExpIdx(idx)

	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type desc_coilsstructuredesc_coils, run function putSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		if (verb()):
			print ('path = ' + path + '  and cpopath = ' + cpopath) 
		self.edges.cpoTime = self.cpoTime
		self.edges.putSlice(path, cpopath)

	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type desc_coilsstructuredesc_coils, run function replaceLastSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		if (verb()):
			print ('path = ' + path + '  and cpopath = ' + cpopath) 
		self.edges.replaceLastSlice(path, cpopath)

	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type desc_coilsstructuredesc_coils, run function putNonTimed') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		check_status(status)
		status = ull.putDouble(self.idx, path, cpopath + 'res', self.res)
		check_status(status)
		status = ull.putInt(self.idx, path, cpopath + 'nturns', self.nturns)
		check_status(status)
		status = ull.putString(self.idx, path, cpopath + 'closed', self.closed)
		check_status(status)
		self.edges.putNonTimed(path, cpopath)

	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type desc_coilsstructuredesc_coils, run function getSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		if not status:
			self.name = ret_name
		status, ret_res = ull.getDouble(self.idx, path, cpopath + 'res')
		check_status(status)
		if not status:
			self.res = ret_res
		...
		status, ret_closed = ull.getString(self.idx, path, cpopath + 'closed')
		check_status(status)
		if not status:
			self.closed = ret_closed
		self.edges.getSlice(path, cpopath, inTime, interpolMode)

	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type desc_coilsstructuredesc_coils, run function build_non_resampled_data') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		if nbslice > 0:
			...
			check_status(status)
			status, resVal = ull.getDouble(self.idx, path, cpopath + 'res')
			check_status(status)
			status, nturnsVal = ull.getInt(self.idx, path, cpopath + 'nturns')
			check_status(status)
			status, closedVal = ull.getString(self.idx, path, cpopath + 'closed')
			check_status(status)
			edgesList = self.edges.build_non_resampled_data(path, cpopath, nbslice)
			for i in range(nbslice):
				slice = desc_coilsstructuredesc_coils(self.base_path)
				slice.setExpIdx(self.idx)
				slice.name = nameVal
				slice.res = resVal
				slice.nturns = nturnsVal
				slice.closed = closedVal
				slice.edges = edgesList[i]
				list.append(slice)
		else:
			print ('error, nbslice must be > 0 and got:' + str(nbslice))
		return list
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type desc_coilsstructuredesc_coilsObj, run function putTimedElt') 
		cpopath = cpopath + '/' 
		return obj

	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type desc_coilsstructuredesc_coilsObj, run function getTimedElt') 
		cpopath = cpopath + '/' 

	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type desc_coilsstructuredesc_coilsObj, run function putNonTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
		...
		if (dev()):
			print ('putDoubleInObject : ' + cpopath + 'res') 
			print ('obj = ' + str(obj))
		obj = ull.putDoubleInObject(self.idx, obj, cpopath + 'res', i, self.res)
		if (dev()):
			print ('putIntInObject : ' + cpopath + 'nturns') 
			print ('obj = ' + str(obj))
		obj = ull.putIntInObject(self.idx, obj, cpopath + 'nturns', i, self.nturns)
		if (dev()):
			print ('putStringInObject : ' + cpopath + 'closed') 
			print ('obj = ' + str(obj))
		obj = ull.putStringInObject(self.idx, obj, cpopath + 'closed', i, self.closed)
		obj = self.edges.putNonTimedElt(path, cpopath + 'edges', i, obj)
		return obj

	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type desc_coilsstructuredesc_coilsObj, run function getNonTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
		...
		if not status:
			self.name = ret_name
		if (dev()):
			print ('getDoubleInObject : ' + cpopath + 'res') 
			print ('obj = ' + str(obj))
		status, ret_res = ull.getDoubleFromObject(self.idx, obj, cpopath + 'res', i)
		check_status(status)
		if not status:
			self.res = ret_res
		if (dev()):
			print ('getIntInObject : ' + cpopath + 'nturns') 
			print ('obj = ' + str(obj))
		status, ret_nturns = ull.getIntFromObject(self.idx, obj, cpopath + 'nturns', i)
		check_status(status)
		if not status:
			self.nturns = ret_nturns
		if (dev()):
		...
		if not status:
			self.closed = ret_closed
		self.edges.getNonTimedElt(path, cpopath + 'edges', i, obj)
	...
	def deleteData(self, path, cpopath):
		...
		ull.deleteData(self.idx, path, cpopath + 'closed')
		ull.deleteData(self.idx, path, cpopath + 'edges')


class edgesstruct_arrayedges:
	'''
	class edgesstruct_arrayedges
	Edges defining the coil volume faces. Structure array. Replicate this edge structure N-times for N-edge cross sections. Use just one for wire coil approximation.

	Attributes:
	- array : list of edgesstruct_arrayedgesObj 
	'''

	def __init__(self, base_path_in='edges'):
		self.base_path = base_path_in
		self.idx = EMPTY_INT
		self.cpoTime = EMPTY_DOUBLE
		self.array = []
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class edgesstruct_arrayedges\n'
		for i in range(len(self.array)):
			ret = ret + space + 'edgesstruct_arrayedges[%d] = \n' %(i) + self.array[i].__str__(depth+1)
		return ret
	...
	def resize(self, nbelt):
		self.array = []
		for i in range(nbelt):
			self.array.append(edgesstruct_arrayedgesObj(self.base_path))
			self.array[i].setExpIdx(self.idx)

	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type edgesstruct_arrayedges, run function putSlice') 

	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type edgesstruct_arrayedges, run function replaceLastSlice') 

	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type edgesstruct_arrayedges, run function putNonTimed') 
		if (dev()):
			print ('beginObject(None, 0, '+path+'/'+cpopath+self.base_path+', NON_TIMED)')
		obj = ull.beginObject(self.idx, None, 0, path + '/' + cpopath + self.base_path, NON_TIMED)
		for i in range(len(self.array)):
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type edgesstruct_arrayedges, run function getSlice') 
		if (dev()):
			print ('getObject('+path+', '+cpopath+self.base_path+', NON_TIMED)')
		status, obj = ull.getObject(self.idx, path, cpopath + self.base_path, NON_TIMED)
		if status:
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type edgesstruct_arrayedges, run function build_non_resampled_data') 
		list=[]
		if nbslice > 0:
			if (dev()):
				print ('getObject('+path+', '+cpopath+self.base_path+', NON_TIMED)')
			...
			if status:
				print ('Failed to get data: ' + path + '/' + cpopath + self.base_path + ' (NON_TIMED)')
				for i in range(nbslice):
					list.append(edgesstruct_arrayedges(self.base_path))
				return list
			obj_size = ull.getObjectDim(self.idx, obj)
			for n in range(nbslice):
				slice = edgesstruct_arrayedges(self.base_path)
				slice.setExpIdx(self.idx)
				slice.resize(obj_size)
				for i in range(obj_size):
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type edgesstruct_arrayedges, run function putNonTimedElt') 
		if (dev()):
			print ('beginObject idx=%d' %(self.idx)) 
		if (dev()):
			print ('beginObject('+str(obj)+', '+str(i)+', '+path+'/'+cpopath+', NON_TIMED)')
	...
	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('field '+self.base_path+' of type edgesstruct_arrayedges, run function getNonTimedElt') 
		if (dev()):
			print ('getObjectFromObject('+str(obj)+', '+cpopath+', '+str(i)+')')
		status, obj2 = ull.getObjectFromObject(self.idx, obj, cpopath, i)
		if status:
...
class edgesstruct_arrayedgesObj(KeepInOrder):
	'''
	class edgesstruct_arrayedgesObj
	Edges defining the coil volume faces. Structure array. Replicate this edge structure N-times for N-edge cross sections. Use just one for wire coil approximation.

	Attributes:
	- edge_rzphi : class edge_rzphistructurerzphi1D
	...
	def __init__(self, base_path_in='edges'):
		...
		self.idx = EMPTY_INT
		self.cpoTime = EMPTY_DOUBLE
		self.edge_rzphi = edge_rzphistructurerzphi1D('edge_rzphi')

	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class edgesstruct_arrayedgesObj\n'
		ret = ret + space + 'Attribute edge_rzphi\n ' + self.edge_rzphi.__str__(depth+1)
		return ret

	def setExpIdx(self, idx):
		self.idx = idx
		self.edge_rzphi.setExpIdx(idx)

	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type edgesstruct_arrayedgesObj, run function putNonTimedElt') 
		cpopath = cpopath + '/'
		obj = self.edge_rzphi.putNonTimedElt(path, cpopath + 'edge_rzphi', i, obj)
		return obj

	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type edgesstruct_arrayedgesObj, run function getNonTimedElt') 
		cpopath = cpopath + '/'
		self.edge_rzphi.getNonTimedElt(path, cpopath + 'edge_rzphi', i, obj)


class edge_rzphistructurerzphi1D(KeepInOrder):
	'''
	class edge_rzphistructurerzphi1D
	Sequence of points describing a coil edge. Vector (npoints)
	...
	def __init__(self, base_path_in='edge_rzphi'):
		self.base_path = base_path_in
		self.idx = EMPTY_INT
		self.cpoTime = EMPTY_DOUBLE
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class edge_rzphistructurerzphi1D\n'
		s = self.r.__str__()
	...
	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type edge_rzphistructurerzphi1D, run function putSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type edge_rzphistructurerzphi1D, run function replaceLastSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type edge_rzphistructurerzphi1D, run function putNonTimed') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type edge_rzphistructurerzphi1D, run function getSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type edge_rzphistructurerzphi1D, run function build_non_resampled_data') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		if nbslice > 0:
			...
			check_status(status)
			for i in range(nbslice):
				slice = edge_rzphistructurerzphi1D(self.base_path)
				slice.setExpIdx(self.idx)
				slice.r = rVal
				slice.z = zVal
				slice.phi = phiVal
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type edge_rzphistructurerzphi1DObj, run function putTimedElt') 
		cpopath = cpopath + '/' 
		return obj

	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type edge_rzphistructurerzphi1DObj, run function getTimedElt') 
		cpopath = cpopath + '/' 

	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type edge_rzphistructurerzphi1DObj, run function putNonTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
	...
	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type edge_rzphistructurerzphi1DObj, run function getNonTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
