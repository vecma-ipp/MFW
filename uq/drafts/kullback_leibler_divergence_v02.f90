...
MODULE math_routines
  ...
  CONTAINS
    ...
    ELEMENTAL FUNCTION sigma_b_2(x)
        IMPLICIT NONE
        REAL(rkind), INTENT(IN) :: x !expected to be in 0..100
        REAL(rkind) :: sigma_b_2
        sigma_b_2=0.1 +0.01*x ! linear increasing
    ...
    ELEMENTAL FUNCTION density1(x)
        ...
        sb=sigma_b_1(xtest)
        if (x>=mu) then
           density1=1.0/(sqrt(2*pi)*sa) * exp(-0.5*((x-mu)/sa)**2)
        else
           density1=1.0/(sqrt(2*pi)*sb) * exp(-0.5*((x-mu)/sb)**2)
        endif
    END FUNCTION density1

    FUNCTION density1_v(x)
        ...
        sb=sigma_b_1(xtest)
        where (x>=mu) 
           density1_v=1.0/(sqrt(2*pi)*sa) * exp(-0.5*((x-mu)/sa)**2)
        elsewhere 
           density1_v=1.0/(sqrt(2*pi)*sb) * exp(-0.5*((x-mu)/sb)**2)
        endwhere
    ...
    ELEMENTAL FUNCTION density2(x)
        ...
        sb=sigma_b_2(xtest)
        if (x>=mu) then
           density2=1.0/(sqrt(2*pi)*sa) * exp(-0.5*((x-mu)/sa)**2)
        else
           density2=1.0/(sqrt(2*pi)*sb) * exp(-0.5*((x-mu)/sb)**2)
        endif
    END FUNCTION density2

    FUNCTION density2_v(x)
        ...
        sb=sigma_b_2(xtest)
        where (x>=mu)
           density2_v=1.0/(sqrt(2*pi)*sa) * exp(-0.5*((x-mu)/sa)**2)
        elsewhere
           density2_v=1.0/(sqrt(2*pi)*sb) * exp(-0.5*((x-mu)/sb)**2)
        endwhere
    END FUNCTION density2_v

    SUBROUTINE calc_cumulative_function(y_cumulative,x_support_points,func,x_integration_lb,x_integration_ub)
        IMPLICIT NONE
        ...
        DO i=1,n
           y_cumulative(i)=qsimp(func,x_integration_lb,x_support_points(i))
        ENDDO
    END SUBROUTINE calc_cumulative_function

    FUNCTION divergence_func(x)
        IMPLICIT NONE
        ...
        SELECT CASE (divergence)
           CASE (1)          !Hellinger-Distance H(p1;p2) =: 1 - int_dx [sqrt(p1(x)*p2(x))]
                             ! 0<= H <= 1
           ...
           CASE (3)        !Wasserstein-metric: int_dx abs (cum(p1(-inf..x))-cum(p2(-inf..x))) : Difference of cumulative functions
              if (xtest_new) then
                 ...
                 call calc_cumulative_function(y_cum1,x_support_points,density1_v,x_integration_lb,x_integration_ub)
                 call calc_cumulative_function(y_cum2,x_support_points,density2_v,x_integration_lb,x_integration_ub)
              ...
              !ENDDO
           CASE DEFAULT
                write(*,*)'ERROR: this divergence function is not implemented. choice=',divergence
        END SELECT  
...
!BL
	...
	REAL(rkind) :: absa,absb
     ...
     RECURSIVE FUNCTION qsimp(func,a,b)
        IMPLICIT NONE
...
USE f90_kind
USE math_routines,        ONLY: qsimp,xtest,divergence,divergence_func,mean1,mean2,sigma_a_1,sigma_a_2,sigma_b_1,sigma_b_2,&
                                density1,density2,xtest_new,x_integration_lb,x_integration_ub
implicit none
REAL(rkind),PARAMETER :: x_min =0.0, x_max=100.0, dx= 0.2
REAL(rkind)    :: divergence_value,x

  write(*,*)'Please enter desired divergence: (1) Hellinger  (2) Jensen-Shannon (3) Wasserstein-1 :  choice='
  read(*,*)divergence
  xtest=x_min
  ...
  DO
      ...
      SELECT CASE (divergence)
      CASE (1)
         write(*,*)xtest,1-divergence_value !by definition of Hellinger distance: 1- (int_dx sqrt(p1*p2))
         write(40+divergence,*)xtest,1-divergence_value
      CASE (2)
         write(*,*)xtest,divergence_value/log(2.d0)   
         write(40+divergence,*)xtest,divergence_value/log(2.d0)  !division by ln(2) to map result into [0..1]-range
      CASE (3)
         write(*,*)xtest,divergence_value
         write(40+divergence,*)xtest,divergence_value
      CASE DEFAULT
