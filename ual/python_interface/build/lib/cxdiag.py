...
class cxdiag:
	...
	- setup : class setupstructurecxsetup
	   diagnostic setup information
	- measure : class measurestructurecxmeasure
	   Measured values
...
class cxdiagArray:
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class cxdiagArray nb_cpos=%d\n' %(len(self.array))
		for i in range(len(self.array)):
			ret = ret + space + 'cxdiag cpos=%d\n' %(i) + self.array[i].__str__(depth+1)
		return ret
...
class setupstructurecxsetup:
	...
	- amn : numpy.ndarray 1D with float
	   Mass of the emmitting impurity. Varies according to channels since they are spanning different lines of sight; Vector (nchannels)
	- zn : numpy.ndarray 1D with float
	   Nuclear charge of the emmitting impurity. Varies according to channels since they are spanning different lines of sight; Vector (nchannels)
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class setupstructurecxsetup\n'
		s = self.amn.__str__()
	...
	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type setupstructurecxsetup, run function putSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type setupstructurecxsetup, run function replaceLastSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type setupstructurecxsetup, run function putNonTimed') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type setupstructurecxsetup, run function getSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type setupstructurecxsetup, run function build_non_resampled_data') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		if nbslice > 0:
			...
			for i in range(nbslice):
				slice = setupstructurecxsetup(self.base_path)
				slice.setExpIdx(self.idx)
				slice.amn = amnVal
				slice.zn = znVal
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type setupstructurecxsetupObj, run function putTimedElt') 
		cpopath = cpopath + '/' 
	...
	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type setupstructurecxsetupObj, run function getTimedElt') 
		cpopath = cpopath + '/' 
		self.position.getTimedElt(path, cpopath + 'position', i, obj)

	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type setupstructurecxsetupObj, run function putNonTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
	...
	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type setupstructurecxsetupObj, run function getNonTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
...
class measurestructurecxmeasure:
	'''
	class measurestructurecxmeasure
	Measured values
	...
	- vtor : class vtorstructureexp1D
	   Toroidal velocity [m/s]. Vector (nchannels)
	- vpol : class vpolstructureexp1D
	   Poloidal velocity [m/s]. Vector (nchannels)
	'''
	...
	def __init__(self, base_path_in='measure'):
		...
		self.vtor = vtorstructureexp1D('vtor')
		self.vpol = vpolstructureexp1D('vpol')

	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class measurestructurecxmeasure\n'
	...
	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type measurestructurecxmeasure, run function putSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type measurestructurecxmeasure, run function replaceLastSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type measurestructurecxmeasure, run function putNonTimed') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type measurestructurecxmeasure, run function getSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type measurestructurecxmeasure, run function build_non_resampled_data') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		if nbslice > 0:
			...
			for i in range(nbslice):
				slice = measurestructurecxmeasure(self.base_path)
				slice.setExpIdx(self.idx)
				slice.ti = tiList[i]
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type measurestructurecxmeasureObj, run function putTimedElt') 
		cpopath = cpopath + '/' 
		obj = self.ti.putTimedElt(path, cpopath + 'ti', i, obj)
		obj = self.vtor.putTimedElt(path, cpopath + 'vtor', i, obj)
		obj = self.vpol.putTimedElt(path, cpopath + 'vpol', i, obj)
		return obj

	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type measurestructurecxmeasureObj, run function getTimedElt') 
		cpopath = cpopath + '/' 
		self.ti.getTimedElt(path, cpopath + 'ti', i, obj)
		self.vtor.getTimedElt(path, cpopath + 'vtor', i, obj)
		self.vpol.getTimedElt(path, cpopath + 'vpol', i, obj)

	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type measurestructurecxmeasureObj, run function putNonTimedElt') 
		cpopath = cpopath + '/' 
		obj = self.ti.putNonTimedElt(path, cpopath + 'ti', i, obj)
		obj = self.vtor.putNonTimedElt(path, cpopath + 'vtor', i, obj)
		obj = self.vpol.putNonTimedElt(path, cpopath + 'vpol', i, obj)
		return obj

	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type measurestructurecxmeasureObj, run function getNonTimedElt') 
		cpopath = cpopath + '/' 
		self.ti.getNonTimedElt(path, cpopath + 'ti', i, obj)
		self.vtor.getNonTimedElt(path, cpopath + 'vtor', i, obj)
...
class vtorstructureexp1D:
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class vtorstructureexp1D\n'
		s = self.value.__str__()
		ret = ret + space + 'Attribute value\n' + space + s.replace('\n', '\n'+space) + '\n'
		s = self.abserror.__str__()
		ret = ret + space + 'Attribute abserror\n' + space + s.replace('\n', '\n'+space) + '\n'
	...
	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type vtorstructureexp1D, run function putSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type vtorstructureexp1D, run function replaceLastSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type vtorstructureexp1D, run function putNonTimed') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type vtorstructureexp1D, run function getSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type vtorstructureexp1D, run function build_non_resampled_data') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		if nbslice > 0:
			...
			check_status(status)
			for i in range(nbslice):
				slice = vtorstructureexp1D(self.base_path)
				slice.setExpIdx(self.idx)
				slice.value = valueList[:,i]
				slice.abserror = abserrorList[:,i]
				slice.relerror = relerrorList[:,i]
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type vtorstructureexp1DObj, run function putTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
			print ('putVect1DDoubleInObject : ' + cpopath + 'value') 
			print ('obj = ' + str(obj))
	...
	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type vtorstructureexp1DObj, run function getTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
			print ('getVect1DDoubleInObject : ' + cpopath + 'value') 
			print ('obj = ' + str(obj))
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type vtorstructureexp1DObj, run function putNonTimedElt') 
		cpopath = cpopath + '/' 
		return obj

	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type vtorstructureexp1DObj, run function getNonTimedElt') 
		cpopath = cpopath + '/' 
...
class vpolstructureexp1D:
	...
	def __str__(self, depth=0):
		space = depth*'\t'
		ret = space + 'class vpolstructureexp1D\n'
		s = self.value.__str__()
		ret = ret + space + 'Attribute value\n' + space + s.replace('\n', '\n'+space) + '\n'
		s = self.abserror.__str__()
		ret = ret + space + 'Attribute abserror\n' + space + s.replace('\n', '\n'+space) + '\n'
	...
	def putSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type vpolstructureexp1D, run function putSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def replaceLastSlice(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type vpolstructureexp1D, run function replaceLastSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def putNonTimed(self, path, cpopath):
		if (verb()):
			print ('field '+self.base_path+' of type vpolstructureexp1D, run function putNonTimed') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def getSlice(self, path, cpopath, inTime, interpolMode):
		if (verb()):
			print ('field '+self.base_path+' of type vpolstructureexp1D, run function getSlice') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
	...
	def build_non_resampled_data(self, path, cpopath, nbslice):
		if (verb()):
			print ('field '+self.base_path+' of type vpolstructureexp1D, run function build_non_resampled_data') 
		if cpopath=='':
			cpopath = self.base_path + '/'
		else:
			cpopath = cpopath + self.base_path + '/' 
		...
		if nbslice > 0:
			...
			check_status(status)
			for i in range(nbslice):
				slice = vpolstructureexp1D(self.base_path)
				slice.setExpIdx(self.idx)
				slice.value = valueList[:,i]
				slice.abserror = abserrorList[:,i]
				slice.relerror = relerrorList[:,i]
	...
	def putTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type vpolstructureexp1DObj, run function putTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
			print ('putVect1DDoubleInObject : ' + cpopath + 'value') 
			print ('obj = ' + str(obj))
	...
	def getTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type vpolstructureexp1DObj, run function getTimedElt') 
		cpopath = cpopath + '/' 
		if (dev()):
			print ('getVect1DDoubleInObject : ' + cpopath + 'value') 
			print ('obj = ' + str(obj))
	...
	def putNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type vpolstructureexp1DObj, run function putNonTimedElt') 
		cpopath = cpopath + '/' 
		return obj

	def getNonTimedElt(self, path, cpopath, i, obj):
		if (verb()):
			print ('object of type vpolstructureexp1DObj, run function getNonTimedElt') 
		cpopath = cpopath + '/' 
