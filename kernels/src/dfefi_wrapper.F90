...
contains
  ...
  subroutine dfefi2buf(equil_in, corep_in, coret_out) 
#ifdef MPI2
    use mpi
#endif
    use iso_c_binding
    ...
    corep_file_in = TRIM(tmpdir)//TRIM(username)//'_dfefi_coreprof_in.cpo'
    equil_file_in = TRIM(tmpdir)//TRIM(username)//'_dfefi_equilibrium_in.cpo'
    ...
    call c_getMillis(t0)
#endif
    call fill_param(code_parameters, 'dfefi.xml', '', 'dfefi.xsd')

    !print *,"run dfefi routine"
    call dfefi(equil, corep, coret, code_parameters)
#ifdef PERF
    call c_getMillis(t1)
    texec = t1 - t0
#endif
    ...
    if (irank.eq.0) then
       ...
       cpt = cpt+1
       coret_file_out = 'dfefi_coretransp_'//cptstr//'.cpo'
  ...
  end subroutine dfefi2buf


  
end module dfefi_wrapper
