program orb5_kernelB
  ...
  do it = init_step,target_step
...
#endif
        print *,'ORB5_B kernel: receive data'
        corep_size = BUF_SIZE
     ...
     print *,"ORB-% kernel: calling wrapper"
     call orb5cpo2buf(equil_in, corep_in, coret_out)
#ifdef PERF
     call c_getMillis(tt1)
     twrap = tt1 - tt0
#endif
     ...
     if (irank.eq.0) then
        ...
        call c_getMillis(tt0)
#endif
        print *,'ORB5_B kernel: send data'
        call MUSCLE_Send(c_char_"coretransp_out"//char(0), coret_out, &
             %REF(size(coret_out)) ,%REF(MUSCLE_RAW))
#ifdef PERF
        call c_getMillis(tt1)
        tsend = tt1 - tt0
        print *,'PERF_ORB5:',it,trecv,twrap,tsend,tread,tbarrier,tiowrite,tioread,texec,twrite
#endif
...
#ifdef DEBUG
  print *,'ORB5_B kernel: end of loop'
#endif
