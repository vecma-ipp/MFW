...
!!$  !!! should not be necessary as subroutine is in module
!!$  interface
!!$     subroutine orb5_cpo(equil, corep, coret)
!!$       use euitm_schemas
!!$       type (type_equilibrium), pointer ::  equil(:)
!!$       type (type_coreprof), pointer :: corep(:)
!!$       type (type_coretransp), pointer :: coret(:)
!!$     end subroutine orb5_cpo
!!$  end interface

contains


  subroutine orb5cpo2buf(equil_in, corep_in, coret_out) 
#ifdef MPI2
    use mpi
#endif
    use module_orb5
    ...
    corep_file_in = TRIM(tmpdir)//TRIM(username)//'_orb5_coreprof_in.cpo'
    equil_file_in = TRIM(tmpdir)//TRIM(username)//'_orb5_equilibrium_in.cpo'
    ...
    if (irank.eq.0) then
       ...
       cpt = cpt+1
       coret_file_out = 'orb5_coretransp_'//cptstr//'.cpo'
  ...
  end subroutine orb5cpo2buf

 
end module orb5_wrapper
