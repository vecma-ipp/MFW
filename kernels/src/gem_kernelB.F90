program gem_kernelB
  ...
  do it = init_step,target_step
...
#endif
        print *,'GEM_B kernel: receive data'
        corep_size = BUF_SIZE
     ...
     call gem2buf(equil_in, corep_in, coret_out)
#ifdef PERF
     call c_getMillis(tt1)
     twrap = tt1 - tt0
#endif
     ...
     if (irank.eq.0) then
        ...
        call c_getMillis(tt0)
#endif
        print *,'GEM_B kernel: send data'
        call MUSCLE_Send(c_char_"coretransp_out"//char(0), coret_out, &
             %REF(size(coret_out)) ,%REF(MUSCLE_RAW))
#ifdef PERF
        call c_getMillis(tt1)
        tsend = tt1 - tt0
        print *,'PERF_GEM:',it,trecv,twrap,tsend,tread,tbarrier,tiowrite,tioread,texec,twrite
#endif
...
#ifdef DEBUG
  print *,'GEM_B kernel: end of loop'
#endif
