...
contains
  ...
  subroutine partial_init2buf(path, &
       corep_out, &
!       coret_out, &
!       cores_out, &
!       corei_out, &
...
!!$    end if
    ...
    corep_file = trim(init_path)//"coreprof_in.cpo"
...
!    corei_file = trim(init_path)//"ets_coreimpur_in.cpo"
    equil_file = trim(init_path)//"equilibrium_in.cpo"
...
!    call deallocate_cpo(toroidf)
  ...
  end subroutine partial_init2buf



end module partial_init_wrapper
