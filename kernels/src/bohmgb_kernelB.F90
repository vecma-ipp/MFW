program bohmgb_kernelB
  ...
  propName = c_char_"cus_ftubes"//C_NULL_CHAR
  call MUSCLE_Get_Property(propName, prop)
  print *,trim(propName),' = ',trim(prop)
  read(prop,'(l)') cus_ftubes

  propName = c_char_"N_ftubes"//C_NULL_CHAR
  call MUSCLE_Get_Property(propName,prop)
  print *,trim(propName),' = ',trim(prop)
  read(prop,'(i4)') nftubes
  ...
  do it = init_step,target_step
...
#endif
     print *,'BOHMGB_B kernel: receive data'
     corep_size = BUF_SIZE
     ...
     call c_getMillis(tt0)
#endif
     print *,'BOHMGB_B kernel: call native routine'
     call bohmgb2buf(equil_in, corep_in, coret_out)
#ifdef PERF
     call c_getMillis(tt1)
     twrap = tt1 - tt0
#endif
     ...
     call c_getMillis(tt0)
#endif
     print *,'BOHMGB_B kernel: send data'
     call MUSCLE_Send(c_char_"coretransp_out"//char(0), coret_out, &
          %REF(size(coret_out)) ,%REF(MUSCLE_RAW))
#ifdef PERF
     call c_getMillis(tt1)
     tsend = tt1 - tt0
     print *,'PERF_BOHMGB:',it,trecv,twrap,tsend,tread,texec,twrite
#endif

     deallocate(coret_out)
  end do

  print *,'BOHMGB_B kernel: end of loop'
