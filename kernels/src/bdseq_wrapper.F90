...
contains
  ...
  subroutine bdseq2buf(equil_in, equil_out) 
    use iso_c_binding
    use string_binding
    use read_structures
    ...
    equil_file_in = TRIM(tmpdir)//TRIM(username)//'_bdseq_equilibrium_in.cpo'
    ...
    !...  run BDSEQ
    call fill_param(code_parameters, 'bdseq.xml', '', 'bdseq.xsd')

    print *,"trick coming from the bdseq wrapper version in ETS workflow"
    if(associated(equil_old(1)%profiles_1d%jparallel)) then
       !write(*,*) 'bdseq_wrapper copied jparallel to jphi [ERROR]'
       if(.not.associated(equil_old(1)%profiles_1d%jphi)) &
            allocate(equil_old(1)%profiles_1d%jphi(size(equil_old(1)%profiles_1d%jparallel)))
       equil_old(1)%profiles_1d%jphi = - equil_old(1)%profiles_1d%jparallel  ! -? ToDo
    endif

    print *,"run bdseq routine"
    call bdseq(equil_old, equil_new, code_parameters)

    print *,"another trick when bdseq returns"
    if(.not.associated(equil_new(1)%profiles_1d%jparallel)) then
       write(*,*) 'bdseq_wrapper copied jphi to jparallel [ERROR]'
       allocate(equil_new(1)%profiles_1d%jparallel(size(equil_new(1)%profiles_1d%jphi)))
       equil_new(1)%profiles_1d%jparallel = - equil_new(1)%profiles_1d%jphi
    endif
#ifdef PERF
    call c_getMillis(t1)
    texec = t1 - t0
    ...
    cpt = cpt+1
    equil_file_out = 'bdseq_equilibrium_'//cptstr//'.cpo'
  ...
  end subroutine bdseq2buf


end module bdseq_wrapper
