...
implicit none
  ...
  ! in practice single node can run 750 iterations per 24h; 
  ! use 5 for debug, 450 regularly
  ! ideally number of iterations should be callculated in advance 
  ! and/or passed through an xml file
  ...
  do it=1, STEPS
     ...
     ! DEBUG
     ! print *,"Here we call gem_cpo"

     ! write(0,*) "DEBUG mpi_comm_world=", MPI_COMM_WORLD, " npes=", npes, " irank=", irank, " ierr= ", ierr
     !   print *, "corep_ets associated: ", associated(corep_ets) !!!DEBUG
    
     if (irank.eq.0) then
       call open_write_file(12, 'corep_ets_'//itstr//'.cpo')
       call write_cpo(corep_in(1), 'coreprof')
       call close_write_file

       call open_write_file(14, 'equil_chease_'//itstr//'.cpo')
       call write_cpo(equil_in(1), 'equilibrium')
       call close_write_file
     end if
  
     !   print *, "equil_chease associated: ", associated(equil_chease) !!!DEBUG
     !print *, "first ", 64, " symbols of equil_chease: ", equil_chease(1:64) !!!DEBUG
   
     !   if (irank.eq.0) then
     !    call printcpo(corep_in)
     !   end if
     ...
     print *, "coret_gem associated: ", associated(coret_gem) !!!DEBUG
     !print *, "first ", 64, " symbols of coret_gem: ", coret_gem(1:64) !!!DEBUG
   ...
   !   call imp4dv_cpo(equil_in, corep_in, coret_gem, coret_imp4dv)
