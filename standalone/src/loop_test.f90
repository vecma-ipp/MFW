program loop
  ...
  open (unit = 10, file = data_path // "ets_coreprof_in.cpo", &
       status = 'old', form = 'formatted', &
       action = 'read', iostat = ios)
  if (ios == 0) then
     close (10)
     call open_read_file(10, data_path // "ets_coreprof_in.cpo")
     call read_cpo(corep_in(1), 'coreprof' )
     call close_read_file
  end if
  open (unit = 11, file = data_path // "ets_coretransp_in.cpo", &
       status = 'old', form = 'formatted', &
       action = 'read', iostat = ios)
  if (ios == 0) then
     close (11)
  ...
  open (unit = 12, file = data_path // "ets_coresource_in.cpo", &
       status = 'old', form = 'formatted', &
       action = 'read', iostat = ios)
  if (ios == 0) then
     close (12)
     call open_read_file(12, data_path // "ets_coresource_in.cpo")
     call read_cpo(cores_in(1), 'coresource' )
     call close_read_file
  end if
  open (unit = 13, file = data_path // "ets_coreimpur_in.cpo", &
       status = 'old', form = 'formatted', &
       action = 'read', iostat = ios)
  if (ios == 0) then
     close (13)
     call open_read_file(13, data_path // "ets_coreimpur_in.cpo")
     call read_cpo(corei_in(1), 'coreimpur' )
     call close_read_file
  end if
  open (unit = 14, file = data_path // "ets_equilibrium_in.cpo", &
       status = 'old', form = 'formatted', &
       action = 'read', iostat = ios)
  if (ios == 0) then
     close (14)
     call open_read_file(14, data_path // "ets_equilibrium_in.cpo")
     call read_cpo(equil_in(1), 'equilibrium' )
     call close_read_file
  end if
  open (unit = 15, file = data_path // "ets_toroidfield_in.cpo", &
       status = 'old', form = 'formatted', &
       action = 'read', iostat = ios)
  if (ios == 0) then
     close (15)
     call open_read_file(15, data_path // "ets_toroidfield_in.cpo")
