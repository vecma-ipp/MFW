...
!  if (rank==0)  write(6,*)'>>> Nb PROCS: ',nbprocs
  ...
  open (unit = 10, file = "ets_coreprof_in.cpo", &
        status = 'old', form = 'formatted', &
        action = 'read', iostat = ios)
  if (ios == 0) then
    close (10)
    call open_read_file(10, "ets_coreprof_in.cpo")
    call read_cpo(corep(1), 'coreprof' )
    call close_read_file
    apar(1) = corep(1)%te%boundary%value(1) 
    apar(2) = corep(1)%ti%boundary%value(1,1)

    !apar(1:npar)   = coret(1)%values(1)%te_transp%diff_eff       
    sigpar(1:npar) = 0.2d0*apar(:) ! parameter uncertainty
    
    else
      print *,"ERROR: ets.ets_coreprof_in.cpo file." 
      STOP
  end if
...
!  ! ... CALL ETS_STANDALONE
!  do i=1, nloc
!    call ets2file( "ets_coreprof_in.cpo", &
...
!    print*, '>>> Elapsed time= ',  t2 - t1, npterme,nfterme, npar, mmax
    ...
    open(4,file='samples.dat',status='unknown')
    do i=1, nloc
      write(4, *) aspeicher(i,:)
    enddo
