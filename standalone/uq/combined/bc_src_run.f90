! -*- coding: UTF-8 -*- 
!> @brief  run UQ for the loop:  ETS + CHEASE + BOHMGB.
!> Uncertainties in:
!> * Boudary conditions of Te and Te (in the eadge).
!> * Gaussian sources (electrons).
...
implicit none
  ...
  real(kind=8) :: Te_boundary, Ti_boundary, WTOT_el, RHEAT_el, FWHEAT_el
  real(8) :: params_in(3)
  ...
  namelist /bc_src_input_file/ Te_boundary, Ti_boundary, &
                          & WTOT_el, RHEAT_el, FWHEAT_el, &
                          & out_file  
  
  open(unit=9, file=trim(in_fname))
  read(9, bc_src_input_file)
...
!  end if
  ...
  params_in(1) = WTOT_el
  params_in(2) = RHEAT_el
  params_in(3) = FWHEAT_el
